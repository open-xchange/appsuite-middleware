...
public final class HeaderCacheMailProviderRegistry {
    ...
    private static final ConcurrentMap<String, MailProvider> cache = new ConcurrentHashMap<String, MailProvider>();

    /**
     * Initializes a new {@link HeaderCacheMailProviderRegistry}
     */
    private HeaderCacheMailProviderRegistry() {
        super();
    }
    ...
    public static MailProvider getMailProvider(final String protocolName) {
        ...
        MailProvider mp = cache.get(protocolName);
        if (null == mp) {
            MailProvider lookedUp = null;
            for (final Iterator<Map.Entry<Protocol, MailProvider>> iter = providers.entrySet().iterator(); null == lookedUp && iter.hasNext();) {
                final Map.Entry<Protocol, MailProvider> entry = iter.next();
                if (entry.getKey().isSupported(protocolName)) {
                    lookedUp = entry.getValue();
                }
            }
            if (null == lookedUp) {
                return null;
            }
            mp = cache.putIfAbsent(protocolName, lookedUp);
            if (null == mp) {
                mp = lookedUp;
            }
        }
    ...
    public static MailProvider unregisterMailProvider(final MailProvider provider) {
         ...
         */
        cache.clear();
        return providers.remove(provider.getProtocol());
    }
