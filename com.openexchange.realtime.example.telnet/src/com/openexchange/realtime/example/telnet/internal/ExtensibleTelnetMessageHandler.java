...
/**
 * {@link ExtensibleTelnetMessageHandler}
 * 
...
public class ExtensibleTelnetMessageHandler {
	...
	public void handle(TelnetChatMessage message) throws OXException {
		...
		String serviceName = message.getHeader("service");
		TelnetChatPlugin handler = handlers.get(serviceName);

		if (handler == null) {
			throw OXException.general("Don't know service " + serviceName);
		}
		
		handler.handleIncoming(message);
	}

	public void add(TelnetChatPlugin service) {
		handlers.put(service.getServiceName(), service);
	}

	public void remove(TelnetChatPlugin service) {
		handlers.remove(service.getServiceName());
	}
