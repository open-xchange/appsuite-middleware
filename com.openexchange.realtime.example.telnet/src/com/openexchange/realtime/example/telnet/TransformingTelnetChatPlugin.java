...
/**
 * {@link TransformingTelnetChatPlugin}
 *
...
public abstract class TransformingTelnetChatPlugin implements TelnetChatPlugin {
	...
	public void handleIncoming(TelnetChatMessage message) throws OXException {
		for(Stanza stanza: transform(message)) {
			if (stanza.getFrom() == null) {
				stanza.setFrom(message.getFrom());
			}
			
			if (stanza.getTo() == null) {
				stanza.setTo(message.getTo());
			}
			
			send(stanza, message.getSession());
		}
	}
	...
	public void handleOutgoing(Stanza stanza, ID to, ServerSession session,
		...
		for(TelnetChatMessage message: transform(stanza, to, session)) {
	...
	protected abstract List<Stanza> transform(TelnetChatMessage message) throws OXException;
	protected abstract List<TelnetChatMessage> transform(Stanza stanza, ID to, ServerSession session) throws OXException;
