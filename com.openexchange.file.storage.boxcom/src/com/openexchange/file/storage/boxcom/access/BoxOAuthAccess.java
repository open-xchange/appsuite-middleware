...
public class BoxOAuthAccess extends AbstractOAuthAccess {
    ...
    public void revoke() throws OXException {
        synchronized (this) {
            ...
            try {
                ...
                LOG.warn("The box.com OAuth token couldn't not be revoked for user '{}' in context '{}'. Status Code: {}, {}", getSession().getUserId(), getSession().getContextId(), statusCode, httpResponse.getStatusLine().getReasonPhrase());
    ...
    private OAuthAccount recreateTokenIfExpired(boolean considerExpired) throws OXException {
        ...
        if (considerExpired || scribeOAuthService.isExpired(boxOAuthAccount.getToken())) {
            ...
            oAuthService.updateAccount(accountId, arguments, getSession().getUserId(), getSession().getContextId(), boxOAuthAccount.getEnabledScopes(), boxOAuthAccount.getExpiration());
