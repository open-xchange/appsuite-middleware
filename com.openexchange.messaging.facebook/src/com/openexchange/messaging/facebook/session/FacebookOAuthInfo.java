...
/**
 * {@link FacebookOAuthInfo} - Initializes and provides Facebook OAuth information.
 * 
 * @author <a href="mailto:thorben.betten@open-xchange.com">Thorben Betten</a>
 * @since Open-Xchange v6.16
 */
public final class FacebookOAuthInfo {
     ...
     */
    public static FacebookOAuthInfo sessionFor(final MessagingAccount messagingAccount, final Session session) throws FacebookMessagingException {
        final FacebookOAuthInfoRegistry registry = FacebookOAuthInfoRegistry.getInstance();
        final int accountId = messagingAccount.getId();
        FacebookOAuthInfo facebookSession = registry.getSession(session.getContextId(), session.getUserId(), accountId);
        if (null == facebookSession) {
            final FacebookOAuthInfo newInstance = new FacebookOAuthInfo(messagingAccount, session.getUserId(), session.getContextId());
     ...
     */
    private FacebookOAuthInfo(final MessagingAccount messagingAccount, final int user, final int contextId) throws FacebookMessagingException {
        super();
        /*
