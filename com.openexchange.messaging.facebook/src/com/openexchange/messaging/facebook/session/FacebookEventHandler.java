        ...
        try {
            if (SessiondEventConstants.TOPIC_LAST_SESSION.equals(topic)) {
                ...
                if (null != contextId) {
                    ...
                    if (null != userId) {
                        if (FacebookOAuthAccessRegistry.getInstance().removeSessionIfLast(contextId, userId)) {
                            LOG.debug("Facebook session removed for user {} in context {}", userId, contextId);
                        }
                    }
                }
            }
