...
public class IMAPMessage extends MimeMessage implements ReadableMime {
	...
	boolean pk = getPeek();	// get before acquiring message cache lock
		...
		if (p.isREV1()) {
		    ...
		    if (pk) {
                long uid = getUID();
                b = uid < 0 ? p.peekBody(getSequenceNumber(), sectionId) : p.peekBody(uid, sectionId);
            } else {
                long uid = getUID();
                b = uid < 0 ? p.fetchBody(getSequenceNumber(), sectionId) : p.fetchBody(uid, sectionId);
            }
		    if (b != null)
			is = b.getByteArrayInputStream();
		} else {
	...
	InputStream is = null;
		...
		if (p.isREV1()) {
		    BODY b;
		    {
		        long uid = getUID();
		        if (uid < 0) {
		            b = p.peekBody(getSequenceNumber(), toSection("HEADER.FIELDS (" + name + ")"));
                } else {
                    b = p.peekBody(uid, toSection("HEADER.FIELDS (" + name + ")"));
                }
		    }
		    if (b != null)
			is = b.getByteArrayInputStream();
	...
	synchronized (getMessageCacheLock()) {
		...
		if (p.isREV1()) {
		    long uid = getUID();
		    BODY b = uid < 0 ? p.peekBody(getSequenceNumber(), toSection("HEADER")) : p.peekBody(uid, toSection("HEADER"));
		    if (b != null)
			is = b.getByteArrayInputStream();
		} else {
		    RFC822DATA rd = p.fetchRFC822(getSequenceNumber(), 
	...
	synchronized(getMessageCacheLock()) {
		...
		long uid = getUID();
		flags = uid < 0 ? p.fetchFlags(getSequenceNumber()) : p.fetchFlags(uid);
