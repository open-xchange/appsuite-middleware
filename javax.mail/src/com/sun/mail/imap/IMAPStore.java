...
public class IMAPStore extends Store
	...
	logger = new MailLogger(this.getClass(),
			"DEBUG " + name.toUpperCase(Locale.ENGLISH), session.getDebug(), session.getDebugOut());
    ...
    if (s != null) {
        propagateClientIpAddress = s;
        logger.log(Level.CONFIG, "mail.imap.propagate.clientipaddres: {0}", propagateClientIpAddress);
    }
   ...
   "added an Authenticated connection -- size: " +
    ...
    try {
        p = getStoreProtocol();
            return Collections.unmodifiableMap(p.getCapabilities());
    } catch (ProtocolException pex) {
        throw new MessagingException(pex.getMessage(), pex);
        } finally {
            releaseStoreProtocol(p);
     ...
     */
    public synchronized String getGreeting()
        throws MessagingException {
    IMAPProtocol p = null;
    try {
    p = getStoreProtocol();
