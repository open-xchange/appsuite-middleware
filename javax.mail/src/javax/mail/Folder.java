...
public abstract class Folder {
    ...
    // Vector of MessageRecent listeners
    private volatile Vector messageRecentListeners = null;

    /**
     * Add a listener for MessageRecent events on this Folder. <p>
     *
     * The implementation provided here adds this listener
     * to an internal list of MessageRecentListeners.
     *
     * @param l     the Listener for MessageRecent events
     * @see     javax.mail.event.MessageRecentEvent
     */
    public synchronized void addMessageRecentListener(MessageRecentListener l) { 
    if (messageRecentListeners == null)
        messageRecentListeners = new Vector();
    messageRecentListeners.addElement(l);
    }

    /**
     * Remove a MessageRecent listener. <p>
     *
     * The implementation provided here removes this listener
     * from the internal list of MessageCountListeners.
     *
     * @param l     the listener
     * @see     #addMessageRecentListener
     */
    public synchronized void
            removeMessageRecentListener(MessageRecentListener l) { 
    if (messageRecentListeners != null) 
        messageRecentListeners.removeElement(l); 
    }

    /**
     * Notify all MessageRecentListeners about recent messages
     * into this folder. Folder implementations are expected to use this 
     * method to broadcast MessageRecent events for indicating arrival of
     * recent messages. <p>
     *
     * The provided implementation queues the event into
     * an internal event queue. An event dispatcher thread dequeues
     * events from the queue and dispatches them to the registered
     * MessageRecentListeners. Note that the event dispatching occurs
     * in a separate thread, thus avoiding potential deadlock problems.
     */
    protected void notifyMessageRecentListeners() { 
    if (messageRecentListeners == null)
        return;

    MessageRecentEvent e = new MessageRecentEvent(this);

    queueEvent(e, messageRecentListeners); 
    }
