...
public class SMSBucketServiceImpl implements SMSBucketService {
    ...
    public int getSMSToken(int userId, int contextId) throws OXException {
        String userIdentifier = contextId + "/" + userId;
        int limit = getUserLimit(userId, contextId);
        ...
        ConfigView view = configFactory.getView(userId, contextId);
    ...
    private int getUserLimit(int userId, int contextId) throws OXException {
        ConfigViewFactory configFactory = Services.getService(ConfigViewFactory.class);
        if (configFactory == null) {
            throw ServiceExceptionCode.SERVICE_UNAVAILABLE.create(ConfigViewFactory.class.getName());
        }
