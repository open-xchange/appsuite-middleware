...
public class HumanReadableRecurrences {
    ...
    public HumanReadableRecurrences(Event event, Locale locale) {
        ...
        try {
            if (null != event && Strings.isNotEmpty(event.getRecurrenceRule())) {
    ...
    private String yearly() {
        ...
        if (byDayPart != null) {
            ...
            String days = parseDays(locale);
            if (Strings.isNotEmpty(days)) {
                return format(locale, HRRStrings.YEARLY_1, parseCount(locale), days, parseMonth(locale));
            }
            return "";
        }
    ...
    private String monthly() {
        ...
        if (byDayPart != null) {
            ...
            String days = parseDays(locale);
            if (Strings.isNotEmpty(days)) {
                return format(locale, HRRStrings.MONTHLY_2, parseCount(locale), days, Autoboxing.I(rrule.getInterval()));
            }
            return "";
        }
        List<Integer> byMonthDay = rrule.getByPart(Part.BYMONTHDAY);
    ...
    private String weekly() {
        if (rrule.getInterval() == 1) {
            String days = parseDays(locale);
            if (Strings.isNotEmpty(days)) {
                return format(locale, HRRStrings.WEEKLY_EACH, days);
            }
            return "";
        }
    ...
    private String parseDays(Locale locale) {
        ...
        return days.length() <= 2 ? "" : days.substring(0, days.length() - 2);
    }
