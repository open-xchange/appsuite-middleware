...
package com.openexchange.oauth.surdoc;
...
public final class SurDocOAuthServiceMetaData extends AbstractOAuthServiceMetaData implements com.openexchange.oauth.ScribeAware, Reloadable {
    ...
    public String processAuthorizationURL(String authUrl) {
        ...
        StringBuilder authUrlBuilder;
        {
            int nextPos = authUrl.indexOf('&', pos + 1);
            if (nextPos < 0) {
                String redirectUri = trimRedirectUri(authUrl.substring(pos + 14)) + AJAXUtility.encodeUrl(new StringBuilder(64).append("?state=").append("__ox").append(uuid).toString(), false, false);
    ...
    public String modifyCallbackURL(final String callbackUrl, final String currentHost, final Session session) {
        ...
        if (null != deferrer && deferrer.isDeferrerURLAvailable(session.getUserId(), session.getContextId())) {
            ...
            LOGGER.debug("Initializing SurDoc OAuth account for user {} in context {} with call-back URL: {}", session.getUserId(), session.getContextId(), retval);
            return retval;
        }
