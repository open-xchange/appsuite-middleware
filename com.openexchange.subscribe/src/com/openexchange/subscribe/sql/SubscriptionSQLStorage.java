...
public class SubscriptionSQLStorage implements SubscriptionStorage {
	...
	public SubscriptionSQLStorage(final DBProvider dbProvider,
			final GenericConfigurationStorageService storageService,
			final SubscriptionSourceDiscoveryService discoveryService) {
		this(dbProvider, DBTransactionPolicy.NORMAL_TRANSACTIONS,
				storageService, discoveryService);
	}

	public SubscriptionSQLStorage(final DBProvider dbProvider,
			final DBTransactionPolicy txPolicy,
			final GenericConfigurationStorageService storageService,
			final SubscriptionSourceDiscoveryService discoveryService) {
	...
	@Override
	public void forgetSubscription(final Subscription subscription)
			throws OXException {
		...
		try {
			writeConnection = dbProvider.getWriteConnection(subscription
					.getContext());
			...
			if (writeConnection != null) {
				try {
					...
					dbProvider.releaseWriteConnection(
							subscription.getContext(), writeConnection);
				}
			}
		}
	}
	...
	@Override
	public Subscription getSubscription(final Context ctx, final int id)
			throws OXException {
		...
		try {
			...
			final SELECT select = new SELECT("id", "user_id",
					"configuration_id", "source_id", "folder_id",
					"last_update", "enabled").FROM(subscriptions).WHERE(
					new EQUALS("id", PLACEHOLDER).AND(new EQUALS("cid",
							PLACEHOLDER)));
			...
			final List<Subscription> subscriptions = parseResultSet(resultSet,
					ctx, readConnection);
	...
	@Override
	public List<Subscription> getSubscriptions(final Context ctx,
			final String folderId) throws OXException {
		...
		try {
			...
			final SELECT select = new SELECT("id", "user_id",
					"configuration_id", "source_id", "folder_id",
					"last_update", "enabled").FROM(subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("folder_id",
							PLACEHOLDER)));
	...
	@Override
	public List<Subscription> getSubscriptionsOfUser(final Context ctx,
		...
		try {
			readConnection = dbProvider.getReadConnection(ctx);
			final SELECT select = new SELECT("*").FROM(subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("user_id",
							PLACEHOLDER)));
	...
	@Override
	public void rememberSubscription(final Subscription subscription)
			throws OXException {
	...
	private void delete(final Subscription subscription,
			final Connection writeConnection) throws SQLException, OXException,
			OXException {
		final DELETE delete = new DELETE().FROM(subscriptions).WHERE(
				new EQUALS("id", PLACEHOLDER)
		...
		new StatementBuilder()
				.executeStatement(writeConnection, delete, values);

		storageService.delete(writeConnection, subscription.getContext(),
				getConfigurationId(subscription));
	}

	private int save(final Subscription subscription,
			final Connection writeConnection) throws OXException, SQLException {
		final int configId = storageService.save(writeConnection,
				subscription.getContext(), subscription.getConfiguration());

		final int id = IDGenerator.getId(subscription.getContext()
				.getContextId(), Types.SUBSCRIPTION, writeConnection);

		final INSERT insert = new INSERT().INTO(subscriptions)
				.SET("id", PLACEHOLDER).SET("cid", PLACEHOLDER)
				.SET("user_id", PLACEHOLDER)
				.SET("configuration_id", PLACEHOLDER)
				.SET("source_id", PLACEHOLDER).SET("folder_id", PLACEHOLDER)
				.SET("last_update", PLACEHOLDER).SET("enabled", PLACEHOLDER);
		...
		new StatementBuilder()
				.executeStatement(writeConnection, insert, values);
		return id;
	}

	private void update(final Subscription subscription,
			final Connection writeConnection) throws OXException, OXException,
			SQLException {
		if (subscription.getConfiguration() != null) {
			final int configId = getConfigurationId(subscription);
			storageService.update(writeConnection, subscription.getContext(),
					configId, subscription.getConfiguration());
		}
		...
		update.WHERE(new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("id",
				PLACEHOLDER)));
		values.add(I(subscription.getContext().getContextId()));
		values.add(I(subscription.getId()));

		if (values.size() > 2) {
			new StatementBuilder().executeStatement(writeConnection, update,
					values);
		}
	}

	private int getConfigurationId(final Subscription subscription)
			throws OXException {
		int retval = 0;
		...
		StatementBuilder builder = null;
		try {
			readConection = dbProvider.getReadConnection(subscription
					.getContext());

			final SELECT select = new SELECT("configuration_id").FROM(
					subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("id",
							PLACEHOLDER)));
			...
			try {
				...
				dbProvider.releaseReadConnection(subscription.getContext(),
						readConection);
			}
		}
		return retval;
	...
	private List<Subscription> parseResultSet(final ResultSet resultSet,
			final Context ctx, final Connection readConnection)
		...
		while (resultSet.next()) {
			...
			storageService.fill(readConnection, ctx,
					resultSet.getInt("configuration_id"), content);

			subscription.setConfiguration(content);
			subscription.setSource(discoveryService.getSource(resultSet
					.getString("source_id")));
	...
	private boolean exist(final int id, final Context ctx) throws OXException {
		...
		try {
			readConnection = dbProvider.getReadConnection(ctx);
			final SELECT select = new SELECT("id").FROM(subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER).AND(new EQUALS("id",
							PLACEHOLDER)));
	...
	@Override
	public void deleteAllSubscriptionsForUser(final int userId,
	...
	@Override
	public void deleteAllSubscriptionsInContext(final int contextId,
			final Context ctx) throws OXException {
		Connection writeConnection = null;
		try {
			writeConnection = dbProvider.getWriteConnection(ctx);
			txPolicy.setAutoCommit(writeConnection, false);
			final DELETE delete = new DELETE().FROM(subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER));

			final List<Object> values = new ArrayList<Object>();
			values.add(I(ctx.getContextId()));

			new StatementBuilder().executeStatement(writeConnection, delete,
					values);
	...
	@Override
	public void deleteAllSubscriptionsWhereConfigMatches(
			final Map<String, Object> query, final String sourceId,
			final Context ctx) throws OXException {
		Connection writeConnection = null;
		try {
			writeConnection = dbProvider.getWriteConnection(ctx);
			txPolicy.setAutoCommit(writeConnection, false);
			final DELETE delete = new DELETE().FROM(subscriptions).WHERE(
					new EQUALS("cid", PLACEHOLDER).AND(
							new EQUALS("source_id", PLACEHOLDER)).AND(
							new EQUALS("configuration_id", PLACEHOLDER)));
			final List<Object> values = new ArrayList<Object>(Arrays.asList(
					null, null, null));
			...
			for (final Integer configId : configIds) {
				...
				final int deleted = new StatementBuilder().executeStatement(
						writeConnection, delete, values);
				if (deleted == 1) {
					// Delete the generic configuration only if the source_id
					// matched
