    ...
    public void handleRequest(HttpServletRequest httpRequest, HttpServletResponse httpResponse) throws IOException {
        try {
            ...
            if (session != null) {
                ...
                SessionUtility.removeOXCookies(httpRequest, httpResponse, Arrays.asList(LoginServlet.SESSION_PREFIX + hash, LoginServlet.SHARD, LoginServlet.SECRET_PREFIX + hash, getPublicSessionCookieName(httpRequest, new String[] { String.valueOf(session.getContextId()), String.valueOf(session.getUserId()) }), SAMLLoginTools.AUTO_LOGIN_COOKIE_PREFIX + hash));
