...
public class StrictValidationStrategy extends AbstractChainBasedValidationStrategy {
    ...
    protected ValidatorChain getValidatorChain(Response response, Binding binding) {
         ...
         */
        chain.add(new ResponseDestinationValidator(config.getAssertionConsumerServiceURL(), binding == Binding.HTTP_POST && response.isSigned()));
         ...
         */
        chain.add(new ResponseIssuerValidator(config.getIdentityProviderEntityID(), response.isSigned() || response.getEncryptedAssertions().size() > 0));
         ...
         */
        chain.add(new AssertionSignatureValidator(credentialProvider.getValidationCredential(), binding == Binding.HTTP_POST));
