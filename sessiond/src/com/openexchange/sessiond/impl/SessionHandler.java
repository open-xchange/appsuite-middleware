...
public class SessionHandler extends TimerTask {
    ...
    public SessionHandler(SessiondConfigInterface config) {
        this.config = config;
    }
    ...
    private static void prependContainer() {
        sessionList.add(0, new Hashtable<String, SessionControlObject>(maxSessions));
    ...
    protected static String addSession(final int userId,  final Credentials cred, final Context context, final String clientHost) throws SessiondException, MaxSessionLimitException {
        ...
        final SessionControlObject sessionControlObject = new SessionControlObject(session, lifeTime);
    ...
    protected static Session getSessionByRandomToken(final String randomToken) {
        ...
        for (int a = 0; a < numberOfSessionContainers; a++) {
            ...
            if (random.containsKey(randomToken)) {
                ...
                if (sessionControlObject.getCreationTime().getTime() + randomTokenTimeout >= now) {
    ...
    protected static boolean isValid(final SessionControlObject session) {
        ...
        try {
            // if (!session.getSession().getContext().isEnabled() || !session.getSession().getUserObject()
            //     .isMailEnabled()) {
            //     return false;
            // }
