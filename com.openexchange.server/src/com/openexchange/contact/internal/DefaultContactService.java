...
public abstract class DefaultContactService implements ContactService {
    ...
    public void createContact(Session session, String folderId, Contact contact, String vCard) throws OXException {
        ...
        if (Strings.isEmpty(vCard)) {
            LOG.warn("VCard for new contact " + contact.getInternalUserId() + " in context " + contact.getContextId() + " is empty. Will not persist it.");
            this.doCreateContact(session, folderId, contact);
            return;
        }
        this.doCreateContact(session, folderId, contact, vCard);
    }
    ...
    @Override
    public void updateContact(final Session session, final String folderId, final String id, final Contact contact, final Date lastRead, String vCard) throws OXException {
        Check.argNotNull(session, "session");
        Check.argNotNull(folderId, "folderId");
        Check.argNotNull(id, "id");
        Check.argNotNull(lastRead, "lastRead");
        ...
        if (Strings.isEmpty(vCard)) {
            LOG.warn("VCard for contact " + contact.getInternalUserId() + " in context " + contact.getContextId() + " is empty. Will not update it.");
            this.doUpdateContact(session, folderId, id, contact, lastRead);
            return;
        }
        this.doUpdateContact(session, folderId, id, contact, lastRead, vCard);
    }
    ...
    protected abstract void doCreateContact(Session session, String folderID, Contact contact, String vCard) throws OXException;
    ...
    protected abstract void doUpdateContact(Session session, String folderID, String objectID, Contact contact, Date lastRead, String vCard)
        throws OXException;
