...
public final class VirtualFolderStorageServiceTracker implements ServiceTrackerCustomizer<FolderStorage, FolderStorage> {
    ...
    private static final org.slf4j.Logger LOG =
        org.slf4j.LoggerFactory.getLogger(VirtualFolderStorageServiceTracker.class);
    ...
    public FolderStorage addingService(final ServiceReference<FolderStorage> reference) {
        ...
        // Add to registry
        if (VirtualFolderStorageRegistry.getInstance().addFolderStorage(treeId, addedService)) {
            return addedService;
        }
        // Nothing to track, return null
        context.ungetService(reference);
    ...
    public void removedService(final ServiceReference<FolderStorage> reference, final FolderStorage service) {
        if (null != service) {
            try {
                ...
                VirtualFolderStorageRegistry.getInstance().removeFolderStorage(treeId, service);
            } finally {
                context.ungetService(reference);
            }
        }
