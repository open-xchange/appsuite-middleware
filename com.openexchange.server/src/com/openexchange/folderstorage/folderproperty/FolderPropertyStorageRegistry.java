...
/**
 * {@link FolderPropertyStorageRegistry} - {@link FolderPropertyStorage}'s registry for real folder storages.
 *
 ...
 */
@SingletonService
public class FolderPropertyStorageRegistry implements FolderStorageDiscoverer {

    private final static org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(FolderPropertyStorageRegistry.class);
    ...
    private static FolderPropertyStorageRegistry instance;

    /**
     * Initializes a new {@link FolderPropertyStorageRegistry}.
     * 
     */
    private FolderPropertyStorageRegistry() {
        super();
    ...
    /**
     * Get the {@link FolderPropertyStorageRegistry} instance
     * 
     * @return The {@link FolderPropertyStorageRegistry} instance
     */
    public static FolderPropertyStorageRegistry getInstance() {
        if (null == instance) {
            instance = new FolderPropertyStorageRegistry();
        }
        return instance;
    }
    ...
    public boolean addFolderStorage(final String treeId, final FolderStorage folderStorage) {
        if ((!FolderStorage.ALL_TREE_ID.equals(treeId) || false == StoragePriority.HIGHEST.equals(folderStorage.getStoragePriority())) && false == FolderStorage.REAL_TREE_ID.equals(treeId)) {
            return false;
        }
        ...
        if (null != supportedContentTypes && supportedContentTypes.length > 0) {
            boolean error = false;
            final Set<ContentType> added = new HashSet<>(supportedContentTypes.length);
            for (ContentType contentType : supportedContentTypes) {
                if (null == contentTypes.putIfAbsent(contentType, folderStorage)) {
                    // Yapp, added
                    added.add(contentType);
                } else {
                    // No, already present                    
                    LOG.error("Duplicate folder storage for content type \"{}\"", contentType.toString(), new Throwable());
                    error = true;
                }
            }
            if (error) {
    ...
    public FolderStorage getFolderStorage(final String treeId, final String folderId) {
        if (false == generalStorages.isEmpty()) {
            /*
             * Check general storages first
             */
            for (final FolderStorage folderStorage : generalStorages) {
    ...
    public FolderStorage[] getTreeFolderStorages(final String treeId) {
        if (false == generalStorages.isEmpty()) {
             ...
             */
            for (final FolderStorage folderStorage : generalStorages) {
                if (false == StoragePriority.HIGHEST.equals(folderStorage.getStoragePriority()) && folderStorage.getFolderType().servesTreeId(treeId)) {
                    return new FolderStorage[] { folderStorage };
                }
            }
        }
    ...
    public void removeFolderStorage(final String treeId, final FolderStorage folderStorage) {
        if ((false == FolderStorage.ALL_TREE_ID.equals(treeId) || false == StoragePriority.HIGHEST.equals(folderStorage.getStoragePriority())) && false == FolderStorage.REAL_TREE_ID.equals(treeId)) {
            return;
        }
