/*
 * @copyright Copyright (c) OX Software GmbH, Germany <info@open-xchange.com>
 * @license AGPL-3.0
 *
 * This code is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with OX App Suite.  If not, see <https://www.gnu.org/licenses/agpl-3.0.txt>.
 *
 * Any use of the work other than as authorized under this license or copyright law is prohibited.
 *
 */

package com.openexchange.mail.compose;

import java.util.List;
import com.google.common.collect.ImmutableList;

/**
 * {@link DefaultAttachmentResult} - The default attachment result implementation.
 *
 * @author <a href="mailto:thorben.betten@open-xchange.com">Thorben Betten</a>
 * @since v7.10.5
 */
public class DefaultAttachmentResult extends AbstractAttachmentResult {

    private final List<? extends Attachment> attachments;

    /**
     * Initializes a new {@link DefaultAttachmentResult}.
     *
     * @param attachments The attachments as a result of the attachment-related operation
     * @param compositionSpaceInfo The composition space information
     * @throws IllegalArgumentException If attachments are <code>null</code>/empty or given composition space information is <code>null</code>
     * @throws NullPointerException If given attachments contain a <code>null</code> element
     */
    public DefaultAttachmentResult(List<? extends Attachment> attachments, CompositionSpaceInfo compositionSpaceInfo) {
        super(compositionSpaceInfo);
        if (attachments == null) {
            throw new IllegalArgumentException("Attachments must not be null");
        }
        this.attachments = attachments.isEmpty() ? ImmutableList.of() : ImmutableList.copyOf(attachments);
    }

    @Override
    public List<? extends Attachment> getAttachments() {
        return attachments;
    }

}
