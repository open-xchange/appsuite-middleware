...
public enum MailExceptionCode implements DisplayableOXExceptionCode {
    ...
    /**
     * OAuth-linked mail access is disabled for server %1$s with login %2$s (user=%3$s, context=%4$s, oauth=%5$s, oauthid=%6$s)
     */
    MAIL_ACCESS_DISABLED_OAUTH("OAuth-linked mail access is disabled for server %1$s with login %2$s (user=%3$s, context=%4$s, oauth=%5$s, oauthid=%6$s)", MailExceptionStrings.MAIL_ACCESS_DISABLED_OAUTH_MSG, CATEGORY_PERMISSION_DENIED, MAIL_ACCESS_DISABLED.getNumber()), // Yapp, same error code
    /**
