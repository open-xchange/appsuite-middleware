...
public final class MimeStructureFixer {
    ...
    private MimeMessage process0(final MimeMessage mimeMessage, final ContentType contentType) throws OXException {
        try {
            ...
            final AtomicReference<MimeMultipart> root = new AtomicReference<MimeMultipart>();
            handlePart((Multipart) mimeMessage.getContent(), new AtomicReference<MimeMultipart>(newMultipart), root);
            // Check if a new root has been set
            final MimeMultipart rootMimeMultipart = root.get();
            if (null == rootMimeMultipart) {
                ...
                rootMimeMultipart.addBodyPart(mimeBodyPart, 0);
                MessageUtility.setContent(rootMimeMultipart, mimeMessage);
            }
    ...
    private void handlePart(final Multipart multipart, final AtomicReference<MimeMultipart> newMultipartRef, final AtomicReference<MimeMultipart> root) throws MessagingException, IOException, OXException {
        ...
        if (toLowerCase(multipart.getContentType()).startsWith("multipart/mixed")) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if ((contentType.startsWith("text/htm") || contentType.startsWith("image/")) && isInline(bodyPart, contentType)) {
                    inlineCount++;
                }
            }
            if (inlineCount > 1) {
                ...
                final List<BodyPart> images = new ArrayList<BodyPart>(count);
                ...
                final MimeMultipart newSubMultipart = new MimeMultipart("related");
                {
                    ...
                    htmlBodyPart.setHeader(CONTENT_TYPE, MimeMessageUtility.foldContentType("text/html; charset=\"" + charset + "\""));
                    htmlBodyPart.setHeader(CONTENT_DISPOSITION, "inline");
                    htmlBodyPart.setHeader(MIME_VERSION, "1.0");
                    htmlBodyPart.setHeader(CONTENT_TRANSFER_ENC, "quoted-printable");
                    newSubMultipart.addBodyPart(htmlBodyPart);
                }
                for (final BodyPart imageBodyPart : images) {
                    newSubMultipart.addBodyPart(imageBodyPart);
                }
                // Replace new multipart
                newMultipartRef.set(newSubMultipart);
                // Check for others
                if (!others.isEmpty()) {
                    MimeMultipart rootMultipart = root.get();
                    if (null == rootMultipart) {
                        rootMultipart = new MimeMultipart("mixed");
                        root.set(rootMultipart);
                    }
                    for (final BodyPart otherBodyPart : others) {
                        rootMultipart.addBodyPart(otherBodyPart);
                    }
                }
                return;
            }
                ...
                if (contentType.startsWith("multipart/")) {
                    final AtomicReference<MimeMultipart> mpReference = new AtomicReference<MimeMultipart>(new MimeMultipart(contentType.getSubType()));
                    {
                        ...
                        if (content instanceof Multipart) {
                            handlePart((Multipart) content, mpReference, root);
                        } else {
                            handlePart(new MimeMultipart(bodyPart.getDataHandler().getDataSource()), mpReference, root);
                        }
                    }
                    final MimeBodyPart mimeBodyPart = new MimeBodyPart();
