...
/**
 * {@link MailAccessDelayQueue} - A clone of Doug Lea's {@link java.util.concurrent.DelayQueue} enhanced by {@link #poll(ElementFilter)}.
 *
 * @author <a href="mailto:thorben.betten@open-xchange.com">Thorben Betten</a>
 */
public class MailAccessDelayQueue extends AbstractQueue<MailAccessDelayElement> implements BlockingQueue<MailAccessDelayElement> {
    ...
    final PriorityQueue<MailAccessDelayElement> q = new PriorityQueue<MailAccessDelayElement>();

    /**
     * Creates a new <tt>MailAccessWatcherDelayQueue</tt> that is initially empty.
     */
    ...
    /**
     * Creates a <tt>MailAccessWatcherDelayQueue</tt> initially containing the elements of the given collection of {@link Delayed}
     * instances.
     *
     * @param c the collection of elements to initially contain
     * @throws NullPointerException if the specified collection or any of its elements are null
     */
    public MailAccessDelayQueue(final Collection<? extends MailAccessDelayElement> c) {
        this.addAll(c);
    }
     ...
     */
    @Override
    public boolean add(final MailAccessDelayElement e) {
        return offer(e);
    }
     ...
     */
    @Override
    public boolean offer(final MailAccessDelayElement e) {
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
            final MailAccessDelayElement first = q.peek();
            q.offer(e);
     ...
     */
    @Override
    public void put(final MailAccessDelayElement e) {
        offer(e);
    }
     ...
     */
    @Override
    public boolean offer(final MailAccessDelayElement e, final long timeout, final TimeUnit unit) {
        return offer(e);
    }
    ...
    public MailAccessDelayElement poll() {
        ...
        try {
            ...
            final MailAccessDelayElement x = q.poll();
            assert x != null;
            if (q.size() != 0) {
                available.signalAll();
            }
    ...
    /**
     * Retrieves and removes the head of this queue, or returns <tt>null</tt> if this queue has no elements with an expired delay or specified filter is not fulfilled.
     *
     * @return the head of this queue, or <tt>null</tt> if this queue has no elements with an expired delay
     */
    public MailAccessDelayElement poll(final ElementFilter filter) {
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
            final MailAccessDelayElement first = q.peek();
            if (first == null || first.getDelay(TimeUnit.NANOSECONDS) > 0 || (null != filter && !filter.accept(first))) {
                return null;
            }
            final MailAccessDelayElement x = q.poll();
            assert x != null;
     ...
     */
    @Override
    public MailAccessDelayElement poll(final long timeout, final TimeUnit unit) throws InterruptedException {
     ...
     */
    @Override
    public int drainTo(final Collection<? super MailAccessDelayElement> c) {
        if (c == null) {
     ...
     */
    @Override
    public int drainTo(final Collection<? super MailAccessDelayElement> c, final int maxElements) {
        if (c == null) {
            throw new NullPointerException();
        }
        if (c == this) {
    ...
    /**
     * Always returns <tt>Integer.MAX_VALUE</tt> because a <tt>MailAccessWatcherDelayQueue</tt> is not capacity constrained.
     *
     ...
     */
    private class Itr implements Iterator<MailAccessDelayElement> {
        ...
        public MailAccessDelayElement next() {
            ...
            lastRet = cursor;
            return (MailAccessDelayElement) array[cursor++];
        }
        ...
        public void remove() {
            ...
            lock.lock();
            try {
                for (final Iterator<MailAccessDelayElement> it = q.iterator(); it.hasNext();) {
                    if (it.next() == x) {
                        it.remove();
                        return;
                    }
