 ...
 * @since v7.10.2
 */
public class Pop3SessionCacheInvalidator implements CacheListener, ServiceTrackerCustomizer<CacheEventService, CacheEventService> {

    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(Pop3SessionCacheInvalidator.class);

    private static final String REGION = RdbMailAccountStorage.REGION_POP3_SESSION_CACHE;
                    ...
                    if (cacheKey instanceof CacheKey) {
                        ...
                        if (contextId > 0) {
                            ...
                            if (pairs.add(new Pair<Integer, Integer>(I(contextId), I(userId)))) {
                                RdbMailAccountStorage.dropPOP3StorageFolders(userId, contextId, false);
                            }
                        }
                    }
                }
