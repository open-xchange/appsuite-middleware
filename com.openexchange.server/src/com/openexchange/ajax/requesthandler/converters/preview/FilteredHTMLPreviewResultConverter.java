            ...
            for (String content : previewDocument.getContent()) {
                if (!DisplayMode.RAW.equals(mode)) {
                    ...
                    if (DisplayMode.MODIFYABLE.isIncluded(mode) && usm.isDisplayHtmlInlineContent()) {
                        ...
                        final String cssPrefix = "ox-" + HtmlProcessingTmp.getHash(Long.toString(System.currentTimeMillis()));
                        final boolean externalImagesAllowed = usm.isAllowHTMLImages();
                        content = htmlService.checkBaseTag(content, externalImagesAllowed);
                        if (HtmlProcessingTmp.useSanitize()) {
                         ...
                         */
                        {
                            ...
                            if (mail != null) {
                                ...
                                content = HtmlProcessingTmp.filterInlineImages(content, session, mailPath);
                            }
                        }
                        /*
                         * Replace CSS classes
                         */
                        content = HtmlProcessingTmp.saneCss(content, htmlService, cssPrefix);
                        if (asDiv) {
