...
public class UpdateETagHeaderAction implements IFileResponseRendererAction {
    ...
    public void call(IDataWrapper data) throws Exception {
        if (data.getDelivery() == null || !data.getDelivery().equalsIgnoreCase(IDataWrapper.DOWNLOAD)) {
            /*
             * ETag present and caching?
             */
            final String eTag = data.getResult().getHeader("ETag");
            if (null != eTag) {
                final long expires = data.getResult().getExpires();
                Tools.setETag(eTag, expires > 0 ? expires : -1L, data.getResponse());
            } else {
                final long expires = data.getResult().getExpires();
                if (expires > 0) {
                    Tools.setExpires(expires, data.getResponse());
                }
            }
            /*
             * Last-Modified
             */
            final String sLastModified = data.getResult().getHeader("Last-Modified");
            if (null != sLastModified) {
                data.getResponse().setHeader("Last-Modified", sLastModified);
            }
        }
