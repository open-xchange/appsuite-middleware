...
public class ManagementTracker implements ServiceTrackerCustomizer<ManagementService, ManagementService> {
     ...
     */
    public ManagementTracker(BundleContext context) {
        super();
    ...
    public ManagementService addingService(ServiceReference<ManagementService> reference) {
        ...
        try {
            ObjectName objectName = Managements.getObjectName(FileChecksumsMBean.class.getName(), FileChecksumsMBean.DOMAIN);
            managementService.registerMBean(objectName, new FileChecksumsMBeanImpl());
            logger.info("Registered MBean {}", FileChecksumsMBean.class.getName());
            return managementService;
        } catch (Exception e) {
            logger.warn("Could not register MBean {}", FileChecksumsMBean.class.getName(), e);
        }
    ...
    public void removedService(ServiceReference<ManagementService> reference, ManagementService managementService) {
        ...
        if (null != managementService) {
            try {
                managementService.unregisterMBean(Managements.getObjectName(FileChecksumsMBean.class.getName(), FileChecksumsMBean.DOMAIN));
                logger.info("Unregistered MBean {}", FileChecksumsMBean.class.getName());
            } catch (Exception e) {
                logger.warn("Could not un-register MBean {}", FileChecksumsMBean.class.getName(), e);
            }
            context.ungetService(reference);
        }
    }
