 ...
 */
public class CreateObjectPermissionAction extends AbstractDBAction {

    private static final String INSERT_OBJECT_PERMISSION =
        "INSERT INTO object_permission (cid, permission_id, module, folder_id, object_id, created_by, shared_by, bits, group_flag) " +
        "VALUES (?,?,?,?,?,?,?,?,?);";

    private static final String DELETE_OBJECT_PERMISSION =
        "DELETE FROM object_permission " +
        "WHERE cid=? AND object_id=?;";

    private final DocumentMetadata document;
    private final int sharedBy;
     ...
     * @param sharedBy The identifier of the sharing user, i.e. the parent sfolder's owner
     */
    public CreateObjectPermissionAction(DBProvider provider, Context context, DocumentMetadata document, int sharedBy) {
        super();
            ...
            for (final ObjectPermission objectPermission : objectPermissions) {
                updates.add(new Update(INSERT_OBJECT_PERMISSION) {
                    ...
                    public void fillStatement() throws SQLException {
                        ...
                        stmt.setInt(6, document.getCreatedBy());
                        stmt.setInt(7, sharedBy);
                        stmt.setInt(8, objectPermission.getPermissions());
                        stmt.setBoolean(9, objectPermission.isGroup());
                    }
                });
            }
        }
