...
public class MailObject {
    ...
    public final void send() throws OXException {
        try {
            ...
            */
            ...
            /*
             * Set reference headers for thread-view alias conversations
             */
            if (Strings.isNotEmpty(messageId)) {
                msg.setHeader(MessageHeaders.HDR_MESSAGE_ID, messageId);
            }
            if (Strings.isNotEmpty(inReplyTo)) {
                msg.setHeader(MessageHeaders.HDR_IN_REPLY_TO, inReplyTo);
            }
            if (Strings.isNotEmpty(reference)) {
                msg.setHeader(MessageHeaders.HDR_REFERENCES, reference);
            }
            /*
             * Set organization
             */
    ...
    public void setMessageId(String uid) {
        if (Strings.isNotEmpty(uid)) {
            String hostname = null == getHostName() ? getStaticHostName() : getHostName();
            this.messageId = "Appointment." + String.valueOf(session.getContextId()) + "." + uid + "." + String.valueOf(System.currentTimeMillis()) + "@" + hostname;
        }
    }

    public void setInReplyTo(String uid) {
        if (Strings.isNotEmpty(uid)) {
            String hostname = null == getHostName() ? getStaticHostName() : getHostName();
            this.inReplyTo = "Appointment." + String.valueOf(session.getContextId()) + "." + uid + "@" + hostname;
        }
    }

    public void setReference(String uid) {
        if (Strings.isNotEmpty(uid)) {
            String hostname = null == getHostName() ? getStaticHostName() : getHostName();
            this.reference = "Appointment." + String.valueOf(session.getContextId()) + "." + uid + "@" + hostname;
        }
    }
}
