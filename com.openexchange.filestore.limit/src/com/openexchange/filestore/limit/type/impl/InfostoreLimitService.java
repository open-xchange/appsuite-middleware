...
import com.openexchange.filestore.limit.osgi.Services;
import com.openexchange.groupware.infostore.InfostoreConfig;
import com.openexchange.groupware.upload.impl.UploadUtility;
import com.openexchange.session.Session;

/**
 * {@link InfostoreLimitService}
 *
 * @author <a href="mailto:martin.schneider@open-xchange.com">Martin Schneider</a>
 * @since v7.10.1
 */
public class InfostoreLimitService extends AbstractCombinedTypeLimitService {
    ...
    private List<OXException> getExceededLimits(String folderId, List<File> files, IDBasedFolderAccess folderAccess) throws OXException {
        ...
        if (fileQuota.getLimit() > 0 && fileQuota.getUsage() + files.size() > fileQuota.getLimit()) {
            exceededLimits.add(LimitExceptionCodes.TOO_MANY_FILES.create(UploadUtility.getSize(fileQuota.getLimit(), 2, false, true)));
        }

        Quota storageQuota = folderAccess.getStorageQuota(folderId);
        long fileTotalSize = files.stream().collect(Collectors.summingLong(File::getFileSize));
        if (storageQuota.getLimit() > 0 && storageQuota.getUsage() + fileTotalSize > storageQuota.getLimit()) {
            exceededLimits.add(LimitExceptionCodes.STORAGE_QUOTA_EXCEEDED.create(UploadUtility.getSize(fileTotalSize, 2, false, true), UploadUtility.getSize(storageQuota.getLimit() - storageQuota.getUsage(), 2, false, true)));
        }
        checkMaxUploadSizePerFile(files, exceededLimits);
