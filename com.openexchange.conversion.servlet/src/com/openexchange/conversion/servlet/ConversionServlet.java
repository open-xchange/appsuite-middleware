...
public final class ConversionServlet extends PermissionServlet {
	...
	private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory
			.getLog(ConversionServlet.class);
	...
	protected void doPut(final HttpServletRequest req, final HttpServletResponse resp) throws ServletException,
		...
		try {
			...
			LOG.error("doPut", e);
			final Response response = new Response();
			response.setException(new ConversionServletException(ConversionServletException.Code.JSON_ERROR, e, e
					.getLocalizedMessage()));
			final PrintWriter writer = resp.getWriter();
			try {
				ResponseWriter.write(response, writer);
	...
	private Response doAction(final HttpServletRequest req) throws ConversionServletException, JSONException,
		...
		throw new ConversionServletException(ConversionServletException.Code.UNSUPPORTED_PARAM, PARAMETER_ACTION,
				actionStr);
	}

	private Response actionConvert(final JSONObject jsonBody, final Session session) throws ConversionServletException,
		...
		try {
			conversionService = ConversionServletServiceRegistry.getServiceRegistry().getService(
					ConversionService.class, true);
		} catch (final ServiceException e) {
			throw new ConversionServletException(e);
		}
		final Object result = conversionService.convert(jsonDataSource.getString(JSON_IDENTIFIER),
				parseDataSourceOrHandlerArguments(jsonDataSource), jsonDataHandler.getString(JSON_IDENTIFIER),
				parseDataSourceOrHandlerArguments(jsonDataHandler), session);
		/*
		 * Compose response
		 */
		final Response response = new Response();
	...
	private static String checkStringParam(final HttpServletRequest req, final String paramName)
			throws ConversionServletException {
