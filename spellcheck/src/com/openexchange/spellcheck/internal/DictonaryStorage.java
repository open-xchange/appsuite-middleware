...
public final class DictonaryStorage {
     ...
     */
    private static final Pattern PAT_FILENAME_PHON = Pattern.compile("([a-z]{2})_([A-Z]{2})\\.phon", Pattern.CASE_INSENSITIVE);

    /**
     * The filename pattern for word lists
     */
    private static final Pattern PAT_FILENAME_WL = Pattern.compile("([a-z]{2})_([A-Z]{2})([^.])*\\.wl", Pattern.CASE_INSENSITIVE);
     ...
     */
    public static SpellDictionary getDictionary(final String localeStr) throws OXException {
    ...
    public static void putDictionary(final String localeStr, final SpellDictionary spellDictionary) {
        ...
        if (!m.matches()) {
            throw new IllegalArgumentException(
                "Illegal locale string: " + localeStr + ". Excpected \"<language-in-lower-case>_<country-in-upper-case>\"");
        }
    ...
    public static void loadDictionaries() throws OXException {
        ...
        {
            final ConfigurationService c = SpellCheckServiceRegistry.getServiceRegistry().getService(ConfigurationService.class);
            if (null != c) {
                final String dirPath = c.getProperty("com.openexchange.spellcheck.dir");
                if (dirPath == null) {
                    throw SpellCheckExceptionCode.MISSING_PROPERTY.create("com.openexchange.spellcheck.dir");
                }
                spellCheckDir = new File(dirPath);
            } else {
                throw ServiceExceptionCode.SERVICE_UNAVAILABLE.create(SpellCheckService.class.getName());
            }
        }
        if (!spellCheckDir.exists() || !spellCheckDir.isDirectory()) {
            throw SpellCheckExceptionCode.MISSING_DIR.create(spellCheckDir.getPath());
        }
        ...
        if ((subdirs == null) || (subdirs.length == 0)) {
            throw SpellCheckExceptionCode.NO_LOCALE_FOUND.create();
        }
    ...
    private static SpellDictionary getDicFromDirectory(final File dir) throws OXException {
        /*
        ...
        final File phonFile;
        {
            ...
            if (files.length == 1) {
                ...
                throw SpellCheckExceptionCode.ONLY_ONE_PHON_FILE.create();
            }
        }
        /*
        ...
        if ((files != null) && (files.length > 0)) {
            ...
            try {
                ...
                throw SpellCheckExceptionCode.IO_ERROR.create(e, e.getMessage());
            }
            return spellDictionary;
        }
        throw SpellCheckExceptionCode.AT_LEAST_ONE_WL_FILE.create();
    }

    private static SpellDictionaryHashMap generateSpellDictionary(final File wordList, final File phonetic) throws IOException {
        Reader wordListReader = null;
        Reader phonetReader = null;
        try {
            wordListReader = new InputStreamReader(new FileInputStream(wordList), detectCharset(new FileInputStream(wordList)));
            if (null == phonetic) {
                return new SpellDictionaryHashMap(wordListReader);
            }
            phonetReader = new InputStreamReader(new FileInputStream(phonetic), detectCharset(new FileInputStream(phonetic)));
