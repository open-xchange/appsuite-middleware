...
public final class HCard implements Serializable {	
	...
	public static final class Name implements Serializable {
		...
		public @Override boolean equals(Object obj) {
			...
			if ( !familyName.equals(other.familyName) ) return false;
			if ( !givenName.equals(other.givenName) ) return false;
			if ( !additionalNames.equals(other.additionalNames) ) return false;
			if ( !honorificPrefixes.equals(other.honorificPrefixes) ) return false;
			if ( !honorificSuffixes.equals(other.honorificSuffixes) ) return false;
			return true;
		}
		
		public @Override String toString() {
			StringBuilder sb = new StringBuilder();
			for ( String honorific : honorificPrefixes ) sb.append(honorific + " ");
			if ( givenName.length() > 0 ) sb.append(givenName + " ");
			for ( String name : additionalNames ) sb.append(name + " ");
			if ( familyName.length() > 0 ) sb.append(familyName + " ");
			for ( String honorific : honorificSuffixes ) sb.append(honorific + " ");
		...
		public String toHTML() {
			...
			for ( String honorific : honorificPrefixes ) sb.append("<span class=\"honorific-prefix\">").append(s2h(honorific)).append("</span> ");
			if ( givenName.length() > 0 ) sb.append("<span class=\"given-name\">").append(s2h(givenName)).append("</span> ");
			for ( String additional : additionalNames ) sb.append("<span class=\"additional-name\">").append(s2h(additional)).append("</span> ");
			if ( familyName.length() > 0 ) sb.append("<span class=\"family-name\">").append(s2h(familyName)).append("</span> ");
			for ( String honorific : honorificSuffixes ) sb.append("<span class=\"honorific-suffix\">").append(s2h(honorific)).append("</span> ");
			sb.append("</div>");
	...
	public static final class Address implements Serializable {
		...
		public @Override boolean equals(Object obj) {
			...
			if ( !extendedAddress.equals(other.extendedAddress) ) return false;
			if ( !locality.equals(other.locality) ) return false;
			if ( !postOfficeBox.equals(other.postOfficeBox) ) return false;
			if ( !postalCode.equals(other.postalCode) ) return false;
			if ( !region.equals(other.region) ) return false;
			if ( !streetAddress.equals(other.streetAddress) ) return false;
			return true;
		}
		
		public @Override String toString() {
			StringBuilder sb = new StringBuilder();
			if ( streetAddress.length() > 0 ) sb.append(streetAddress + "\n");
			if ( extendedAddress.length() > 0 ) sb.append(extendedAddress + "\n");
			boolean space = false;
			...
			if ( space ) sb.append('\n');
			if ( countryName.length() > 0 ) sb.append(countryName + "\n");
		...
		public String toHTML() {
			...
			sb.append(":<br />");
			if ( streetAddress.length() > 0 ) sb.append("<span class=\"street-address\">").append(s2h(streetAddress)).append("</span><br />");
			if ( extendedAddress.length() > 0 ) sb.append("<span class=\"extended-address\">").append(s2h(extendedAddress)).append("</span><br />");
	...
	public static final class Organization implements Serializable {
		...
		public @Override String toString() {
			if ( unit.equals("") ) return name;
			else return String.format("%s (%s)", name, unit);
		}
		...
		public String toHTML() {
			...
			if ( unit.length() > 0 ) sb.append(" [<span class=\"organization-unit\">").append(s2h(unit)).append("</span>]");
			sb.append("</div>");
			return sb.toString();
		}
	}
	...
	public static final class XFNURL implements Serializable {
		...
		public XFNURL(URI url, List<XFNRelationship> rels) {
			...
			this.url = url;
			if ( rels == null || rels.isEmpty() ) this.rels = Collections.emptyList();
			else this.rels = Collections.unmodifiableList(new ArrayList<XFNRelationship>(rels));
		}
		...
		@Override public boolean equals(Object obj) {
			if ( this == obj ) return true;
			if ( ! (obj instanceof XFNURL) ) return false;
			final XFNURL other = (XFNURL)obj;
			if ( !url.equals(other.url) || rels.size() != other.rels.size() ) return false;
		...
		static boolean equalRels(List<XFNRelationship> one, List<XFNRelationship> two) {
			...
			for ( XFNRelationship rel : two ) if ( !list.remove(rel) ) return false;
			return true;
		}
		
		@Override public String toString() {
			if ( rels.isEmpty() ) return url.toString();
			else return String.format("%s  rel: %s", url.toString(), xfnRelsToString());
		}
		
		private String xfnRelsToString() {
			StringBuilder out = new StringBuilder();
			for ( XFNRelationship rel : rels ) out.append(rel).append(" ");
			out.setLength(out.length() -1);
			return out.toString();
		}
		
		public String toJSON() {
			if ( rels.isEmpty() ) return String.format("{\"url\":%s}", s2j(url.toString()));
			else return String.format("{\"url\":%s,\"rels\":%s", s2j(url.toString()), sl2j(rels));
		}
		...
		public String toHTML() {
			...
			if ( rels.isEmpty() )
				...
				for ( XFNRelationship rel : rels ) sb.append(rel).append(" ");
				sb.setCharAt(sb.length() -1, ':');
				sb.append(" <a class=\"url\" rel=\"");
				for ( XFNRelationship rel : rels ) sb.append(s2a(rel.toString())).append(" ");
				sb.setCharAt(sb.length() -1, '"');
			}
	...
	HCard(String fn, Name n, List<String> nicknames, List<URI> photos, Long bday, List<Address> adrs, List<String> labels, List<Tel> tels,
		...
		if ( fn == null ) throw new IllegalArgumentException("fn must be specified.");
		this.fn = fn;
		...
		for ( XFNURL xfn : xfnUrls ) urls.add(xfn.url);
	...
	public @Override String toString() {
		...
		if ( !n.isEmpty() ) sb.append("  Name: ").append(n).append('\n');
		if ( nicknames.size() > 0 ) sb.append("  Nickname: ").append(printCommaList(nicknames)).append('\n');
		if ( bday != null ) sb.append("  Birth day: ").append(printDate(bday.longValue())).append('\n');
		if ( tels.size() > 0 ) sb.append("  Tel Nr: ").append(printCommaList(tels)).append('\n');
		if ( emails.size() > 0 ) sb.append("  Email: ").append(printCommaList(emails)).append('\n');
		if ( geo != null ) sb.append("  Geolocation: ").append(geo).append('\n');
		if ( tz != null ) sb.append("  Timezone: ").append(tz).append('\n');
		if ( adrs.size() > 0 ) sb.append("  Address:\n").append(printBlockList(adrs));
		if ( labels.size() > 0 ) sb.append("  Label:\n").append(printBlockList(labels));
		if ( photos.size() > 0 ) sb.append("  Photo: ").append(printLineList(photos)).append('\n');
		if ( sounds.size() > 0 ) sb.append("  Sounds: ").append(printLineList(sounds)).append('\n');
		if ( mailers.size() > 0 ) sb.append(" Mailer:").append(printLineList(mailers)).append('\n');
		if ( titles.size() > 0 ) sb.append("  Title:").append(printCommaList(titles)).append('\n');
		if ( orgs.size() > 0 ) sb.append("  Organization: ").append(printLineList(orgs)).append('\n');
		if ( roles.size() > 0 ) sb.append("  Roles: ").append(printCommaList(roles)).append('\n');
		if ( logos.size() > 0 ) sb.append("  Logo: ").append(printLineList(logos)).append('\n');
		if ( agents.size() > 0 ) sb.append("  Agent: ").append(printLineList(agents)).append('\n');
		if ( categories.size() > 0 ) sb.append("  Category: ").append(printCommaList(categories)).append('\n');
		if ( xfnUrls.size() > 0 ) sb.append("  Url: ").append(printLineList(urls)).append('\n');
		if ( keys.size() > 0 ) sb.append("  Key: ").append(printBlockList(keys));
		if ( notes.size() > 0 ) sb.append("  Note: ").append(printBlockList(notes));
		if ( rev != null ) sb.append("  Rev: ").append(printDate(rev.longValue())).append('\n');
		if ( sortString != null ) sb.append("  SortString: ").append(sortString).append('\n');
		if ( uid != null ) sb.append("  UID: " ).append(uid).append('\n');
		if ( accessClass != null ) sb.append(" Class: ").append(accessClass).append('\n');
	...
	public String toHTML() {
		...
		if ( !orgCard ) {
			if ( n.isEmpty() ) sb.append("<span class=\"n\"></span>");
			else sb.append("Proper Name: ").append(n.toHTML());
		}
		
		sb.append(printStrings2HTMLLine(nicknames, "nickname", "Nickname", "Nicknames"));
		
		if ( bday != null ) sb.append("Birth Date: <span class=\"bday\">").append(date2s(bday)).append("</span><br />");
		
		for ( Tel tel : tels ) sb.append(tel.toHTML());
		for ( Email email : emails ) sb.append(email.toHTML());
		if ( geo != null ) sb.append(geo.toHTML());
		if ( tz != null ) sb.append("Timezone: <span class=\"tz\">").append(tz2s(tz)).append("</span><br />");
		for ( Address adr : adrs ) sb.append(adr.toHTML());
		for ( String label : labels ) sb.append("Shipping Label: <div class=\"label\">").append(label).append("</div>");
		for ( URI photo : photos ) sb.append("<img class=\"photo\" src=\"").append(photo.toASCIIString()).append("\" /><br />");
		for ( XFNURL url : xfnUrls ) sb.append(url.toHTML());
		
		if ( !orgCard ) for ( Organization org : orgs ) sb.append(org.toHTML());
		for ( URI logo : logos ) sb.append("<img class=\"logo\" src=\"").append(logo.toASCIIString()).append("\" /><br />");
		...
		if ( keys.size() > 0 ) {
			...
			for ( String key : keys ) sb.append("<div class=\"key\">").append(s2h(key)).append("</div>");
		}
		
		if ( notes.size() > 0 ) {
			sb.append("Note").append(notes.size() == 1 ? "" : "s").append(" :<br />");
			for ( String note : notes ) sb.append("<div class=\"note\">").append(s2h(note)).append("</div>");
		}
		
		if ( rev != null ) sb.append("Last Updated: <span class=\"rev\">").append(date2s(rev)).append("</span><br />");
		if ( sortString != null ) sb.append("Sort String: <span class=\"sort-string\">").append(s2h(sortString)).append("</span><br />");
		if ( accessClass != null ) sb.append("Access class: <span class=\"class\">").append(s2h(accessClass)).append("</span><br />");
		if ( uid != null ) sb.append("UID: <span class=\"uid\">").append(s2h(uid)).append("</span><br />");
	...
	private static String printURIs2HTMLBlock(List<URI> items, String classValue, String singular, String plural) {
		...
		default:
			...
			for ( URI item : items ) sb.append("<li>").append(uri2ahref(item, classValue)).append("</li>"); 
	...
	public String toJSON() {
		...
		if ( nicknames.size() > 0 ) appendStringValue(sb, "nicknames", sl2j(nicknames));
		if ( bday != null ) appendStringValue(sb, "bday", "" + bday);
		...
		if ( geo != null ) appendStringValue(sb, "geo", geo.toJSON());
		if ( tz != null ) appendStringValue(sb, "tz", tz2s(tz));
		...
		if ( labels.size() > 0 ) appendStringValue(sb, "labels", sl2j(nicknames));
		if ( photos.size() > 0 ) appendStringValue(sb, "photos", sl2j(photos));
		if ( sounds.size() > 0 ) appendStringValue(sb, "sounds", sl2j(sounds));
		if ( mailers.size() > 0 ) appendStringValue(sb, "mailers", sl2j(mailers));
		if ( titles.size() > 0 ) appendStringValue(sb, "titles", sl2j(titles));
		...
		if ( roles.size() > 0 ) appendStringValue(sb, "roles", sl2j(roles));
		if ( logos.size() > 0 ) appendStringValue(sb, "logos", sl2j(logos));
		if ( agents.size() > 0 ) appendStringValue(sb, "agents", sl2j(agents));
		if ( categories.size() > 0 ) appendStringValue(sb, "categories", sl2j(categories));
		if ( urls.size() > 0 ) appendStringValue(sb, "urls", sl2j(urls));
		if ( xfnUrls.size() > 0 ) {
			sb.append("\"xfnUrls\":[");
			boolean first = true;
		...
		if ( keys.size() > 0 ) appendStringValue(sb, "keys", sl2j(keys));
		if ( notes.size() > 0 ) appendStringValue(sb, "notes", sl2j(notes));
		if ( rev != null ) appendStringValue(sb, "rev", "" + rev);
		if ( sortString != null ) appendStringValue(sb, "sort-string", s2j(sortString));
		if ( uid != null ) appendStringValue(sb, "uid", s2j(uid));
		if ( accessClass != null ) appendStringValue(sb, "class", s2j(accessClass));
	...
	public @Override boolean equals(Object obj) {
		if ( this == obj ) return true;
		if ( !(obj instanceof HCard) ) return false;
		final HCard other = (HCard)obj;
		
		if ( !fn.equals(other.fn) ) return false;
		if ( !adrs.equals(other.adrs) ) return false;
		if ( !categories.equals(other.categories) ) return false;
		if ( !emails.equals(other.emails) ) return false;
		if ( !keys.equals(other.keys) ) return false;
		if ( !labels.equals(other.labels) ) return false;
		if ( !logos.equals(other.logos) ) return false;
		if ( !mailers.equals(other.mailers) ) return false;
		if ( !nicknames.equals(other.nicknames) ) return false;
		if ( !notes.equals(other.notes) ) return false;
		if ( !orgs.equals(other.orgs) ) return false;
		if ( !photos.equals(other.photos) ) return false;
		if ( !roles.equals(other.roles) ) return false;
		if ( !tels.equals(other.tels) ) return false;
		if ( !titles.equals(other.titles) ) return false;
		if ( !xfnUrls.equals(other.xfnUrls) ) return false;
		
		if ( bday == null ) {
			if ( other.bday != null ) return false;
		} else if ( !bday.equals(other.bday) ) return false;
		...
		if ( rev == null ) {
			if ( other.rev != null ) return false;
		} else if ( !rev.equals(other.rev) ) return false;
	...
	public List<XFNRelationship> getXFNRelationships() {
		if ( xfnUrls.isEmpty() ) return Collections.emptyList();
		List<XFNRelationship> rels = new ArrayList<XFNRelationship>();
		for ( XFNURL xfn : xfnUrls ) {
			if ( xfn.rels.isEmpty() ) continue;
			if ( rels.isEmpty() ) rels.addAll(xfn.rels);
			else if ( !XFNURL.equalRels(rels, xfn.rels) ) throw new IllegalStateException("Conflicting XFN relations - check each url individually.");
		}
	...
	public String getDefaultTelValue() {
		...
		if ( value == null ) value = tels.get(0).value;
	...
	public String getTelValue(String type) {
		...
		for ( Tel tel : tels ) if ( tel.types.contains(type) ) return tel.value;
		return null;
	}
	
	public Address getDefaultAddress() {
		if ( adrs.isEmpty() ) return null;
		
		Address address = getAddress(Address.POSTAL);
		if ( address == null ) address = adrs.get(0);
	...
	public Address getAddress(String type) {
		...
		type = type.toLowerCase();
		for ( Address adr : adrs ) if ( adr.types.contains(type) ) return adr;
		return null;
	}
	...
	public String getDefaultEmailValue() {
		...
		if ( email == null ) email = emails.get(0).value;
	...
	public String getEmailValue(String type) {
		...
		type = type.toLowerCase();
		for ( Email email : emails ) if ( email.types.contains(type) ) return email.value;
		return null;
	}
}
