...
public final class UnifiedInboxFolderStorage extends MailFolderStorage implements IMailFolderStorageEnhanced2 {
    ...
    @Override
    public long getSize(final String fullName) throws OXException {
        if (DEFAULT_FOLDER_ID.equals(fullName)) {
            return 0;
        }
        ...
        if (UnifiedInboxAccess.KNOWN_FOLDERS.contains(fullName)) {
            ...
            final int length = accounts.size();
            final Executor executor = ThreadPools.getThreadPool().getExecutor();
            final TrackingCompletionService<Long> completionService = new UnifiedInboxCompletionService<Long>(executor);
            for (final MailAccount mailAccount : accounts) {
                completionService.submit(new LoggingCallable<Long>(session) {
                    ...
                    public Long call() throws Exception {
                        ...
                        try {
                            ...
                            if (folderStorage instanceof IMailFolderStorageEnhanced2) {
                                return Long.valueOf(((IMailFolderStorageEnhanced2) folderStorage).getSize(fn));
                            }
            ...
            try {
                long size = 0;
                for (int i = 0; i < length; i++) {
                    long sz = completionService.take().get().longValue();
                    if (sz < 0) {
                        return -1L;
                    }
                    size = size + sz;
                }
                LOG.debug("Retrieving size for folder \"{}\" took {}msec.", fullName, completionService.getDuration());
                // Return
                return size;
            } catch (final InterruptedException e) {
                Thread.currentThread().interrupt();
        ...
        try {
            ...
            IMailFolderStorage folderStorage = mailAccess.getFolderStorage();
            if (folderStorage instanceof IMailFolderStorageEnhanced2) {
                return ((IMailFolderStorageEnhanced2) folderStorage).getSize(fa.getFullname());
            }
            return -1L;
        } finally {
            closeSafe(mailAccess);
