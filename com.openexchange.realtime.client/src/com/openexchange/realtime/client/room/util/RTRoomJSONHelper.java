...
public class RTRoomJSONHelper {
     ...
     * 
     * @param to - String with the address to send the message to
     * @param payload - {@link JSONValue} with the message (all required payload) to say.
     * @return {@link JSONValue} to say a message into a room
     * @throws JSONException
     */
    public static JSONValue createSayMessage(String to, JSONValue payload) throws JSONException {
        JSONObject objectToSend = new JSONObject();
        objectToSend.put("element", "message");
        objectToSend.put("to", to);
        objectToSend.put("payloads", payload);
