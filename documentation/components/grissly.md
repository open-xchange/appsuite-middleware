...
<IfModule mod_proxy_http.c>
   ...
   <Proxy balancer://eas_oxcluster>
      ...
      Allow from all
      # multiple server setups need to have the hostname inserted instead localhost
      BalancerMember http://localhost:8009 timeout=1900 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0
      #BalancerMember http://10.242.2.6:8009 timeout=1900 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0
      # Enable and maybe add additional hosts running OX here
...
<IfModule mod_proxy_http.c>
   ...
   <Proxy balancer://oxcluster_tls>
       ...
       Allow from all
       # multiple server setups need to have the hostname inserted instead localhost
       BalancerMember https://localhost_tls:8010 timeout=100 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0_TLS
       #BalancerMember http://10.242.2.6:8009 timeout=100 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0
       # Enable and maybe add additional hosts running OX here
       # BalancerMember http://oxhost2:8009 timeout=100 smax=0 ttl=60 retry=60 loadfactor=50 route=OX2
      ProxySet stickysession=JSESSIONID|jsessionid scolonpathdelim=On
   ...
   #
   # Alternatively select one or more hosts of your cluster to be restricted to handle only eas/usm requests
   <Proxy balancer://eas_oxcluster_tls>
      Order deny,allow
      Allow from all
      # multiple server setups need to have the hostname inserted instead localhost
      BalancerMember https://localhost_tls:8010 timeout=1900 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0_TLS
      #BalancerMember http://10.242.2.6:8009 timeout=1900 smax=0 ttl=60 retry=60 loadfactor=50 route=OX0
      # Enable and maybe add additional hosts running OX here
      # BalancerMember http://oxhost2:8009 timeout=1800  smax=0 ttl=60 retry=60 loadfactor=50 route=OX2
      ProxySet stickysession=JSESSIONID|jsessionid scolonpathdelim=On
...
<VirtualHost *:443>
   ...
   ProxyPass /usm-json balancer://eas_oxcluster_tls/usm-json
   ProxyPass /Microsoft-Server-ActiveSync balancer://eas_oxcluster_tls/Microsoft-Server-ActiveSync
