...
public class AgglomerativeClustering {
	...
	public static void main( final String args[] ) throws IOException {
		...
		final Getopt g = new Getopt( "AgglomerativeClustering", args, "h", longopts );
		g.setOpterr( true );

		if ( args.length < 1 ) {
			LOG.error( "The path where the files to be clustered are must be specified on the command line." );
			LOG.error( "See help for more details on usage." );
			return ;
		}
		...
		final LanguageModel[] lm = new LanguageModel[n];
		for ( int i = 0; i < n; i++ ) {
		...
		final int useTopmostNgrams = lc.getUseTopmostNgrams();
		...
		final IntArrayList[] clusters = new IntArrayList[n];
		...
		while ( numClusters > 1 ) {
			...
			for ( int i = 0; i < n; i++ ) {
				for ( int j = i; j < n; j++ ) {
                    if ( distance[i][j] > 0 && lm[i] != null && lm[j] != null && distance[i][j] < minDistance ) {
