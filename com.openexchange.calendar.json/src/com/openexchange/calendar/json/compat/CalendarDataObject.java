...
/**
 * moved from com.openexchange.groupware.calendar.CalendarDataObject
 *
 * CalendarDataObject
...
public class CalendarDataObject extends Appointment {
    ...
    private CalendarCollection getCalendarCollectionService() {
    ...
    public final void setUntil(final Date until) {
        if (until != null) {
            final long mod = until.getTime() % getCalendarCollectionService().MILLI_DAY;
            if (mod != 0) {
                if (getCalendarCollectionService().exceedsHourOfDay(until.getTime(), getTimezoneFallbackUTC())) {
                    until.setTime((((until.getTime() - mod) + getCalendarCollectionService().MILLI_DAY)));
                } else {
