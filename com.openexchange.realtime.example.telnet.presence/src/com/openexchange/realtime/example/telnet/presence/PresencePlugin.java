...
public class PresencePlugin implements TelnetChatPlugin {
	...
	public void handleIncoming(TelnetChatMessage message) throws OXException {
		...
		presenceService.changeState(message.getFrom(), getState(message), message.getPayload(), message.getSession());
	}

	private PresenceState getState(TelnetChatMessage message) throws OXException {
		message.requireHeader("state");
	
		String state = message.getHeader("state");
		if (state.equals("aw")) {
			return PresenceState.AWAY;
		} else if (state.equals("dnd")) {
			return PresenceState.DO_NOT_DISTURB;
		} else if (state.equals("on")) {
			return PresenceState.ONLINE;
		} else if (state.equals("off")) {
			return PresenceState.OFFLINE;
		} else if (state.equals("welcome")) {
			return PresenceState.CHAT_ME_UP;
		}
		return null;
	}
	
	private String getState(PresenceState state) {
		switch (state) {
		case AWAY: return "aw";
		case DO_NOT_DISTURB: return "dnd";
		case ONLINE: return "on";
		case OFFLINE: return "off";
		case CHAT_ME_UP: return "welcome";
		}
		return null;
	}
	...
	public boolean canHandleOutgoing(String namespace) throws OXException {
		return namespace.equals("presence");
	}
	...
	public void handleOutgoing(Stanza stanza, ID to, ServerSession session,
		...
		msg.setHeader("state", getState(status.getState()));
