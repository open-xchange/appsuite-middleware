...
public final class CacheFolderStorage implements FolderStorage {
	...
	public void onCacheAvailable() throws OXException {
		cacheService = CacheServiceRegistry.getServiceRegistry().getService(
				CacheService.class, true);
	...
	public void checkConsistency(final String treeId,
		...
		for (final FolderStorage folderStorage : registry
			...
			final boolean started = folderStorage.startTransaction(
	...
	public void restore(final String treeId, final String folderId,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
	...
	public Folder prepareFolder(final String treeId, final Folder folder,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			final Folder preparedFolder = storage.prepareFolder(treeId, folder,
					storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
			if (preparedFolder.isCacheable()
					&& preparedFolder.isGlobalID() != folder.isGlobalID()) {
	...
	private PathPerformer newPathPerformer(
			final StorageParameters storageParameters) throws OXException {
		final Session session = storageParameters.getSession();
		if (null == session) {
			return new PathPerformer(storageParameters.getUser(),
					storageParameters.getContext(), null, registry);
		}
		try {
			return new PathPerformer(new ServerSessionAdapter(session), null,
					registry);
		} catch (final OXException e) {
			throw new OXException(e);
		}
	...
	public void createFolder(final Folder folder,
		...
		if (null == session) {
			folderId = new CreatePerformer(storageParameters.getUser(),
					storageParameters.getContext(), registry).doCreate(folder);
		} else {
			try {
				folderId = new CreatePerformer(
						new ServerSessionAdapter(session), registry)
						.doCreate(folder);
			} catch (final OXException e) {
				throw new OXException(e);
			}
		...
		final Folder createdFolder = loadFolder(treeId, folderId,
				StorageType.WORKING, true, storageParameters);
		 ...
		 * Refresh parent
		 */
		removeSingleFromCache(folder.getParentID(), treeId,
				storageParameters.getUserId(), storageParameters.getSession());
		final Folder parentFolder = loadFolder(treeId, folder.getParentID(),
				StorageType.WORKING, true, storageParameters);
	...
	private void putFolder(final Folder folder, final String treeId,
			final StorageParameters storageParameters) throws OXException {
		/*
		 * Put to cache
		 */
		if (folder.isGlobalID()) {
			globalCache.putInGroup(newCacheKey(folder.getID(), treeId),
					String.valueOf(storageParameters.getContextId()), folder);
		} else {
	 ...
	 *            The tree identifier
	 * @param singleOnly
	 *            <code>true</code> if only specified folder should be removed;
	 *            otherwise <code>false</code> for complete folder's path to
	 *            root folder
	 * @param session
	 *            The session providing user information
	 * @throws OXException
	 *             If removal fails
	 */
	public void removeFromCache(final String id, final String treeId,
			final boolean singleOnly, final Session session) throws OXException {
		if (singleOnly) {
			removeSingleFromCache(id, treeId, session.getUserId(), session);
		} else {
			removeFromCache(id, treeId, session, new PathPerformer(
					new ServerSessionAdapter(session), null, registry));
		}
	}

	private void removeFromCache(final String id, final String treeId,
			final Session session, final PathPerformer pathPerformer)
			throws OXException {
		if (null == id) {
			return;
		}
		{
			List<String> ids;
			try {
				if (existsFolder(treeId, id, StorageType.WORKING,
						pathPerformer.getStorageParameters())) {
					final UserizedFolder[] path = pathPerformer.doPath(treeId,
							id, true);
				...
				final org.apache.commons.logging.Log log = com.openexchange.log.Log
						.valueOf(org.apache.commons.logging.LogFactory
								.getLog(CacheFolderStorage.class));
				if (log.isDebugEnabled()) {
					log.debug(e.getMessage(), e);
				}
				try {
					ids = new ArrayList<String>(Arrays.asList(pathPerformer
							.doForcePath(treeId, id, true)));
			...
			if (FolderStorage.REAL_TREE_ID.equals(treeId)) {
				for (final String folderId : ids) {
					cache.removeFromGroup(newCacheKey(folderId, treeId),
				...
				for (final String folderId : ids) {
					...
					cache.removeFromGroup(
							newCacheKey(folderId, FolderStorage.REAL_TREE_ID),
							String.valueOf(contextId));
					if (null != folderMap) {
	 ...
	 *            The session
	 */
	public void removeSingleFromCache(final String id, final String treeId,
			final int userId, final Session session) {
		removeSingleFromCache(id, treeId, userId, session.getContextId(),
				session);
	}
	 ...
	 */
	public void removeSingleFromCache(final String id, final String treeId,
			final int userId, final int contextId, final Session session) {
		final Cache cache = globalCache;
		cache.removeFromGroup(newCacheKey(id, treeId),
				String.valueOf(contextId));
		if (userId > 0) {
			final FolderMap folderMap = optFolderMapFor(session);
			if (null != folderMap) {
		...
		if (!FolderStorage.REAL_TREE_ID.equals(treeId)) {
			// Now for real tree, too
			cache.removeFromGroup(newCacheKey(id, FolderStorage.REAL_TREE_ID),
					String.valueOf(contextId));
			if (userId > 0) {
				final FolderMap folderMap = optFolderMapFor(session);
				if (null != folderMap) {
	...
	public void clearFolder(final String treeId, final String folderId,
		 ...
		 */
		if (null == session) {
			new ClearPerformer(storageParameters.getUser(),
					storageParameters.getContext(), registry).doClear(treeId,
					folderId);
		} else {
			try {
				new ClearPerformer(new ServerSessionAdapter(session), registry)
						.doClear(treeId, folderId);
			} catch (final OXException e) {
				throw new OXException(e);
			}
		}
	...
	public void deleteFolder(final String treeId, final String folderId,
		...
		final boolean global;
		{
			...
			try {
				 ...
				 */
				final Session session = storageParameters.getSession();
				globalCache.removeFromGroup(newCacheKey(folderId, treeId),
						String.valueOf(storageParameters.getContextId()));
				final FolderMap folderMap = optFolderMapFor(session);
				if (null != folderMap) {
					folderMap.remove(folderId, treeId);
				}
			...
			if (!FolderStorage.REAL_TREE_ID.equals(treeId)) {
				...
				final FolderStorage folderStorage = registry.getFolderStorage(
						FolderStorage.REAL_TREE_ID, folderId);
				final boolean started = folderStorage.startTransaction(
						parameters, false);
				try {
					realParentId = folderStorage.getFolder(
							FolderStorage.REAL_TREE_ID, folderId, parameters)
							.getParentID();
					if (started) {
					...
					throw FolderExceptionErrorMessage.UNEXPECTED_ERROR
							.create(e);
				}
			} else {
				realParentId = null;
		 ...
		 */
		if (null == session) {
			new DeletePerformer(storageParameters.getUser(),
					storageParameters.getContext(), registry).doDelete(treeId,
					folderId, storageParameters.getTimeStamp());
		} else {
			try {
				new DeletePerformer(new ServerSessionAdapter(session), registry)
						.doDelete(treeId, folderId,
		...
		if (null != realParentId && !FolderStorage.ROOT_ID.equals(realParentId)) {
			removeFromCache(realParentId, treeId,
					storageParameters.getSession(),
					newPathPerformer(storageParameters));
		}
		if (!FolderStorage.ROOT_ID.equals(parentId)) {
			removeFromCache(parentId, treeId, storageParameters.getSession(),
					newPathPerformer(storageParameters));
			final Folder parentFolder = loadFolder(treeId, parentId,
					StorageType.WORKING, true, storageParameters);
			if (parentFolder.isCacheable()) {
				putFolder(parentFolder, treeId, storageParameters);
			}
	...
	public String getDefaultFolderID(final User user, final String treeId,
		...
		final FolderStorage storage = registry.getFolderStorageByContentType(
				treeId, contentType);
		if (null == storage) {
			throw FolderExceptionErrorMessage.NO_STORAGE_FOR_CT.create(treeId,
					contentType);
		}
		final String folderId;
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			folderId = storage.getDefaultFolderID(user, treeId, contentType,
	...
	public Type getTypeByParent(final User user, final String treeId,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			type = storage.getTypeByParent(user, treeId, parentId,
					storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	...
	public boolean containsForeignObjects(final User user, final String treeId,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			final boolean containsForeignObjects = storage
					.containsForeignObjects(user, treeId, folderId,
							storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	...
	public boolean isEmpty(final String treeId, final String folderId,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			final boolean isEmpty = storage.isEmpty(treeId, folderId,
					storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	...
	public void updateLastModified(final long lastModified,
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			storage.updateLastModified(lastModified, treeId, folderId,
					storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
		 ...
		 */
		removeFromCache(folderId, treeId, storageParameters.getSession(),
				newPathPerformer(storageParameters));
	}
	...
	private Folder getCloneFromCache(final String treeId,
			final String folderId, final StorageParameters storageParameters) {
		final int contextId = storageParameters.getContextId();
		/*
		 * Try global cache key
		 */
		Folder folder = (Folder) globalCache.getFromGroup(
				newCacheKey(folderId, treeId), String.valueOf(contextId));
		if (null != folder) {
			/*
		 ...
		 * Try user cache key
		 */
		final FolderMap folderMap = optFolderMapFor(storageParameters
				.getSession());
	...
	public List<Folder> getFolders(final String treeId,
		...
		final TObjectIntHashMap<String> toLoad = new TObjectIntHashMap<String>(
				size);
		/*
		...
		for (int i = 0; i < size; i++) {
			...
			final Folder folder = getCloneFromCache(treeId, folderId,
					storageParameters);
			if (null == folder) {
				/*
		...
		if (toLoad.isEmpty()) {
			...
			fromStorage = loadFolders(treeId,
					Arrays.asList(toLoad.keys(new String[toLoad.size()])),
					storageType, storageParameters);
		}
		/*
		 * Fill return value
		 */
	...
	public SortableId[] getVisibleFolders(final String treeId,
		...
		final FolderStorage folderStorage = registry
				.getFolderStorageByContentType(treeId, contentType);
		...
		final boolean started = folderStorage.startTransaction(
				storageParameters, true);
		try {
			final SortableId[] ret = folderStorage.getVisibleFolders(treeId,
					contentType, type, storageParameters);
			if (started) {
				folderStorage.commitTransaction(storageParameters);
			}
			return ret;
	...
	public SortableId[] getSubfolders(final String treeId,
		...
		if (null == subfolders) {
			 ...
			 * Get needed storages
			 */
			final FolderStorage[] neededStorages = registry
					.getFolderStoragesForParent(treeId, parentId);
			...
			try {
				...
				if (1 == neededStorages.length) {
					...
					final boolean started = neededStorage.startTransaction(
							storageParameters, false);
					try {
						allSubfolderIds = Arrays.asList(neededStorage
								.getSubfolders(treeId, parentId,
										storageParameters));
						if (started) {
					...
					allSubfolderIds = new ArrayList<SortableId>(
							neededStorages.length * 8);
					final CompletionService<java.util.List<SortableId>> completionService = new ThreadPoolCompletionService<java.util.List<SortableId>>(
							CacheServiceRegistry.getServiceRegistry()
									.getService(ThreadPoolService.class));
					/*
									...
									public java.util.List<SortableId> call()
										...
										final boolean started = neededStorage
												.startTransaction(
														newParameters, false);
										try {
											final java.util.List<SortableId> l = Arrays.asList(neededStorage
													.getSubfolders(treeId,
															parentId,
															newParameters));
											if (started) {
												neededStorage
														.commitTransaction(newParameters);
											}
											return l;
										} catch (final Exception e) {
											if (started) {
												neededStorage
														.rollback(newParameters);
											}
											throw e;
										}
									}
					 ...
					 * Wait for completion
					 */
					final List<List<SortableId>> results = ThreadPools
							.takeCompletionService(completionService,
									neededStorages.length, FACTORY);
				...
				ret = allSubfolderIds.toArray(new SortableId[allSubfolderIds
						.size()]);
			} catch (final OXException e) {
				throw e;
			} catch (final Exception e) {
	...
	public void updateFolder(final Folder folder,
		...
		final String oldParentId = isMove ? getFolder(folder.getTreeID(),
				oldFolderId, storageParameters).getParentID() : null;
		if (null == session) {
			new UpdatePerformer(storageParameters.getUser(),
					storageParameters.getContext(), registry).doUpdate(folder,
					storageParameters.getTimeStamp());
		} else {
			try {
				new UpdatePerformer(new ServerSessionAdapter(session), registry)
						.doUpdate(folder, storageParameters.getTimeStamp());
			} catch (final OXException e) {
				throw new OXException(e);
			}
		}
		...
		if (isMove) {
			...
			removeFromCache(oldParentId, treeId, session,
					newPathPerformer(storageParameters));
		} else {
			removeFromCache(newFolderId, treeId, session,
					newPathPerformer(storageParameters));
		}
		/*
		 * Put updated folder
		 */
		final Folder updatedFolder = loadFolder(treeId, newFolderId,
				StorageType.WORKING, true, storageParameters);
		if (isMove) {
			/*
			...
			if (null != newParentId && !newParentId.equals(oldParentId)) {
				removeSingleFromCache(newParentId, treeId, userId,
						storageParameters.getSession());
			}
		}
	...
	private String[] getChangedFolderIDs(final int index, final String treeId,
			final Date timeStamp, final ContentType[] includeContentTypes,
			final StorageParameters storageParameters) throws OXException {
		final Session session = storageParameters.getSession();
		/*
		...
		if (null == session) {
			folders = new UpdatesPerformer(storageParameters.getUser(),
					storageParameters.getContext(),
					storageParameters.getDecorator(), registry).doUpdates(
					treeId, timeStamp, ignoreDelete, includeContentTypes)[index];
		} else {
			try {
	...
	public boolean containsFolder(final String treeId, final String folderId,
		...
		final boolean started = storage.startTransaction(storageParameters,
				true);
		try {
			final boolean contains = storage.containsFolder(treeId, folderId,
					storageType, storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	 ...
	 */
	private CacheKey newCacheKey(final String folderId, final String treeId,
			final int cid, final int user) {
		return cacheService.newCacheKey(cid, Integer.valueOf(user), treeId,
				folderId);
	}

	private boolean existsFolder(final String treeId, final String folderId,
			final StorageType storageType,
			final StorageParameters storageParameters) throws OXException {
		final FolderStorage storage = registry.getFolderStorage(treeId,
				folderId);
		...
		final boolean started = storage.startTransaction(storageParameters,
				false);
		try {
			final boolean exists = storage.containsFolder(treeId, folderId,
					storageType, storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	...
	private Folder loadFolder(final String treeId, final String folderId,
			final StorageType storageType,
			final StorageParameters storageParameters) throws OXException {
		return loadFolder(treeId, folderId, storageType, false,
				storageParameters);
	}

	private Folder loadFolder(final String treeId, final String folderId,
			final StorageType storageType, final boolean readWrite,
			final StorageParameters storageParameters) throws OXException {
		final FolderStorage storage = registry.getFolderStorage(treeId,
				folderId);
		if (null == storage) {
		...
		final boolean started = storage.startTransaction(storageParameters,
				readWrite);
		try {
			final Folder folder = storage.getFolder(treeId, folderId,
					storageType, storageParameters);
			if (started) {
				storage.commitTransaction(storageParameters);
			}
	...
	private Map<String, Folder> loadFolders(final String treeId,
			final List<String> folderIds, final StorageType storageType,
			final StorageParameters storageParameters) throws OXException {
		/*
		...
		for (int i = 0; i < size; i++) {
			...
			if (null == tmp) {
				throw FolderExceptionErrorMessage.NO_STORAGE_FOR_ID.create(
						treeId, id);
			}
		...
		if (1 == map.size()) {
			...
			paramsProvider = new InstanceStorageParametersProvider(
					storageParameters);
		} else {
			completionService = new ThreadPoolCompletionService<Object>(
					CacheServiceRegistry.getServiceRegistry().getService(
							ThreadPoolService.class, true));

			final Session session = storageParameters.getSession();
			paramsProvider = null == session ? new SessionStorageParametersProvider(
					storageParameters.getUser(), storageParameters.getContext())
					: new SessionStorageParametersProvider(
							(ServerSession) storageParameters.getSession());
		}
		/*
		 * Create destination map
		 */
		final Map<String, Folder> ret = new ConcurrentHashMap<String, Folder>(
				size);
		int taskCount = 0;
		for (final java.util.Map.Entry<FolderStorage, TIntArrayList> entry : map
				.entrySet()) {
			...
			completionService.submit(new Callable<Object>() {
				...
				public Object call() throws Exception {
					...
					try {
						 ...
						 * Load them & commit
						 */
						final List<Folder> folders = tmp.getFolders(treeId,
								ids, storageType, newParameters);
		 ...
		 */
		ThreadPools
				.takeCompletionService(completionService, taskCount, FACTORY);
		return ret;
	}
	...
	static StorageParameters newStorageParameters(final StorageParameters source) {
		...
		if (null == session) {
			return new StorageParametersImpl(source.getUser(),
					source.getContext());
		}
	...
	public static void dropUserEntries(final int userId, final int contextId) {
		final FolderMap folderMap = FolderMapManagement.getInstance().optFor(
				userId, contextId);
		if (null != folderMap) {
			folderMap.clear();
		}
