...
public class AJPv13Exception extends AbstractOXException {
	...
	public static enum AJPCode {
		...
		/**
		 * Corrupt AJP package #%1$d: First two bytes do not indicate a package
		 * from server to container: %2$s %3$s\nWrong AJP package's data:\n%4$s
		 * <p>
		...
		/**
		 * Socket closed by web server. Wait for input data of package #%1$d
		 * took %2$dmsec.
		 */
		SOCKET_CLOSED_BY_WEB_SERVER("Socket closed by web server. Wait for input data of package #%1$d took %2$dmsec.",
		...
		INTEGER_VALUE_TOO_BIG(new StringBuilder("Integer value exceeds max allowed value (").append(
				AJPv13Response.MAX_INT_VALUE).append("): %1$d").toString(), Category.CODE_ERROR, 10),
		/**
		 ...
		 */
		UNPARSEABLE_HEADER_FIELD("Unparseable header field %1$s in forward request package",
				Category.SOCKET_CONNECTION, 12),
		/**
		...
		/**
		 * Cookie JSESSIONID contains non-matching JVM route: %1$s not equal to
		 * %2$s
		 */
		WRONG_JVM_ROUTE("Cookie JSESSIONID contains non-matching JVM route: %1$s not equal to %2$s",
				Category.SOCKET_CONNECTION, 19),
		/**
		 * Unexpected empty body package received from web server.
		 * Total-Received: %1$d | Content-Length: %2$d.\nCorresponding AJP
		 * forward package:\n%3$s
		 */
		...
		/**
		 * AJP server socket could not be bind to port %1$d. Probably another
