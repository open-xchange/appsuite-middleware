...
public class CodecUtils {
	...
	private static final String[] ENCODINGS = new String[] { ENCODING_QP, ENCODING_BASE64, ENCODING_7BIT,
			ENCODING_8BIT, ENCODING_BINARY, ENCODING_UUENCODE };
	 ...
	 */
	public static String encodeQuotedPrintable(final String originalStr, final String charset)
			throws UnsupportedEncodingException {
	...
	/**
	 * Encodes specified original string with given character encoding and
	 * transfer encoding <code>BASE 64</code>.
	 * 
	 * @param originalStr
	 *            The original string to encode
	 * @param charset
	 ...
	 *             If specified character encoding is not supported
	 */
	public static String encodeBase64(final String originalStr, final String charset)
			throws UnsupportedEncodingException {
	...
	/**
	 * Decodes the specified possibly decoded string using the following
	 * encodings: "quoted-printable", "base64", "7bit", "8bit", "binary", and
	 * "uuencode". If not encoded the string is returned as is.
	 * 
	 * @param encoded
	 *            The (possibly) encoded string
	 * @param charset
	 *            The charset encoding to use
	 * @return The decoded string if encoded; otherwise the specified string
	 *         itself
	 */
	public static String decode(final String encoded, final String charset) {
		for (int i = 0; i < ENCODINGS.length; i++) {
			if ((i == 0 || i == 1) && !isAscii(encoded)) {
				/*
				 * An quoted-printable or base64 encoded string cannot contains
	 ...
	 */
	public static String decodeQuotedPrintable(final String quotedPrintableStr, final String charset)
	 ...
	 *             If a messaging error occurs
	 */
	public static String decodeBase64(final String base64Str, final String charset) throws IOException,
			MessagingException {
		return decode(base64Str, ENCODING_BASE64, charset);
	}

	/**
	 * Decodes specified encoded string using given transfer-encoding and
	 * character-encoding.
	 * 
	 * @param encodedStr
	 *            The encoded string
	 ...
	 *             If a messaging error occurs
	 */
	private static String decode(final String encodedStr, final String transferEncoding, final String charset)
			throws IOException, MessagingException {
		final InputStream inStream = MimeUtility.decode(new UnsynchronizedByteArrayInputStream(encodedStr
				.getBytes(charset)), transferEncoding);
