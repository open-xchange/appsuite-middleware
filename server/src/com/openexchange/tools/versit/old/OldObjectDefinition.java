...
public class OldObjectDefinition implements VersitDefinition {
    ...
    public OldObjectDefinition(final String[] propertyNames, final OldPropertyDefinition[] properties) {
        this(propertyNames, properties, new String[] {}, new OldObjectDefinition[] {});
    }

    public OldObjectDefinition(final String[] propertyNames, final OldPropertyDefinition[] properties, final String[] childNames, final OldObjectDefinition[] children) {
    ...
    private OldObjectDefinition(final String name, final HashMap<String, OldPropertyDefinition> properties, final HashMap<String, OldObjectDefinition> children) {
        Name = name;
    ...
    public void write(final OldFoldingWriter fw, final VersitObject object) throws IOException {
        fw.write("BEGIN");
        fw.write(":");
    ...
    public VersitObject parseBegin(final Reader reader) throws IOException {
        ...
        while (s.peek != -1) {
            if ("BEGIN".equalsIgnoreCase(s.parseName()) && object.name.equalsIgnoreCase(getType(s))) {
                return object;
            }
    ...
    public VersitObject parseChild(final Reader reader, final VersitObject object) throws IOException {
        ...
        while (s.peek != -1) {
            ...
            if ("BEGIN".equalsIgnoreCase(name) && s.peek != '.' && s.peek != ';') {
