...
public class OldRecurrencePropertyDefinition extends OldPropertyDefinition {
    ...
    public OldRecurrencePropertyDefinition(final String[] paramNames, final OldParamDefinition[] params) {
    ...
    protected Object parseValue(final Property property, final OldScanner s, final byte[] value, final String charset) throws IOException {
        final StringScanner ss = new StringScanner(s, new String(value, charset).trim().toUpperCase());
        ...
        case 'M':
            ...
            case 'P':
                ...
                while (week != null || ss.peek >= 'A' && ss.peek <= 'Z') {
                    if (week != null) {
                        weekNo = (week.charAt(0) - '0') * (week.charAt(1) == '-' ? -1 : 1);
                    } else {
                        if (weekNo == 0) {
    ...
    protected String writeValue(final Property property) {
        ...
        case RecurrenceValue.WEEKLY:
            ...
            for (int k = 0; k < weeklySize; k++) {
                sb.append(' ');
                sb.append(weekdays[((RecurrenceValue.Weekday) weeklyIter.next()).day - Calendar.SUNDAY]);
            }
            break;
        case RecurrenceValue.MONTHLY:
            ...
            if (recur.ByDay.size() > 0) {
                ...
                for (int k = 0; k < monthlySize; k++) {
                    final RecurrenceValue.Weekday day = (RecurrenceValue.Weekday) monthlyIter.next();
