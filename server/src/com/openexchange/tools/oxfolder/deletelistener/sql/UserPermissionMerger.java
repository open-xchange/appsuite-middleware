...
public final class UserPermissionMerger {
	...
	private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory
			.getLog(UserPermissionMerger.class);
	 ...
	 */
	public static void handleCorruptUserPermissions(final CorruptPermission[] corruptPermissions, final Connection con)
			throws SQLException {
		...
		for (final CorruptPermission corruptPermission : corruptPermissions) {
			 ...
			 * permission on the folder in question. If he does entity's
			 * permission has to be deleted whereby admin's permission is set to
			 * merged permission. Otherwise entity's permission is merged and
			 * assigned to admin.
			 */
			delete[0] = false;
			final Permission merged = getMergedPermission(corruptPermission.permission_id, admin,
			...
			if (delete[0]) {
				...
				if (LOG.isInfoEnabled()) {
					...
					LOG.info(logBuilder.append("Permission deleted for entity ")
							.append(corruptPermission.permission_id).append(" on folder ").append(
									corruptPermission.fuid).append(" in context ").append(corruptPermission.cid)
							.toString());
				}
				...
				if (LOG.isInfoEnabled()) {
					logBuilder.setLength(0);
					LOG
							.info(logBuilder.append("...and merged to context admin: ").append(merged.toString())
									.toString());
				}
			} else {
				updatePermission(merged, corruptPermission.permission_id, admin, corruptPermission.fuid,
						corruptPermission.cid, con);
				if (LOG.isInfoEnabled()) {
					logBuilder.setLength(0);
					LOG.info(logBuilder.append("Permission merged to context admin: ").append(merged.toString())
							.toString());
				}
			}
		}
	...
	private static final String SQL_REASSIGN_UPDATE_PERM = "UPDATE oxfolder_permissions "
			+ "SET fp = ?, orp = ?, owp = ?, odp = ?, admin_flag = ?, permission_id = ?, group_flag = 0 "
