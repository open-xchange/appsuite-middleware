...
public final class JSONMessageHandler implements MailMessageHandler {
	...
	private JSONMessageHandler(final MailPath mailPath, final MailMessage mail, final DisplayMode displayMode,
		...
		try {
			...
			if (null != mail) {
				...
				jsonObject.put(MailJSONField.HAS_ATTACHMENTS.getKey(), mail.containsHasAttachment() ? mail
						.hasAttachment() : mail.getContentType().isMimeType(MIMETypes.MIME_MULTIPART_MIXED));
	...
	public boolean handleInlinePlainText(final String plainTextContentArg, final ContentType contentType,
		...
		try {
			 ...
			 */
			asPlainText(id, contentType.getBaseType(), HTMLProcessing.formatTextForDisplay(plainTextContentArg, usm,
					displayMode));
			if (textAppended) {
				/*
				 * A plain text message body has already been detected; append
				 * inline text as an attachment, too
				 */
				asAttachment(id, contentType.getBaseType(), plainTextContentArg.length(), fileName);
			} else {
				textAppended = true;
	...
	private String getHtmlDisplayVersion(final ContentType contentType, final String src) {
		if (contentType.isMimeType(MIMETypes.MIME_TEXT_ENRICHED)
				|| contentType.isMimeType(MIMETypes.MIME_TEXT_RICHTEXT)) {
			return HTMLProcessing.formatHTMLForDisplay(ENRCONV.convert(src), contentType.getCharsetParameter(),
					session, mailPath, usm, modified, displayMode);
			...
			return HTMLProcessing.formatHTMLForDisplay(RTF2HTMLConverter.convertRTFToHTML(src), contentType
					.getCharsetParameter(), session, mailPath, usm, modified, displayMode);
		}
	...
	private void asDisplayHtml(final String id, final String baseContentType, final String htmlContent,
		...
		try {
			final JSONObject jsonObject = new JSONObject();
			jsonObject.put(MailListField.ID.getKey(), id);
			final String content = HTMLProcessing.formatHTMLForDisplay(htmlContent, charset, session, mailPath, usm,
