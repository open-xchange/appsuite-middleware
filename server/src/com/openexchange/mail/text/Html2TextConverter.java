...
public final class Html2TextConverter {
    ...
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(Html2TextConverter.class);
    ...
    private static final Pattern PAT_STYLE_RM = Pattern.compile("<style[^>]*>.*?</style[^>]*>", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
    ...
    private static boolean isTag(final String tag, final String pattern) {
        return Pattern.compile(new StringBuilder(16).append('<').append(pattern).append("[^>]*>").toString(), Pattern.CASE_INSENSITIVE).matcher(
            tag).matches();
    }

    private static final Pattern PATTERN_HREF_CONTENT = Pattern.compile("href=\"?([^\\s\">]+)\"?", Pattern.CASE_INSENSITIVE);

    private static final Pattern PATTERN_ALT_CONTENT = Pattern.compile("alt=\"?([^\">]+)\"?", Pattern.CASE_INSENSITIVE);

    private static final Pattern PATTERN_SRC_CONTENT = Pattern.compile("src=\"?([^\\s\">]+)\"?", Pattern.CASE_INSENSITIVE);
    ...
    private String convertTag(final String t, final int prevChar) {
        ...
        if (isTag(t, "body")) {
            ...
            result = LINEBREAK;
        } else if (isTag(t, "h1") || isTag(t, "h2") || isTag(t, "h3") || isTag(t, "h4") || isTag(t, "h5") || isTag(t, "h6") || isTag(
            t,
            "h7")) {
            result = LINEBREAK;
        } else if (isTag(t, "/h1") || isTag(t, "/h2") || isTag(t, "/h3") || isTag(t, "/h4") || isTag(t, "/h5") || isTag(t, "/h6") || isTag(
