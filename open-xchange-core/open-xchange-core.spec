...
EOF
    ...
    if ox_scr_todo SCR-676
    then
        # rename all appenders, converters, encoders and policies classes
        logconfig=/opt/open-xchange/etc/logback.xml
        tmp=${logconfig}.tmp
        cp -a --remove-destination $logconfig $tmp
        sed -i 's/com\.openexchange\.logback\.extensions\.logstash\.LogstashSocketAppender/com.openexchange.logback.extensions.appenders.logstash.LogstashSocketAppender/g' $tmp
        sed -i 's/com\.openexchange\.logback\.extensions\.logstash\.CustomFieldsAction/com.openexchange.logback.extensions.encoders.CustomFieldsAction/g' $tmp
