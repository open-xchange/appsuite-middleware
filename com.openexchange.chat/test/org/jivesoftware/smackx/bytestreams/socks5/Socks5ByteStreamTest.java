...
public class Socks5ByteStreamTest extends SmackTestCase {
    ...
    public void testInitializationSocks5FeaturesAndListenerOnStartup() throws XMPPException {
        final Connection connection = getConnection(0);
...
//        assertEquals(XMPPError.Condition.no_acceptable.toString(), result.getError().getCondition());
    ...
    public void testSocks5BytestreamWithLocalSocks5Proxy() throws Exception {
        ...
        final Connection initiatorConnection = getConnection(0);
        final Connection targetConnection = getConnection(1);

        // test data
        final byte[] data = new byte[] { 1, 2, 3 };
        final SynchronousQueue<byte[]> queue = new SynchronousQueue<byte[]>();

        final Socks5BytestreamManager targetByteStreamManager = Socks5BytestreamManager.getBytestreamManager(targetConnection);

        final Socks5BytestreamListener incomingByteStreamListener = new Socks5BytestreamListener() {

            @Override
            public void incomingBytestreamRequest(final Socks5BytestreamRequest request) {
