...
public class LocalCache<K, V> implements EntryListener<K, V> {
     ...
     */
    public static <K, V> LocalCache<K, V> newLocalCache(final IMap<K, V> distributedMap) {
        final LocalCache<K, V> localCache = new LocalCache<K, V>();
        // Listen for all added/updated/removed entries
        distributedMap.addEntryListener(localCache, true);
        return localCache;
    }
