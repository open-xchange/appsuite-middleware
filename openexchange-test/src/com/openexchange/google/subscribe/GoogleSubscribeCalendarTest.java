...
public class GoogleSubscribeCalendarTest extends AbstractGoogleSubscribeTest {
    ...
    public void testSingleAppointment() {
        final int parentFolderID = getFolderManager().getFolderFromServer("com.openexchange.subscribe.google.calendar").getObjectID();
        final Appointment[] appointments = getCalendarManager().all(parentFolderID, new Date(0), new Date(Long.MAX_VALUE));
        for(Appointment a : appointments) {
            System.err.println(a.getTitle() + " "  + a.getObjectID());
        }
    }
}
