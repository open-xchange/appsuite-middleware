...
public class Bug21794Test extends CalDAVTest {
	...
	public void testDeclineImportedAppointment() throws Exception {
         ...
         */
        for (String[] line : iCalResource.getICalFile().getLines()) {
        	if (line[0].equals("ATTENDEE") && line[2].contains(super.getAJAXClient().getValues().getDefaultAddress())) {
        		line[1] = line[1].replace("ACCEPTED", "DECLINED");
        		break;
        	}
        }
        ...
        assertEquals("UID wrong", uid, iCalResource.getUID());
        String[] attendeeLine = null;
        for (String[] line : iCalResource.getICalFile().getLines()) {
        	if (line[0].equals("ATTENDEE") && line[2].contains(Config.getLogin())) {
        		attendeeLine = line;
        	}
        }
        assertNotNull("declining attendee not found", attendeeLine);
        assertTrue("partstat status wrong", attendeeLine[1].contains("DECLINED"));
        assertFalse("partstat status wrong", attendeeLine[1].contains("ACCEPTED"));
	}
