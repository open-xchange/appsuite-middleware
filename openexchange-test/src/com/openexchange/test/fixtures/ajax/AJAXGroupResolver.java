...
public class AJAXGroupResolver implements GroupResolver {
	...
	public ContactObject[] resolveGroup(String simpleName) {
		// TODO Auto-generated method stub
		return null;
	}

	public ContactObject[] resolveGroup(int groupId) {
		GetRequest group = new GetRequest(groupId);
		try {
			AbstractAJAXResponse response = client.execute(group);
			JSONObject data = (JSONObject) response.getData();
			JSONArray members = data.getJSONArray("members");
			ContactObject[] groupMembers = new ContactObject[members.length()];
