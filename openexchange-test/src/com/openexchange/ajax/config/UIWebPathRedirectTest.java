...
/**
 * {@link UIWebPathRedirectTest}
 *
 * @author <a href="mailto:kevin.ruthmann@open-xchange.com">Kevin Ruthmann</a>
 * @since v7.8.0
 */
public class UIWebPathRedirectTest extends AbstractAJAXSession {

    private String login;

    private String password;

    public UIWebPathRedirectTest(String name) {
        super(name);
    }
    ...
    /**
     * Tests if UIWebPath is automatically assigned.
     * 
     * Expects the following configuration:
     * com.openexchange.ajax.login.http-auth.client=open-xchange-appsuite
     * com.openexchange.UIWebPath=client-defined
     * 
     * @throws OXException
     * @throws IOException
     * @throws JSONException
     */
    public void testUIWebPathRedirect() throws OXException, IOException, JSONException {

        AJAXSession session = new AJAXSession();
        AJAXClient myClient = new AJAXClient(session, false);
        try {
            //Test Appsuite client
            String client = "open-xchange-appsuite";
            String version = "7.8.0";
            FormLoginResponse response = myClient.execute(new FormLoginRequest(login, password, LoginTools.generateAuthId(), client, version, false));
            assertNotNull("Path of redirect response is not found.", response.getPath());
            assertTrue("Path of redirect response is wrong.", response.getPath().contentEquals("/appsuite/"));
        } finally {
            myClient.logout();
        }
    }
