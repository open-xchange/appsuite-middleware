...
public class Bug4344Test extends AppointmentTest {
	...
	public Bug4344Test(String name) {
		super(name);
	}
	...
	public void testBug4392() throws Exception {
		...
		appointmentObj.removeUntil();
		
		AppointmentObject loadAppointment = loadAppointment(getWebConversation(), objectId, appointmentFolderId, start, end, _appointmentFields, timeZone, getHostName(), getSessionId()); 
		try {
			compareObject(appointmentObj, loadAppointment, appointmentObj.getStartDate().getTime()+dayInMillis, appointmentObj.getEndDate().getTime()+dayInMillis);
