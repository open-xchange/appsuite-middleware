...
public class FolderUpdatesParser extends CommonUpdatesParser<FolderUpdatesResponse> {
    ...
    public boolean getsIgnored(final int column) {
        return column == DataObject.LAST_MODIFIED_UTC || column == FolderObject.OWN_RIGHTS // generated based on user
            ...
            || column == FolderObject.STANDARD_FOLDER || column == FolderObject.TOTAL || column == FolderObject.NEW || column == FolderObject.UNREAD || column == FolderObject.DELETED || column == FolderObject.CAPABILITIES || column == FolderObject.SUBSCRIBED || column == FolderObject.SUBSCR_SUBFLDS
            || column > 1000 ;//ibg numbers are used by addons that might not be installed for core tests.
    }
