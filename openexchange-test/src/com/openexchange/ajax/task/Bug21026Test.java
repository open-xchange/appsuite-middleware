...
public class Bug21026Test extends AbstractAJAXSession {
    ...
    protected void setUp() throws Exception {
        ...
        client.execute(new InsertRequest(task, TimeZones.UTC)).fillTask(task);
        first = new Task();
        ...
        first.setLastModified(client.execute(new UpdateRequest(first, TimeZones.UTC)).getTimestamp());
    ...
    public void testNoFurtherOccurrence() throws OXJSONException, AjaxException, IOException, JSONException {
        assertNotNull("Second occurrence not found.", second);
        assertEquals("Occurrences of first task is wrong.", 2, client.execute(new GetRequest(first.getParentFolderID(), first.getObjectID())).getTask(TimeZones.UTC).getOccurrence());
        assertEquals("Occurrences of second task is wrong.", 1, client.execute(new GetRequest(second.getParentFolderID(), second.getObjectID())).getTask(TimeZones.UTC).getOccurrence());
        ...
        finish.setLastModified(client.execute(new UpdateRequest(finish, TimeZones.UTC)).getTimestamp());
    ...
    private Task findNext(final Task previous) throws AjaxException, IOException, JSONException {
