...
public class VCardSingleAndBatchExportTest extends AbstractManagedContactTest {
    ...
    public void testVCardSingleExport() throws OXException, IOException, JSONException {
        ...
        assertFileName(vcardExportResponse.getHttpResponse(), contact.getGivenName()+" "+contact.getSurName());
    }
    ...
    public void testVCardSingleDistributionListExport() throws OXException, IOException, JSONException {
        ...
        String[] result = vcard.split("END:VCARD\\r?\\nBEGIN:VCARD");
        assertEquals("One vCard expected!", 1, result.length);        
        assertFileName(vcardExportResponse.getHttpResponse(), list.getDisplayName());
    }  
    ...
    public void testCrossFolderBatchExportTest() throws OXException, IOException, JSONException {
        ...
        assertFileName(vcardExportResponse.getHttpResponse(), "Contacts");
    }
    
    private void assertFileName(HttpResponse httpResp, String expectedFileName) {
        Header[] headers = httpResp.getHeaders("Content-Disposition");
        for (Header header : headers) {
            assertNotNull(header.getValue());
            assertTrue(header.getValue().contains(expectedFileName+".vcf"));
        }
    }    
    
    private JSONObject addRequestIds(int folderId, int objectId) throws JSONException {
