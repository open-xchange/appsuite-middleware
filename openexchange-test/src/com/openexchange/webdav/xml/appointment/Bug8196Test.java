...
public class Bug8196Test extends AppointmentTest {
    ...
    public void testBug8196() throws Exception {
        final FolderObject folderObj = FolderTest.getAppointmentDefaultFolder(getSecondWebConversation(), getHostURI(), getSecondLogin(), getPassword(), context);
        ...
        Appointment loadAppointment = loadAppointment(getSecondWebConversation(), objectId, secondAppointmentFolderId, getHostURI(), getSecondLogin(), getPassword(), context);
        compareObject(appointmentObj, loadAppointment);

        final Date modified = new Date(loadAppointment.getLastModified().getTime() - 1000);

        loadAppointment = loadAppointment(getSecondWebConversation(), objectId, secondAppointmentFolderId, modified, getHostURI(), getSecondLogin(), getPassword(), context);
        compareObject(appointmentObj, loadAppointment);
