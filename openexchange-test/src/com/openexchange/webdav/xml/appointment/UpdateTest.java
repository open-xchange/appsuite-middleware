...
public class UpdateTest extends AppointmentTest {
    ...
    public void testUpdate() throws Exception {
        ...
        updateAppointment(webCon, appointmentObj, objectId, appointmentFolderId, getHostURI(), login, password, context);
    ...
    public void testUpdateConcurentConflict() throws Exception {
        ...
        try {
            updateAppointment(webCon, appointmentObj, objectId, appointmentFolderId, new Date(0), getHostURI(), login, password, context);
    ...
    public void testUpdateNotFound() throws Exception {
        ...
        try {
            updateAppointment(webCon, appointmentObj, (objectId + 1000), appointmentFolderId, new Date(0), getHostURI(), login, password, context);
    ...
    public void testUpdateAppointmentWithParticipants() throws Exception {
        ...
        final int userParticipantId = GroupUserTest.getUserId(getWebConversation(), getHostURI(), userParticipant3, getPassword(), context);
        assertTrue("user participant not found", userParticipantId != -1);
        final Group[] groupArray = GroupUserTest.searchGroup(webCon, groupParticipant, new Date(0), getHostURI(), login, password, context);
        assertTrue("group array size is not > 0", groupArray.length > 0);
        final int groupParticipantId = groupArray[0].getIdentifier();
        final Resource[] resourceArray = GroupUserTest.searchResource(webCon, resourceParticipant, new Date(0), getHostURI(), login, password, context);
    ...
    public void testUpdateRecurrenceWithDatePosition() throws Exception {
        ...
        appointmentObj.setIgnoreConflicts(true);
        final int objectId = insertAppointment(getWebConversation(), appointmentObj, getHostURI(), login, password, context);
        appointmentObj.setObjectID(objectId);
        Appointment loadAppointment = loadAppointment(getWebConversation(), objectId, appointmentFolderId, getHostURI(), login, password, context);
        compareObject(appointmentObj, loadAppointment);
        ...
        final int newObjectId = updateAppointment(getWebConversation(), appointmentObj, objectId, appointmentFolderId, getHostURI(), login, password, context);
        ...
        loadAppointment = loadAppointment(getWebConversation(), newObjectId, appointmentFolderId, getHostURI(), login, password, context);

        // Loaded exception MUST NOT contains any recurrence information except recurrence identifier and position.
        compareObject(appointmentObj, loadAppointment);

        deleteAppointment(getWebConversation(), new int[][] { { objectId, appointmentFolderId } }, getHostURI(), login, password, context);
    }
    ...
    public void testShouldExtendSeriesFromLimitedToEndless() throws OXException, Exception {
        ...
        updateAppointment(getWebConversation(), appointmentObj, objectId, appointmentFolderId, getHostURI(), login, password, context);

        loadAppointment = loadAppointment(getWebConversation(), objectId, appointmentFolderId, getHostURI(), login, password, context);
