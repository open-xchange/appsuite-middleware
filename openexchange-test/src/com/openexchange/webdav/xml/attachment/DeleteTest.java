...
public class DeleteTest extends AttachmentTest {
    ...
    public void testDeleteAttachment() throws Exception {
        final FolderObject folderObj = FolderTest.getContactDefaultFolder(webCon, getHostURI(), getLogin(), getPassword(), context);
        final int contactFolderId = folderObj.getObjectID();
        final Contact contactObj = new Contact();
        contactObj.setSurName("testDeleteAttachment");
        contactObj.setParentFolderID(contactFolderId);

        final int objectId = ContactTest.insertContact(webCon, contactObj, getHostURI(), getLogin(), getPassword(), context);
        ...
        final int attachmentId = insertAttachment(webCon, attachmentObj, byteArrayInputStream, getHostURI(), getLogin(), getPassword(), context);
        assertTrue("attachment is 0", attachmentId > 0);

        attachmentObj.setId(attachmentId);

        deleteAttachment(webCon, attachmentObj, getHostURI(), getLogin() + "@" + context, getPassword());
    }
}
