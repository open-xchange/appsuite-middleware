...
public class PermissionTest extends TaskTest {
    ...
    public void testInsertTaskInPrivateFolderWithoutPermission() throws Exception {
        ...
        try {
            insertTask(getSecondWebConversation(), taskObj, getHostURI(), getSecondLogin(), getPassword(), context);
    ...
    public void testUpdateTaskInPrivateFolderWithoutPermission() throws Exception {
        ...
        final int taskObjectId = insertTask(getWebConversation(), taskObj, getHostURI(), getLogin(), getPassword(), context);
        taskObj.setObjectID(taskObjectId);

        try {
            updateTask(getSecondWebConversation(), taskObj, taskObjectId, parentFolderId, getHostURI(), getSecondLogin(), getPassword(), context);
            fail("permission exception expected!");
        } catch (final OXException exc) {
            assertExceptionMessage(exc.getMessage(), XmlServlet.PERMISSION_STATUS);
        }
    ...
    public void testDeleteTaskInPrivateFolderWithoutPermission() throws Exception {
        ...
        try {
            deleteTask(getSecondWebConversation(), taskObjectId, parentFolderId, getHostURI(), getSecondLogin(), getPassword(), context);
            fail("permission exception expected!");
        } catch (final OXException exc) {
            assertExceptionMessage(exc.getMessage(), XmlServlet.PERMISSION_STATUS);
        }
