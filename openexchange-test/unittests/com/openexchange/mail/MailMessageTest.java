...
public final class MailMessageTest extends AbstractMailTest {
    ...
    public void testGetMessages() {
        try {
            ...
            try {
                ...
                final MailMessage[] msgs = mailConnection.getMessageStorage().searchMessages(inboxFolder.getFullname(), null, MailSortField.RECEIVED_DATE, OrderDirection.DESC, null, COMMON_LIST_FIELDS);
                ...
                if (cached) {
                     ...
                     */
                    final MailMessage[] mails = mailConnection.getMessageStorage().getMessages(inboxFolder.getFullname(), uids, new MailField[] { MailField.SIZE });
    ...
    private void checkSubfolders(final MailFolder parent, final MailAccess<?, ?> mailConnection) throws OXException {
        ...
        assertTrue("Has Subfolders is wrong!", parent.hasSubfolders() ? subfolders != null && subfolders.length > 0 : subfolders == null || subfolders.length <= 0);
    ...
    private void checkMessages(final MailFolder folder, final MailAccess<?, ?> mailConnection) {
        ...
        for (int i = 0; i < msgs.length; i++) {
            ...
            assertTrue(String.format(TEMPL, "Subject", Long.valueOf(msgs[i].getMailId()), folder.getFullname()), msgs[i].containsSubject());
            assertTrue(String.format(TEMPL, "Priority", Long.valueOf(msgs[i].getMailId()), folder.getFullname()), msgs[i].containsPriority());
            assertTrue(String.format(TEMPL, "To", Long.valueOf(msgs[i].getMailId()), folder.getFullname()), msgs[i].containsTo());
            assertTrue(String.format(TEMPL, "Flags", Long.valueOf(msgs[i].getMailId()), folder.getFullname()), msgs[i].containsFlags());
            assertTrue(String.format(TEMPL, "User Flags", Long.valueOf(msgs[i].getMailId()), folder.getFullname()), msgs[i].containsUserFlags());
        }
    }
}
