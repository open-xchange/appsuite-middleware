...
import com.openexchange.publish.helpers.AccessInformationImpl;
...
public class PublicationSQLTest extends AbstractPublicationSQLStorageTest {
    ...
    public void testShouldStoreUsernameAndPassword() throws PublicationException{
        pub1.addAccessInformation(new AccessInformationImpl("tobias.prinz@open-xchange.com", "myHashedPassword"));
        storage.rememberPublication(pub1);
        List<Publication> publications = storage.getPublicationsOfUser(ctx, ctx.getContextId(), userId);
        assertEquals("Should find one publication", publications.size(), 3);
        
        List<AccessInformationImpl> accessInformation = publications.get(0).getAccessInformation();
        assertEquals("Should find one set of access restrictions", accessInformation.size(), 1);
        assertEquals("Username should match", "tobias.prinz@open-xchange.com", accessInformation.get(0).getUsername());
        assertEquals("Password should match", "myHashedPassword", accessInformation.get(0).getPasswordHash());
    }
