...
public class TestContextWatcher implements Runnable {
    ...
    public void run() {
        while (true) {
            sleep();
            checkAndRemoveOverdues();
        }
    }

    private void checkAndRemoveOverdues() {
        for (Map.Entry<TestContext, Long> contextToCheck : contextsInUse.entrySet()) {
            long acquired = contextToCheck.getValue().longValue();
    ...
    public void contextInUse(TestContext context) {
        contextsInUse.putIfAbsent(context, Calendar.getInstance().getTimeInMillis());
    }
