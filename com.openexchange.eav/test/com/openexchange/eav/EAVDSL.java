...
import static com.openexchange.eav.TransformList.ADD;
import static com.openexchange.eav.TransformList.REMOVE;
 ...
 *
 */
public class EAVDSL extends TestCase{
    // Array Transformations

    public static EAVSetTransformation TRANS(String name, EAVSetTransformation... children) {
        EAVSetTransformation transformation = new EAVSetTransformation(name);
        transformation.addChildren(children);
        return transformation;
    }

    public static  EAVSetTransformation TRANS(String name, TransformList list) {
        return TRANS(name, list, null);
    }

    public static  EAVSetTransformation TRANS(String name, TransformList list1, TransformList list2) {
        EAVSetTransformation transformation = new EAVSetTransformation(name);
    ...
    public static  EAVNode N(String name, EAVNode... children) {
        EAVNode node = new EAVNode(name);
        node.addChildren(children);
        return node;
    }

    public static  EAVNode EMPTY_OBJECT(String name) {
        EAVNode node = new EAVNode(name);
        return node;
    }

    public static  EAVNode NULL(String name) {
        return EAVNode.nullNode(name);
    }

    // Strings
    public static  EAVNode N(String name, String... values) {
        EAVNode node = new EAVNode(name);
        if (values.length == 1) {
    ...
    public static  EAVNode MULTISET(String name, String... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(values);
        return node;
    }

    public static  EAVNode SET(String name, String... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(EAVContainerType.SET, values);
        return node;
    }

    public static  TransformList ADD(String... strings) {
        return new TransformList(EAVType.STRING, ADD, strings);
    }

    public static  TransformList REMOVE(String... strings) {
        return new TransformList(EAVType.STRING, REMOVE, strings);
    }

    // Booleans

    public static  EAVNode N(String name, Boolean... values) {
        EAVNode node = new EAVNode(name);
        if (values.length == 1) {
            node.setPayload(values[0]);
        } else {
    ...
    public static  EAVNode MULTISET(String name, Boolean... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(values);
        return node;
    }

    public static  EAVNode SET(String name, Boolean... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(EAVContainerType.SET, values);
        return node;
    }

    public static  TransformList ADD(Boolean... values) {
        return new TransformList(EAVType.BOOLEAN, ADD, values);
    }

    public static  TransformList REMOVE(Boolean... values) {
        return new TransformList(EAVType.BOOLEAN, REMOVE, values);
    }

    // Numbers

    public static  EAVNode N(String name, Number... values) {
        EAVNode node = new EAVNode(name);
        if (values.length == 1) {
            node.setPayload(values[0]);
        } else {
    ...
    public static  EAVNode MULTISET(String name, Number... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(values);
        return node;
    }

    public static  EAVNode SET(String name, Number... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(EAVType.NUMBER, EAVContainerType.SET, values);
        return node;
    }

    public static  TransformList ADD(Number... values) {
        return new TransformList(EAVType.NUMBER, ADD, values);
    }

    public static  TransformList REMOVE(Number... values) {
    ...
    public static  EAVNode N(String name, EAVType type, Number... values) {
    ...
    public static  EAVNode N(String name, byte[]... values) {
        EAVNode node = new EAVNode(name);
        if (values.length == 1) {
    ...
    public static  EAVNode MULTISET(String name, byte[]... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(wrap(values));
        return node;
    }

    public static  EAVNode SET(String name, byte[]... values) {
        EAVNode node = new EAVNode(name);
        node.setPayload(EAVContainerType.SET, wrap(values));
        return node;
    }

    public static  TransformList ADD(byte[]... values) {
        return new TransformList(EAVType.NUMBER, ADD, wrap(values));
    }

    public static  TransformList REMOVE(byte[]... values) {
        return new TransformList(EAVType.NUMBER, REMOVE, wrap(values));
    }

    private static InputStream[] wrap(byte[]... values) {
    ...
    public static  EAVNode MULTISET(String name, EAVType type, Number... values) {
        if (!ACCEPTABLE_NUMBER_TYPES.contains(type)) {
            throw new IllegalArgumentException("Numbers can not be of type " + type.name());
        }
        EAVNode node = new EAVNode(name);
    ...
    public static  EAVNode SET(String name, EAVType type, Number... values) {
        if (!ACCEPTABLE_NUMBER_TYPES.contains(type)) {
            throw new IllegalArgumentException("Numbers can not be of type " + type.name());
        }
        EAVNode node = new EAVNode(name);
    ...
    public static  TransformList ADD(EAVType type, Number... values) {
        if (!ACCEPTABLE_NUMBER_TYPES.contains(type)) {
            throw new IllegalArgumentException("Numbers can not be of type " + type.name());
        }
    ...
    public static  TransformList REMOVE(EAVType type, Number... values) {
        if (!ACCEPTABLE_NUMBER_TYPES.contains(type)) {
            throw new IllegalArgumentException("Numbers can not be of type " + type.name());
        }
    ...
    public static  EAVTypeMetadataNode TYPE(String name, EAVTypeMetadataNode... children) {
        EAVTypeMetadataNode node = new EAVTypeMetadataNode(name);
        node.addChildren(children);
        return node;
    }

    public static  EAVTypeMetadataNode TYPE(String name, EAVType type) {
        EAVTypeMetadataNode node = new EAVTypeMetadataNode(name);
        node.setType(type);
    ...
    public static  EAVTypeMetadataNode TYPE(String name, EAVType type, EAVContainerType cType) {
        EAVTypeMetadataNode node = new EAVTypeMetadataNode(name);
        node.setType(type);
    ...
    public static  EAVTypeMetadataNode TYPE(String name, EAVType type, EAVContainerType cType, Map<String, Object> options) {
        EAVTypeMetadataNode node = new EAVTypeMetadataNode(name);
        node.setType(type);
        node.setContainerType(cType);
    ...
    public static  EAVTypeMetadataNode TYPE(String name, EAVType type, Map<String, Object> options) {
        EAVTypeMetadataNode node = new EAVTypeMetadataNode(name);
        node.setType(type);
        node.setContainerType(EAVContainerType.SINGLE);
        node.setOptions(options);
    ...
    private static <T extends AbstractNode<T>>  void failComparison(String message, AbstractNode<T> expected, AbstractNode<T> actual) {
        org.junit.Assert.assertEquals(message, treeString(expected), treeString(actual));
        fail(message);
    }
    ...
    public static void assertType(EAVTypeMetadataNode types, EAVType type, EAVContainerType cType, String... pathElements) {
    ...
    public static Map<String, Object> M(String... strings) {
