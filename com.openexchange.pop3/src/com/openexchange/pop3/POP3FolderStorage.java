...
public final class POP3FolderStorage extends MailFolderStorage {
    ...
    private final POP3Store imapStore;
    ...
    public MailFolder[] getSubfolders(final String parentFullname, final boolean all) throws MailException {
        try {
            ...
            if (DEFAULT_FOLDER_ID.equals(parentFullname)) {
                parent = (Folder) imapStore.getDefaultFolder();
            ...
            if (parent.exists()) {
                ...
                if (imapConfig.isSupportsACLs() && isSelectable(parent)) {
                    try {
                        if (!aclExtension.canLookUp(RightsCache.getCachedRights(parent, true, session, accountId))) {
                            throw new POP3Exception(POP3Exception.Code.NO_LOOKUP_ACCESS, parentFullname);
                        }
                    } catch (final MessagingException e) {
                        throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, parentFullname);
                    }
                }
            ...
            synchronized (session) {
                try {
                    ...
                    if (!inboxFolder.exists()) {
                        throw new POP3Exception(POP3Exception.Code.FOLDER_NOT_FOUND, STR_INBOX);
                    }
                     ...
                     */
                    if (imapConfig.getPOP3Capabilities().hasNamespace()) {
                        /*
                        ...
                        if (personalNamespaces == null || personalNamespaces[0] == null) {
                            throw new POP3Exception(POP3Exception.Code.MISSING_PERSONAL_NAMESPACE);
                        }
                    ...
                    final SpamHandler spamHandler;
                    {
                        ...
                        try {
                            final MailAccount mailAccount = POP3ServiceRegistry.getServiceRegistry().getService(
        ...
        try {
            ...
            if (DEFAULT_FOLDER_ID.equals(parentFullname)) {
                ...
                if (toCreate.containsSeparator() && !checkFolderPathValidity(parentFullname, toCreate.getSeparator())) {
                    throw new POP3Exception(POP3Exception.Code.INVALID_FOLDER_NAME, Character.valueOf(toCreate.getSeparator()));
                }
                parent = (IMAPFolder) imapStore.getFolder(parentFullname);
            ...
            if (!parent.exists()) {
                parent = checkForNamespaceFolder(parentFullname);
                if (null == parent) {
                    throw new POP3Exception(POP3Exception.Code.FOLDER_NOT_FOUND, parentFullname);
                }
            }
            /*
            ...
            if (!inferiors(parent)) {
                throw new POP3Exception(
                    POP3Exception.Code.FOLDER_DOES_NOT_HOLD_FOLDERS,
            ...
            if (imapConfig.isSupportsACLs()) {
                try {
                    if (!aclExtension.canCreate(RightsCache.getCachedRights(parent, true, session, accountId))) {
                        throw new POP3Exception(POP3Exception.Code.NO_CREATE_ACCESS, parentFullname);
                    }
                } catch (final MessagingException e) {
                    /*
                     * MYRIGHTS command failed for given mailbox
                     */
                    if (!imapConfig.getPOP3Capabilities().hasNamespace() || !NamespaceFoldersCache.containedInPersonalNamespaces(
                        parent.getFullName(),
                        imapStore,
                        true,
                        session,
            ...
            if (!checkFolderNameValidity(name, parent.getSeparator())) {
                throw new POP3Exception(POP3Exception.Code.INVALID_FOLDER_NAME, Character.valueOf(parent.getSeparator()));
            }
            if (isParentDefault) {
                /*
            ...
            if (createMe.exists()) {
                throw new POP3Exception(POP3Exception.Code.DUPLICATE_FOLDER, createMe.getFullName());
            }
            final int ftype;
            if (POP3Config.isMBoxEnabled()) {
                /*
            ...
            try {
                if (!(created = createMe.create(ftype))) {
                    throw new POP3Exception(
                        POP3Exception.Code.FOLDER_CREATION_FAILED,
                ...
                if (initialACLs != null) {
                    ...
                    if (!equals(initialACLs, m, entity2ACL, args)) {
                        if (!aclExtension.canSetACL(createMe.myRights())) {
                            throw new POP3Exception(POP3Exception.Code.NO_ADMINISTER_ACCESS_ON_INITIAL, createMe.getFullName());
                        }
                        ...
                        if (!adminFound) {
                            throw new POP3Exception(POP3Exception.Code.NO_ADMIN_ACL, createMe.getFullName());
                        }
                        /*
                         * Apply new ACLs
                         */
        ...
        try {
            ...
            final String newName;
            {
                ...
                if (pos == -1) {
                    ...
                    if (pos == newFullname.length() - 1) {
                        throw new POP3Exception(POP3Exception.Code.INVALID_FOLDER_NAME, Character.valueOf(separator));
                    }
            ...
            if (move) {
                ...
                if (isStandardFolder(moveMe.getFullName())) {
                    throw new POP3Exception(POP3Exception.Code.NO_DEFAULT_FOLDER_UPDATE, moveMe.getFullName());
                }
                ...
                if (!destFolder.exists()) {
                    ...
                    if (null == destFolder) {
                         ...
                         */
                        throw new POP3Exception(POP3Exception.Code.FOLDER_NOT_FOUND, newParent);
                    }
                }
                if (destFolder instanceof DefaultFolder) {
                    if (!inferiors(destFolder)) {
                        throw new POP3Exception(POP3Exception.Code.FOLDER_DOES_NOT_HOLD_FOLDERS, destFolder.getFullName());
                    }
                    ...
                    try {
                        if (!aclExtension.canCreate(RightsCache.getCachedRights(destFolder, true, session, accountId))) {
                            throw new POP3Exception(POP3Exception.Code.NO_CREATE_ACCESS, newParent);
                        }
                    } catch (final MessagingException e) {
                        /*
                         * MYRIGHTS command failed for given mailbox
                        ...
                        if (!imapConfig.getPOP3Capabilities().hasNamespace() || !NamespaceFoldersCache.containedInPersonalNamespaces(
                             ...
                             * No namespace support or given parent is NOT covered by user's personal namespaces.
                             */
                            throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, newParent);
                        }
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("MYRIGHTS command failed on namespace folder", e);
                        }
                ...
                if (destFolder.getFullName().startsWith(moveMe.getFullName())) {
                    throw new POP3Exception(POP3Exception.Code.NO_MOVE_TO_SUBFLD, moveMe.getName(), destFolder.getName());
                }
                try {
            ...
            if (rename) {
                ...
                if (isStandardFolder(moveMe.getFullName())) {
                    ...
                    try {
                        if (!aclExtension.canCreate(RightsCache.getCachedRights(moveMe, true, session, accountId))) {
                            throw new POP3Exception(POP3Exception.Code.NO_CREATE_ACCESS, moveMe.getFullName());
                        }
                    } catch (final MessagingException e) {
                        throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, moveMe.getFullName());
                    }
                }
                if (!checkFolderNameValidity(newName, separator)) {
                    throw new POP3Exception(POP3Exception.Code.INVALID_FOLDER_NAME, Character.valueOf(separator));
                ...
                if (renameFolder.exists()) {
                    throw new POP3Exception(POP3Exception.Code.DUPLICATE_FOLDER, renameFolder.getFullName());
                }
                /*
                 ...
                 */
                if (!success) {
                    throw new POP3Exception(POP3Exception.Code.UPDATE_FAILED, moveMe.getFullName());
                }
                ...
                if (oldACLs != null) {
                    ...
                    if (!equals(oldACLs, m, entity2ACL, args)) {
                        ...
                        if (isStandardFolder(updateMe.getFullName()) && !stillHoldsFullRights(updateMe, newACLs, imapConfig, session, ctx)) {
                            throw new POP3Exception(POP3Exception.Code.NO_DEFAULT_FOLDER_UPDATE, updateMe.getFullName());
                        }
                        if (!aclExtension.canSetACL(RightsCache.getCachedRights(updateMe, true, session, accountId))) {
                            throw new POP3Exception(POP3Exception.Code.NO_ADMINISTER_ACCESS, updateMe.getFullName());
                        }
                        /*
                         * Check new ACLs
                         */
                        if (newACLs.length == 0) {
                            throw new POP3Exception(POP3Exception.Code.NO_ADMIN_ACL, updateMe.getFullName());
                        }
                        {
                            boolean adminFound = false;
                            for (int i = 0; (i < newACLs.length) && !adminFound; i++) {
            ...
            try {
                ...
                if (imapConfig.isSupportsACLs()) {
                    ...
                    if (!aclExtension.canRead(myrights)) {
                        throw new POP3Exception(POP3Exception.Code.NO_READ_ACCESS, f.getFullName());
                    }
                    if (!aclExtension.canDeleteMessages(myrights)) {
                        throw new POP3Exception(POP3Exception.Code.NO_DELETE_ACCESS, f.getFullName());
                    }
                }
            } catch (final MessagingException e) {
                throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, f.getFullName());
            }
                ...
                if (blockSize > 0) {
                    ...
                    while (msgCount > blockSize) {
                        ...
                        try {
                            ...
                            throw new POP3Exception(POP3Exception.Code.CONNECT_ERROR, e, imapConfig.getServer(), imapConfig.getLogin());
    ...
    public MailFolder[] getPath2DefaultFolder(final String fullname) throws MailException {
        try {
            ...
            if (imapConfig.isSupportsACLs() && isSelectable(f)) {
                try {
                    if (!aclExtension.canLookUp(RightsCache.getCachedRights(f, true, session, accountId))) {
                        throw new POP3Exception(POP3Exception.Code.NO_LOOKUP_ACCESS, fullname);
                    }
                } catch (final MessagingException e) {
                    throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, fullname);
                }
            }
    ...
    public com.openexchange.mail.Quota[] getQuotas(final String folder, final com.openexchange.mail.Quota.Type[] types) throws MailException {
        try {
            final IMAPFolder f;
            {
                ...
                try {
                    ...
                    if (imapConfig.isSupportsACLs()) {
                        final Rights myrights = RightsCache.getCachedRights(f, true, session, accountId);
                        if (!aclExtension.canRead(myrights)) {
                            throw new POP3Exception(POP3Exception.Code.NO_READ_ACCESS, fullname);
                        }
                        if (!aclExtension.canDeleteMailbox(myrights)) {
                            throw new POP3Exception(POP3Exception.Code.NO_DELETE_ACCESS, fullname);
                        }
                    }
                } catch (final MessagingException e) {
                    throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, fullname);
            ...
            if (!imapConfig.getPOP3Capabilities().hasQuota()) {
    ...
    private void deleteFolder(final IMAPFolder deleteMe) throws MailException, MessagingException {
        if (isStandardFolder(deleteMe.getFullName())) {
            throw new POP3Exception(POP3Exception.Code.NO_DEFAULT_FOLDER_DELETE, deleteMe.getFullName());
        } else if (!deleteMe.exists()) {
            throw new POP3Exception(POP3Exception.Code.FOLDER_NOT_FOUND, deleteMe.getFullName());
        }
        try {
            if (imapConfig.isSupportsACLs() && isSelectable(deleteMe) && !aclExtension.canDeleteMailbox(RightsCache.getCachedRights(
                ...
                session,
                accountId))) {
                throw new POP3Exception(POP3Exception.Code.NO_CREATE_ACCESS, deleteMe.getFullName());
            }
        } catch (final MessagingException e) {
            throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, deleteMe.getFullName());
        }
        if (deleteMe.isOpen()) {
        ...
        if (!deleteMe.delete(true)) {
            throw new POP3Exception(POP3Exception.Code.DELETE_FAILED, deleteMe.getFullName());
        }
        mailInterfaceMonitor.addUseTime(System.currentTimeMillis() - start);
        /*
    ...
    private static boolean stillHoldsFullRights(final IMAPFolder defaultFolder, final ACL[] newACLs, final POP3Config imapConfig, final Session session, final Context ctx) throws AbstractOXException, MessagingException {
        /*
    ...
    private IMAPFolder moveFolder(final IMAPFolder toMove, final IMAPFolder destFolder, final IMAPFolder newFolder, final boolean checkForDuplicate) throws MessagingException, MailException {
        ...
        if (imapConfig.isSupportsACLs() && ((toMoveType & Folder.HOLDS_MESSAGES) > 0)) {
            try {
                if (!aclExtension.canRead(RightsCache.getCachedRights(toMove, true, session, accountId))) {
                    throw new POP3Exception(POP3Exception.Code.NO_READ_ACCESS, moveFullname);
                }
            } catch (final MessagingException e) {
                throw new POP3Exception(POP3Exception.Code.NO_ACCESS, e, moveFullname);
            }
            try {
                if (!aclExtension.canCreate(RightsCache.getCachedRights(toMove, true, session, accountId))) {
                    throw new POP3Exception(POP3Exception.Code.NO_CREATE_ACCESS, moveFullname);
                }
            } catch (final MessagingException e) {
                /*
                 * MYRIGHTS command failed for given mailbox
        ...
        if (checkForDuplicate && newFolder.exists()) {
            throw new POP3Exception(POP3Exception.Code.DUPLICATE_FOLDER, newFolder.getName());
        }
        /*
        ...
        if (imapConfig.isSupportsACLs()) {
            ...
            try {
                ...
                throw new POP3Exception(POP3Exception.Code.NO_WRITE_ACCESS, e, newFullname);
            }
        }
        ...
        if (!toMove.delete(true) && LOG.isWarnEnabled()) {
            final POP3Exception e = new POP3Exception(POP3Exception.Code.DELETE_FAILED, moveFullname);
    ...
    private String checkDefaultFolder(final String prefix, final String name, final int type, final int subscribe, final StringBuilder tmp) throws MessagingException {
        ...
        if (!f.exists() && !f.create(type)) {
            final POP3Exception oxme = new POP3Exception(
                POP3Exception.Code.NO_DEFAULT_FOLDER_CREATION,
