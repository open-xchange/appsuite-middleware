...
/**
 * All command line specific menus and informations are made here <br>
 * creation : 24 août 07
 *
...
public final class CommandLine {
    ...
    /**
     * returns the command url (which is supposed to be sent to an open office server instance).
     *
     * @param command a command (ex : stop)
     * @return the uno url
     */
    private static String getCommandUrl(final String command) {
     ...
     * @throws Exception
     */
    private static void printStatus(final XNameAccess status) throws Exception {
        System.out.println("STATUS");
        System.out.println("Instances in daemon (free/total) : " + status.getByName("available") + "/" + status.getByName("poolsize"));

        final Object[] workers = (Object[]) status.getByName("workers");
        System.out.println("Worker\tId\tin use\tusages\tduration\tuser-directory");

        for (int i = 0; i < workers.length; i++) {
            final HashMap worker = namedValueArrayToHashMap((NamedValue[]) workers[i]);
            final String out = "";
            String inuse = " ";
            String duration = "        \t";
            final Long usageTime = (Long) worker.get("usage-time");
            if (usageTime.longValue() > 0) {
                inuse = "x";
    ...
    /**
     * Executes command and ask directly to the daemon instance running if necessary using some connection (socket, etc...)
     *
    ...
    private static void executeCommand(final String command) throws Exception {
        ...
        final XUnoUrlResolver unoUrlResolver = (XUnoUrlResolver) UnoRuntime.queryInterface(XUnoUrlResolver.class, unoResolverObj);

        final String url = getCommandUrl(command);
        try {
            if (command.equals(cmdStatus)) {
    ...
    private static void startAdminMode(final String configPath, final String adminCommand, final String logLevel) {
        if (!isAdminCommandValid(adminCommand)) {
            System.err.println(ERROR_TAG + "invalid admin command : " + adminCommand);
            System.exit(1);
        }
        try {
    ...
    private static void startNormalMode(final String configPath, final String logLevel) {
        final Daemon daemon = new Daemon(configPath);
    ...
    public static void start(final String[] args) {
        ...
        Logger.newInstance(new LoggerBasicImpl());
        try {
