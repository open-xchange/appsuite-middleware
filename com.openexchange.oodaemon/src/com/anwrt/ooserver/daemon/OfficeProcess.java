...
/**
 * All the informations and management of an open office server instance <br>
 * creation : 28 août 07
 *
...
public class OfficeProcess extends Thread {
    ...
    public OfficeProcess(final Daemon daemon, final String userId, final Integer index) {
        _daemon = daemon;
    ...
    private XConnector getNewConnector() throws Exception {
        ...
        final XComponentContext context = _daemon.getInitialContext();

        final Object connectorObj = context.getServiceManager().createInstanceWithContext("com.sun.star.connection.Connector", context);
    ...
    public void start() {
        /*
         * tcpNoDelay : Corresponds to the socket option tcpNoDelay. For a UNO connection, this parameter should be set to 1 (this is NOT
         * the default ? it must be added explicitly). If the default is used (0), it may come to 200 ms delays at certain call
         * combinations.
         */
        final String[] command =
            new String[] {
                OSAbstractLayer.concatPaths(_config.officeProgramDirectoryPath, "soffice"), "-env:UserInstallation=" + _userId,
    ...
    /**
     * Kills the java process representing the openOffice server instance TODO process.destroy() does not kill both soffice.bin and
     * soffice.exe, one of the two is still alive after the call
     */
    private void kill() {
        if (_process != null) {
    ...
    public void terminate() {
        if (_context != null) {
            final Thread t = new TerminateThread(_context);
     ...
     *
     * @param timeout try to connect in the given timeout, the method returns if the timeout is reached
     * @return true if connected successfully
     */
    public boolean waitTillReady(final Integer timeout) {
    ...
    public boolean isResponsive() {
        final ResponsivenessChecker rc = new ResponsivenessChecker(_config, this);
        rc.start();
        return rc.isResponsive();
    }

    /**
     * Tests if it's necessary to restart an OpenOffice instance (not responsive or maxUsageCount reached). If it is, it will try to restart
     * it.
     *
    ...
    /**
     * Builds a connection string that is used by oo servers. Pipe is used by default : daemon should resides on the same machine than open
     * office server user installations
     *
    ...
    public String toStringDetailed() {
        return "<oood.OfficeProcess " + _userId + ";process=" + _process + ";connectStr=" + getConnectString() + ",usage=" + _usage + ">";
    }
}
