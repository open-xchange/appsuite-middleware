...
/**
 * Thread that tests if an OpenOffice process is still available and responsive (no deadlock, etc...) <br>
 * creation : 30 août 07
 *
...
public class ResponsivenessChecker extends Thread {
    ...
    public ResponsivenessChecker(final Config config, final OfficeProcess process) {
        super();
        _process = process;
        _responsive = false;
        _config = config;
    ...
    public void run() {
        try {
            synchronized (this) {
                // Still alive ?
                final XComponentContext context = _process.getContext();
                final XMultiComponentFactory manager = context.getServiceManager();
                final Object desktopObj = manager.createInstanceWithContext("com.sun.star.frame.Desktop", context);
                final XDesktop desktop = (XDesktop) UnoRuntime.queryInterface(XDesktop.class, desktopObj);
            ...
            Logger.error("responsiveness-check for " + _process + " failed: " + ex);
            Logger.debug(ex);
            _responsive = false;
        }
    }
