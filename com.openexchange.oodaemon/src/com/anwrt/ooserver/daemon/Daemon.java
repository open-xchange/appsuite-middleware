 ...
 * <strong>warning</strong> if there is no output, do not forget to instantiate the logger, (ex : Logger.newInstance(new
 * LoggerBasicImpl());) otherwise the logger will not be enabled.<br>
 * <br>
 * creation : 24 août 07
 *
...
public class Daemon {
     ...
     * used to know when the daemon called the shutdown method (basically to avoid unnecessary exceptions)
     *
     * @return true if the daemon is in a shutdown process (shutdown method just called and finished)
     */
     ...
     */
    public Daemon(final Config config) {
        _config = config;
        init();
    }
     ...
     */
    public Daemon(final String configPath) {
        try {
    ...
    private void initOOAPI() throws Exception {
        ...
        // Service Manager
        final XMultiComponentFactory manager = _initialContext.getServiceManager();

        _processPool = new ProcessPool(_config);
        _shutdownThread = null;

        // Acceptor
        final Object acceptorObj = manager.createInstanceWithContext("com.sun.star.connection.Acceptor", _initialContext);
        _acceptor = (XAcceptor) UnoRuntime.queryInterface(XAcceptor.class, acceptorObj);

        // Bridge factory
        final Object bridgeFactoryObj = manager.createInstanceWithContext("com.sun.star.bridge.BridgeFactory", _initialContext);
        _bridgeFactory = (XBridgeFactory) UnoRuntime.queryInterface(XBridgeFactory.class, bridgeFactoryObj);
    }

    /**
     * called to shutdown the daemon (all the terminating code is not included into this method, there might be other code elsewhere such as
     * in run() )
     */
    ...
    public static String extractContactInfo(final String namevalue) {
        final String[] list = namevalue.split(",");
        String host = "";
        String port = "";
    ...
    private void createBridgeWithEmptyPoolInstanceProvider(final XConnection connection, final String connectionDesc) {
        Logger.error(_processPool.getStateString() + " " + connectionDesc + " rejected, all workers are busy");
        try {
            _bridgeFactory.createBridge("", "urp", connection, new EmptyPoolInstanceProvider());
        } catch (final com.sun.star.bridge.BridgeExistsException ex) {
    ...
    private void createBridgeWithOfficeInstanceProvider(final XConnection connection, final OfficeProcess process, final String connectionDesc) {
        process.startUsage();
        Logger.info(_processPool.getStateString() + " -> " + process + " serves " + connectionDesc);

        /*
         * XConnectionBroadcaster : allows to add listeners to a connection.
         */
        final XConnectionBroadcaster xConnectionBroadcaster =
            (XConnectionBroadcaster) UnoRuntime.queryInterface(XConnectionBroadcaster.class, connection);
        xConnectionBroadcaster.addStreamListener(new ConnectionListener(_processPool, process, connectionDesc));

        try {
            _bridgeFactory.createBridge("", "urp", connection, new OfficeInstanceProvider(process));
        } catch (final com.sun.star.bridge.BridgeExistsException ex) {
            Logger.error("Bridge already exists, cannot create new one");
            Logger.debug(ex);
        } catch (final com.sun.star.lang.IllegalArgumentException ex) {
    ...
    /**
     * Main loop that accepts connections from clients. When a client is accepted, it calls an Instance provider.
     */
    private void mainLoop() {
        while (true) {
            ...
            } catch (final AlreadyAcceptingException ex) {
            ...
            final String connectionDesc = extractContactInfo(connection.getDescription());
            Logger.info("Incoming request for a worker from " + connectionDesc);
            Logger.debug("process pool size : " + _processPool.size());
            final OfficeProcess process = _processPool.pop();
            if (process == null) {
    ...
    /**
     * Creates all server instances at once. There is one instance per user installation (OpenOffice server doesn't allow multithreading,
     * it's unstable). The total number of users allowed in parallel is the number of user installations.
     */
    public void fillProcessPool() {
        int index = 0;
        final Iterator<String> it = _config.userInstallation.iterator();
        while (it.hasNext()) {
            final String userId = it.next();
            final OfficeProcess p = new OfficeProcess(this, userId, new Integer(index));
            p.start();
    ...
    /**
     * Called when starting the exiting process. The shutdownThread will call shutdown() method.
     */
    private void terminate() {
        Logger.info("Accepting on " + _config.acceptor + " stopped, waiting for shutdownthread");
    ...
    /**
     * Logs a big message (more visible than normal logs) It is written as a single string to be written in a single pass (otherwise there
     * would have been conflicts with other threads writing at the same time)
     *
     * @param msg the message that will be displayed, with decoration around
     */
    private void logBigMessage(final String msg) {
        final String line = "======================================\n\n";
        final String str = "\n" + line + "    " + msg + "\n\n" + line;
        Logger.info(str);
    }
    ...
    /**
     * Treatment to be done when the program is interrupted ( warning : kill -9 does not work with it )
     */
