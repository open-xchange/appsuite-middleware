...
/**
 * A container that have all informations about configuration <br>
 * creation : 28 août 07
 *
...
public class Config {
    ...
    public List<String> userInstallation = new ArrayList<String>();
    ...
    public int calculateMaxUsageCount() {
        return (int) Math.round(maxUsageCountPerInstance + (1. - 2 * Math.random()) * randomUsageCountPerInstance);
    }

    public int convertMillisToSeconds(final int millis) {
        return (int) Math.round(millis * 0.001);
    }

    public void validate() throws Exception {
        File tmp;
        if (userInstallation.isEmpty()) {
            throw new Exception(CONFIG_TAG + "no user installation found");
        }
        if (acceptor == null || acceptor.equals("")) {
            throw new Exception(CONFIG_TAG + "acceptor is empty");
        }
        tmp = new File(officeProgramDirectoryPath);
        if (!tmp.canRead()) {
            throw new Exception(
                CONFIG_TAG + "office program directory path (" + officeProgramDirectoryPath + ") does not exist or is not readable");
        }
    }
