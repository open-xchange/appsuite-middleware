...
/**
 * Pool of ready Open Office server instances <br>
 * creation : 30 août 07
 *
...
public class ProcessPool {
     ...
     */
    public ProcessPool(final Config config) {
     ...
     */
    public synchronized void append(final OfficeProcess item) {
        if (item == null) {
    ...
    public void waitTillReady() {
        final Iterator it = _list.iterator();
        while (it.hasNext()) {
            final OfficeProcess p = (OfficeProcess) it.next();
            final int t = size() * _config.toleratedStartupTimePerInstance;
