...
/**
 * An abstract layer that is useful for some system calls <br>
 * creation : 30 août 07
 *
...
public class OSAbstractLayer {
     ...
     * @throws IOException
     */
    public void kill(final String processName) throws IOException {
        switch (_os) {
        case OS_WINDOWS:
            final String[] cmd = { "tskill", processName };
        ...
        case OS_UNIXCOMPATIBLE:
            ...
            if (pid != null) {
                while (pid != null) {
                    final String[] killCmd = { "/bin/bash", "-c", "kill -9 " + pid };
     ...
     * @throws IOException
     */
    private String getProcessID(final String processName) throws IOException {
        switch (_os) {
        case OS_UNIXCOMPATIBLE:
            final Runtime runtime = Runtime.getRuntime();

            // Get process id
            final String[] getPidCmd = { "/bin/bash", "-c", "ps -e|grep " + processName + "|awk '{print $1}'" };
            final Process getPidProcess = runtime.exec(getPidCmd);

            // Read process id
            final InputStreamReader isr = new InputStreamReader(getPidProcess.getInputStream());
    ...
    public boolean isProcessAlive(final int pid) {
        final String nullDevice = "0";
        // Get process
        final String[] cmd = { "ps", "-p", "" + pid, ">", nullDevice };
    ...
    public static String concatPaths(final String path1, final String path2) {
        String ret;
        final char lastChar = path1.charAt(path1.length() - 1);
