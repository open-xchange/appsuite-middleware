    ...
    public void onAfterOAuthAccountDeletion(int oauthAccountId, Map<String, Object> eventProps, int user, int cid, Connection con) {
        try {
            ...
            for (FileStorageAccount deleteMe : toDelete) {
                ...
                LOG.info("Deleted Microsoft OneDrive file storage account with ID {} as OAuth account {} was deleted for user {} in context {}", deleteMe.getId(), oauthAccountId, user, cid);
                boolean purged = registry.purgeUserAccess(session.getContextId(), session.getUserId());
                if (purged) {
                    LOG.info("Removed Microsoft OneDrive OAuth accesses from registry for the deleted OAuth account with id '{}' for user '{}' in context '{}'", deleteMe.getId(), user, cid);
                }
            }
