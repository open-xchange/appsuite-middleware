    ...
    public void handleEvent(final Event event) {
        ...
        try {
            if (SessiondEventConstants.TOPIC_REMOVE_SESSION.equals(topic) || SessiondEventConstants.TOPIC_STORED_SESSION.equals(topic)) {
                ...
                if (!session.isTransient() && OneDriveAccessRegistry.getInstance().removeAccessIfLast(session.getContextId(), session.getUserId())) {
                    LOG.debug("Live Connect access removed for user {} in context {}", session.getUserId(), session.getContextId());
                }
