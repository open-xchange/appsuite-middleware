...
/**
 * {@link OneDriveAccessRegistry}
 *
 * @author <a href="mailto:thorben.betten@open-xchange.com">Thorben Betten</a>
 */
public final class OneDriveAccessRegistry {

    private static final OneDriveAccessRegistry INSTANCE = new OneDriveAccessRegistry();
     ...
     */
    public static OneDriveAccessRegistry getInstance() {
        return INSTANCE;
    }

    private final ConcurrentMap<SimpleKey, ConcurrentMap<String, OneDriveAccess>> map;

    /**
     * Initializes a new {@link OneDriveAccessRegistry}.
     */
    private OneDriveAccessRegistry() {
        super();
        map = new ConcurrentHashMap<SimpleKey, ConcurrentMap<String, OneDriveAccess>>();
    }
    ...
    public OneDriveAccess addAccess(final int contextId, final int userId, final String accountId, final OneDriveAccess boxAccess) {
        ...
        if (null == inner) {
            final ConcurrentMap<String, OneDriveAccess> tmp = new ConcurrentHashMap<String, OneDriveAccess>();
    ...
    public boolean removeAccessIfLast(final int contextId, final int userId) {
        ...
        if (null == sessiondService || null == sessiondService.getAnyActiveSessionForUser(userId, contextId)) {
             ...
             */
            final ConcurrentMap<String, OneDriveAccess> inner = map.remove(SimpleKey.valueOf(contextId, userId));
    ...
    public boolean purgeUserAccess(final int contextId, final int userId, final String accountId) {
        final SimpleKey key = SimpleKey.valueOf(contextId, userId);
        final ConcurrentMap<String, OneDriveAccess> inner = map.get(key);
        if (null == inner) {
            return false;
        }
        final OneDriveAccess oAuthInfo = inner.remove(accountId);
        if (null == oAuthInfo) {
            return false;
        }
        if (inner.isEmpty()) {
