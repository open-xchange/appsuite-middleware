...
@OXExceptionSource(
classId=Classes.COM_OPENEXCHANGE_ADMIN_DATASOURCE_OXUSER,
    ...
    category=Category.USER_INPUT,
            desc="The data sent by the client is invalid",
            exceptionId=0,
            msg="Invalid data sent-%s"
            )
        ...
        if( userData.containsKey(I_OXUser.ALIAS) &&
                ! ( userData.get(I_OXUser.ALIAS) instanceof String[] ) ) {
        ...
        if( userData.containsKey(I_OXUser.BIRTHDAY) &&
                ! ( userData.get(I_OXUser.BIRTHDAY) instanceof java.util.Date ) ) {
            throw USER_EXCEPTIONS.create(0,"BIRTHDAY parameter must be of type java.util.Date");
        }
        if( userData.containsKey(I_OXUser.ANNIVERSARY) &&
                ! ( userData.get(I_OXUser.ANNIVERSARY) instanceof java.util.Date ) ) {
        ...
        if ( prop.getUserProp( AdminProperties.User.CHECK_NOT_ALLOWED_CHARS, true ) ) {
            try{
                AdminDaemonTools.validateUserName( userData.get( I_OXUser.UID ).toString() );
        ...
        // check if I_OXUser.EMAIL1 is set and if yes, check if value is set in aliases[]
    ...
    @OXThrowsMultiple(
    category={Category.USER_INPUT,Category.USER_INPUT,Category.PROGRAMMING_ERROR,Category.PROGRAMMING_ERROR},
            desc={" ",
            "The user id sent by the client already exists in the system",
            "SQL query failed to check if the context/user exists","Error generating password for a new user"},
            exceptionId={1,2,3,4},
            msg={OXContextException.NO_SUCH_CONTEXT,OXUserException.USER_EXISTS,OXContext.MSG_SQL_OPERATION_ERROR,"Error generating password"}
    )
    ...
    @OXThrowsMultiple(
    category={Category.USER_INPUT,Category.USER_INPUT,Category.PROGRAMMING_ERROR},
            desc={" ",
            "The user id sent by the client does not exist in the system",
            " "},
            exceptionId={6,7,8},
            msg={OXContextException.NO_SUCH_CONTEXT+" %s",OXContext.MSG_NO_SUCH_USER_IN_CONTEXT,OXContext.MSG_SQL_OPERATION_ERROR}
    )
    ...
    category={Category.USER_INPUT,Category.PROGRAMMING_ERROR},
            desc={" ",
            "SQL query failed to get all users for a context!"},
            exceptionId={12,13},
            msg={OXContextException.NO_SUCH_CONTEXT+" %s",OXContext.MSG_SQL_OPERATION_ERROR}
    )
    ...
    public Vector changeUserData(int context_id, int user_id, Hashtable userData) throws RemoteException {
        ...
        try {
            ...
            if ( userData.containsKey(I_OXUser.UID) &&
                    prop.getGroupProp( AdminProperties.User.AUTO_LOWERCASE, true ) ) {
    ...
    category=Category.USER_INPUT,
        ...
        // MAIL ATTRIBUTE CHANGE NOT SUPPORTTED FOR 1und1 - DISABLED CAUSE OF MEETING 13.12.2006
//        if( userData.containsKey(I_OXUser.PRIMARY_MAIL)) {
//            throw USER_EXCEPTIONS.create(23,"Changing mail attribute not allowed");
//        }
...
//        }
    ...
    @OXThrowsMultiple(
    category={Category.USER_INPUT,Category.USER_INPUT,Category.PROGRAMMING_ERROR,Category.USER_INPUT},
            desc={" ",
            " ",
            " ",""},
            exceptionId={24,25,26,27},
            msg={OXContextException.NO_SUCH_CONTEXT+" %s",OXContext.MSG_NO_SUCH_USER_IN_CONTEXT,OXContext.MSG_SQL_OPERATION_ERROR,"%s"}
    )
    ...
    public static String getDisplayName(Hashtable<String, String> userData){
        ...
        if(userData.containsKey(I_OXUser.DISPLAY_NAME)){
            ...
            if(userData.containsKey(I_OXUser.GIVEN_NAME)){
                // SET THE DISPLAYNAME AS NEEDED BY CUSTOMER, SHOULD BE DEFINED ON SERVER SIDE
