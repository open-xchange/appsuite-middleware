...
@OXExceptionSource(classId = Classes.COM_OPENEXCHANGE_ADMIN_DATASOURCE_OXUTIL, component = Component.ADMIN_UTIL)
public class OXUtil extends BasicAuthenticator implements OXUtilInterface {

    // static UtilExceptionFactory UTIL_EXCEPTIONS = new UtilExceptionFactory
    // (OXUtil.class);
    ...
    // desc={" ","Invalid data sent by client","store already exists","Filestore
    // size to large for database (max=8796093022208)"},
    // exceptionId={0,1,2,54},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXUtilException.STORE_EXISTS+"
    // %s","Filestore error %s"}
    // )
    public int registerFilestore(final Filestore fstore, final Credentials auth) throws RemoteException, StorageException, InvalidCredentialsException, InvalidDataException {
        ...
        try {
            ...
            if (!f.exists()) {
                ...
                // throw UTIL_EXCEPTIONS.create (1,"No such directory: \""+
                // fstore.getUrl () +"\"");
            }
            if (!f.isDirectory()) {
                throw new InvalidDataException("No directory: \"" + fstore.getUrl() + "\"");
                // throw UTIL_EXCEPTIONS.create (1,"No directory: \""+
                // fstore.getUrl () +"\"");
            }
    ...
    // @OXThrowsMultiple(
    // category={Category.PROGRAMMING_ERROR,Category.USER_INPUT,Category.USER_INPUT,Category.USER_INPUT},
    // desc={" ","Invalid data sent by client","store does not exist","Filestore
    // size to large for database (max=8796093022208)"},
    // exceptionId={3,4,5,55},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXUtilException.NO_SUCH_STORE+"
    // %s","Filestore error %s"}
    // )
    public void changeFilestore(final Filestore fstore, final Credentials auth) throws RemoteException, StorageException, InvalidCredentialsException, InvalidDataException {
        ...
        if (!AdminDaemonTools.checkValidStoreURI(fstore.getUrl())) {
            ...
            // throw new StorageException(UTIL_EXCEPTIONS.create ( 4, "INVALID
            // STORE \""+ fstore.getUrl () +"\"" ));
    ...
    public Filestore[] listFilestores(final String search_pattern, final Credentials auth) throws RemoteException, StorageException, InvalidCredentialsException, InvalidDataException {
        ...
        if (search_pattern.trim().length() == 0) {
            ...
            // throw UTIL_EXCEPTIONS.create (7,"Invalid pattern
            // \""+search_pattern+"\"" );
        }
    ...
    // exceptionId={8,9,51},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXUtilException.NO_SUCH_STORE+"
    // %s",OXUtilException.STORE_IN_USE+" %s"}
    // )
    ...
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXUtilException.REASON_EXISTS+"
    // %s",OXContext.MSG_INVALID_DATA_SENT+"-%s"}
    // )
    ...
    // exceptionId={13,14},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXUtilException.NO_SUCH_REASON+"
    // %s"}
    // )
    ...
    // desc={"
    // ",OXContext.MSG_INVALID_DATA_SENT,OXContext.MSG_INTERNAL_ERROR,"db
    // already exists"},
    // exceptionId={24,25,26,27,28},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXContext.MSG_INTERNAL_ERROR,OXUtilException.DATABASE_EXISTS+"
    // %s","DB_CLUSTER_WEIGHT not within range (0-100): %s"}
    // )
    ...
    // desc={"
    // ",OXContext.MSG_INVALID_DATA_SENT,OXContext.MSG_INTERNAL_ERROR,"server
    // exists"},
    // exceptionId={29,30,31,32},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXContext.MSG_INTERNAL_ERROR,OXUtilException.SERVER_EXISTS+"
    // %s"}
    // )
    ...
    // category={Category.PROGRAMMING_ERROR,Category.USER_INPUT,Category.INTERNAL_ERROR,Category.USER_INPUT,Category.USER_INPUT},
    // desc={"
    // ",OXContext.MSG_INVALID_DATA_SENT,OXContext.MSG_INTERNAL_ERROR,"no such
    // db","pool in use"},
    // exceptionId={33,34,35,36,52},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXContext.MSG_INTERNAL_ERROR,OXUtilException.NO_SUCH_DATABASE+"
    // %s",OXUtilException.POOL_IN_USE+" %s"}
    // )
    ...
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXContext.MSG_INTERNAL_ERROR,OXUtilException.NO_SUCH_SERVER+"
    // %s",OXUtilException.SERVER_IN_USE+" %s"}
    // )
    ...
    // @OXThrowsMultiple (
    // category={Category.PROGRAMMING_ERROR,Category.USER_INPUT,Category.USER_INPUT,Category.USER_INPUT},
    // desc={" ",OXContext.MSG_INVALID_DATA_SENT,"no such db","db already
    // exists"},
    // exceptionId={47,48,49,50},
    // msg={OXContext.MSG_SQL_OPERATION_ERROR,OXContext.MSG_INVALID_DATA_SENT+"-%s",OXUtilException.NO_SUCH_DATABASE+"
    // %s",OXUtilException.DATABASE_EXISTS+" %s"}
    // )
