...
public final class CalendarRecurringCollection {
    ...
    public static RecurringResults calculateRecurring(final CalendarObject cdao, final long range_start, final long range_end, final int pos, final int PMAXTC, final boolean ignore_exceptions, final boolean calc_until, final int userId, final int cid) throws OXException {
        ...
        if (cdao instanceof CalendarDataObject) {
			final CalendarDataObject cdo = (CalendarDataObject) cdao;
			if (!ignore_exceptions) {
				change_exceptions = cdo.getExceptions();
				delete_exceptions = cdo.getDelExceptions();
			}
			if (cdo.getFullTime() != true) {
				calc_timezone = cdo.getTimezone();
			}
			recurrencePattern = cdo.getRecurrence();
		} else {
