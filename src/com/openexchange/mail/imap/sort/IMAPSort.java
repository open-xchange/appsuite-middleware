...
package com.openexchange.mail.imap.sort;
...
import com.openexchange.mail.imap.IMAPCommandsCollection;
import com.openexchange.mail.imap.IMAPException;
import com.openexchange.mail.imap.IMAPStorageUtils.DummyAddress;
...
public final class IMAPSort {
	...
	public static Message[] sortMessages(final IMAPFolder imapFolder, final Message[] filteredMessages,
			...
			final Locale locale) throws IMAPPropertyException, MessagingException {
		boolean applicationSort = true;
		Message[] msgs = null;
		final MailListField sortField = sortFieldArg == null ? MailListField.SENT_DATE : sortFieldArg;
			...
			try {
				if (filteredMessages == null) {
					...
					if (msgs.length < IMAPProperties.getMessageFetchLimit()) {
						msgs = new FetchIMAPCommand(imapFolder, msgs, IMAPUtils.getDefaultFetchProfile(), true, true)
								.doCommand();
					} else {
						msgs = new FetchIMAPCommand(imapFolder, msgs, getFetchProfile(fields, sortField), true, true)
								.doCommand();
					}
				} else {
					/*
		...
		if (applicationSort) {
			...
			if (filteredMessages == null) {
				...
				if (allMsgCount < IMAPProperties.getMessageFetchLimit()) {
					msgs = new FetchIMAPCommand(imapFolder, IMAPUtils.getDefaultFetchProfile(), allMsgCount)
							.doCommand();
				} else {
					/*
					 * No messages were fetched. At least, prefetch according to
					 * given columns and sort criteria
					 */
					msgs = new FetchIMAPCommand(imapFolder, getFetchProfile(fields, sortField), allMsgCount)
							.doCommand();
				}
			} else {
