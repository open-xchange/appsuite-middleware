...
/**
 * This class models database indexes (in mysql).An index operates on a table, has a name and a list of
 * columns it indexes. You can use the find-methods to load indexes from the database. Use the instance methods
 * to drop or create an index.
 *
...
public class Index {
	...
	/**
	 * Tries to find all indexes on a table that contain a certain column, no matter on which posistion, and no matter
	 * if other indexes are present. 
	 * 
	 * Consider a table with indexes on cid, id and cid, field01 and cid  and last_modified. When you use this
	 * method to search for indexes for the field cid, then the first three indexes (all except the one on last_modified)
	...
	/**
	 * Tries to find all indexes on a table that match a certain column signature, 
	 * 
	 * Consider a table with indexes on cid, id and cid, field01 and cid  and last_modified. When you use this
	 * method to search for indexes for the fields cid, id, then the first index (containing exactly cid, id in that order)
