 ...
 * @since v7.10.0
 */
public class RdbEventStorage extends CalendarStorageWarnings implements EventStorage {
    ...
    public void insertEvent(Event event) throws OXException {
        Failsafe
            .with(new RetryPolicy().withMaxRetries(MAX_RETRIES).retryOn(f -> handle(event, f)))
            .run(() -> delegate.insertEvent(event)
        );
    }
    ...
    public void insertEvents(List<Event> events) throws OXException {
        Failsafe
            .with(new RetryPolicy().withMaxRetries(MAX_RETRIES).retryOn(f -> handle(events, f)))
            .run(() -> delegate.insertEvents(events)
        );
    }
    ...
    public void updateEvent(Event event) throws OXException {
        Failsafe
            .with(new RetryPolicy().withMaxRetries(MAX_RETRIES).retryOn(f -> handle(event, f)))
            .run(() -> delegate.updateEvent(event)
        );
    }
    ...
    public void insertEventTombstone(Event event) throws OXException {
        Failsafe
            .with(new RetryPolicy().withMaxRetries(MAX_RETRIES).retryOn(f -> handle(event, f)))
            .run(() -> delegate.insertEventTombstone(event)
        );
    }
    ...
    public void insertEventTombstones(List<Event> events) throws OXException {
        Failsafe
            .with(new RetryPolicy().withMaxRetries(MAX_RETRIES).retryOn(f -> handle(events, f)))
            .run(() -> delegate.insertEventTombstones(events)
        );
    }
