    ...
    private static void fold(final List<Conversation> toFold, Map<String, Conversation> lookupTable) {
        for (Iterator<Conversation> iter = toFold.iterator(); iter.hasNext();) {
            ...
            for (String messageId : conversation.getMessageIds()) {
                Conversation existing = lookupTable.get(messageId);
                if (null != existing) {
                    if (!conversation.equals(existing)) {
                        if (!removed) {
                            iter.remove();
                            removed = true;
                        }
                    ...
                    lookupTable.put(messageId, conversation);
                }
            }

            for (String reference : conversation.getReferences()) {
                Conversation existing = lookupTable.get(reference);
                if (null != existing) {
                    if (!conversation.equals(existing)) {
                        if (!removed) {
                            iter.remove();
                    ...
                    lookupTable.put(reference, conversation);
                }
            }
        }
    }
