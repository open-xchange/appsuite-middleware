        ...
        if (breakers != null) {
            for (AbstractFailsafeCircuitBreakerCommandExecutor breaker : breakers) {
                try {
                    ...
                    LoggerHolder.LOG.warn("Failed to apply metric service to circuit breaker: {}", breaker.getDescription(), e);
                }
            }
        }
        return service;
        ...
        if (breakers != null) {
            for (AbstractFailsafeCircuitBreakerCommandExecutor breaker : breakers) {
                try {
                    ...
                    LoggerHolder.LOG.warn("Failed to remove metric service from circuit breaker: {}", breaker.getDescription(), e);
                }
            }
        }
    }
