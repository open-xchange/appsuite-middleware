...
public class OXContext extends OXContextCommonImpl implements OXContextInterface {
    ...
    @Override
    public void changeModuleAccess(Context ctx, UserModuleAccess access, boolean value, Credentials credentials) throws RemoteException, InvalidCredentialsException, NoSuchContextException, StorageException, InvalidDataException {
        final Credentials auth = credentials == null ? new Credentials("", "") : credentials;
        ...
        try {
            ...
            // Trigger plug-in extensions
            {
                // Mimic old behavior for plug-in invocations that is to take context administrator's module accesses as template
                UserModuleAccess moduleAccess = oxu.getModuleAccess(ctx, tool.getAdminForContext(ctx));
                merge(access, value, moduleAccess);

                final PluginInterfaces pluginInterfaces = PluginInterfaces.getInstance();
                if (null != pluginInterfaces) {
                    for (final OXContextPluginInterface oxContextPlugin : pluginInterfaces.getContextPlugins().getServiceList()) {
                        oxContextPlugin.changeModuleAccess(ctx, moduleAccess, auth);
                    }
                }
            }
            ...
            if (access.isPublicFolderEditable()) {
                ...
                oxu.changeModuleAccess(ctx, new int[] { adminId }, access, value);
                access.setPublicFolderEditable(false);
                oxu.changeModuleAccess(ctx, I2i(userIds), access, value);
            } else {
                oxu.changeModuleAccess(ctx, oxu.getAll(ctx), access, value);
            }
        } catch (final StorageException e) {
            LOGGER.error("", e);
            throw e;
    ...
    private void merge(UserModuleAccess access, boolean value, UserModuleAccess moduleAccess) {
        if (access.getCalendar()) {
            moduleAccess.setCalendar(value);
        }
        if (access.getContacts()) {
            moduleAccess.setContacts(value);
        }
        if (access.getDelegateTask()) {
            moduleAccess.setDelegateTask(value);
        }
        if (access.getEditGroup()) {
            moduleAccess.setEditGroup(value);
        }
        if (access.getEditPassword()) {
            moduleAccess.setEditPassword(value);
        }
        if (access.getEditPublicFolders()) {
            moduleAccess.setEditPublicFolders(value);
        }
        if (access.getEditResource()) {
            moduleAccess.setEditResource(value);
        }
        if (access.getIcal()) {
            moduleAccess.setIcal(value);
        }
        if (access.getInfostore()) {
            moduleAccess.setInfostore(value);
        }
        if (access.getReadCreateSharedFolders()) {
            moduleAccess.setReadCreateSharedFolders(value);
        }
        if (access.getSyncml()) {
            moduleAccess.setSyncml(value);
        }
        if (access.getTasks()) {
            moduleAccess.setTasks(value);
        }
        if (access.getVcard()) {
            moduleAccess.setVcard(value);
        }
        if (access.getWebdav()) {
            moduleAccess.setWebdav(value);
        }
        if (access.getWebdavXml()) {
            moduleAccess.setWebdavXml(value);
        }
        if (access.getWebmail()) {
            moduleAccess.setWebmail(value);
        }
        if (access.isActiveSync()) {
            moduleAccess.setActiveSync(value);
        }
        if (access.isCollectEmailAddresses()) {
            moduleAccess.setCollectEmailAddresses(value);
        }
        if (access.isDeniedPortal()) {
            moduleAccess.setDeniedPortal(value);
        }
        if (access.isGlobalAddressBookDisabled()) {
            moduleAccess.setGlobalAddressBookDisabled(value);
        }
        if (access.isMultipleMailAccounts()) {
            moduleAccess.setMultipleMailAccounts(value);
        }
        if (access.isOLOX20()) {
            moduleAccess.setOLOX20(value);
        }
        if (access.isPublication()) {
            moduleAccess.setPublication(value);
        }
        if (access.isPublicFolderEditable()) {
            moduleAccess.setPublicFolderEditable(value);
        }
        if (access.isSubscription()) {
            moduleAccess.setSubscription(value);
        }
        if (access.isUSM()) {
            moduleAccess.setUSM(value);
        }
    }
