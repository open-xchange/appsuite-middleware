...
import com.openexchange.ajp13.xajp.http.XAJPv13ServletOutputStream;
...
public final class XAJPv13ForwardRequest extends XAJPv13Request {
    ...
    private static final org.apache.commons.logging.Log LOG = org.apache.commons.logging.LogFactory.getLog(XAJPv13ForwardRequest.class);
    ...
    public void processRequest(final XAJPv13Session session) throws AJPv13Exception, IOException {
         ...
         */
        final HttpServletRequestWrapper servletRequest = new HttpServletRequestWrapper(session);
        servletRequest.setInputStream(new XAJPv13ServletInputStream(connection));
        /*
        ...
        parseRequestHeaders(servletRequest, numHeaders);
        {
            ...
            if (contentLength == -1) {
                session.setContentLength(XAJPv13Session.NOT_SET);
            } else {
        ...
        if (jsessionID == null) {
             ...
             */
            checkJSessionIDCookie(servletRequest, servletResponse, session);
        } else {
            final int dot = jsessionID.lastIndexOf('.');
            if ((dot == -1) || (AJPv13Config.getJvmRoute().equals(jsessionID.substring(dot + 1)))) {
                addJSessionIDCookie(jsessionID, servletResponse, session);
            } else {
                /*
                 * JVM route does not match
                 */
                createJSessionIDCookie(servletResponse, session);
            }
        }
        /*
         * Apply request/response to AJP request handler
         */
        session.setServletRequest(servletRequest);
        session.setServletResponse(servletResponse);
        /*
         * Create servlet instance dependent on requested URI
         */
        session.setServletInstance(requestURI);
        if (null != session.getServletPath()) {
            /*
             * Apply the servlet path with leading "/" to the request
             */
            if (allPath(session.getServletPath())) {
                /*
                 ...
                 */
                servletRequest.setServletPath(new StringBuilder().append('/').append(session.getServletPath()).toString());
            }
        }
    }
    ...
    private static void checkJSessionIDCookie(final HttpServletRequestWrapper servletRequest, final HttpServletResponseWrapper resp, final XAJPv13Session session) {
            ...
            NextCookie: for (int i = 0; (i < cookies.length) && (jsessionIDCookie == null); i++) {
                ...
                if (AJPv13RequestHandler.JSESSIONID_COOKIE.equals(current.getName())) {
                    ...
                    if (pos > -1) {
                        ...
                        session.setHttpSessionId(id, true);
                    } else {
                        /*
        ...
        if (jsessionIDCookie == null) {
            createJSessionIDCookie(resp, session);
        }
    }

    private static void createJSessionIDCookie(final HttpServletResponseWrapper resp, final XAJPv13Session session) {
        addJSessionIDCookie(null, resp, session);
    }

    private static final String DEFAULT_PATH = "/";

    private static void addJSessionIDCookie(final String id, final HttpServletResponseWrapper resp, final XAJPv13Session session) {
        ...
        session.setHttpSessionId(jsessionIdVal, join);
