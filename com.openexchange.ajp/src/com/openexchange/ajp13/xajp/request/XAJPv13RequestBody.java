...
public final class XAJPv13RequestBody extends XAJPv13Request {
    ...
    @Override
    public void processRequest(final XAJPv13Session session) throws AJPv13Exception, IOException {
        if (isPayloadNull()) {
        ...
        if (getPayloadLength() == 0 || (chunkContentLength = parseInt()) == 0) {
            ...
            if (session.isMoreDataExpected()) {
                ...
                if (LOG.isWarnEnabled()) {
                    final AJPv13Exception ajpExc = new AJPv13Exception(
                        ...
                        true,
                        Long.valueOf(session.getTotalRequestedContentLength()),
                        Long.valueOf(session.getContentLength()),
                        /* TODO: ajpRequestHandler.getForwardRequest() */"");
             ...
             * been set
             */
            if (session.isNotSet() || session.isMoreDataReadThanExpected()) {
                session.makeEqual();
            }
            session.setData(null);
            return;
    ...
    private static final org.apache.commons.logging.Log LOG = com.openexchange.exception.Log.valueOf(org.apache.commons.logging.LogFactory.getLog(XAJPv13RequestBody.class));
