...
public class PushConfigInterfaceImpl extends AbstractConfigWrapper implements PushConfigInterface {
	...
	public PushConfigInterfaceImpl(final ConfigurationService conf, final boolean ignoreIsInit) {
		...
		isPushEnabled = parseProperty(conf,
				"com.openexchange.push.udp.pushEnabled",
				isPushEnabled);
		if (LOG.isDebugEnabled()) {
			LOG
					.debug("PushHandler property: com.openexchange.push.udp.pushEnabled="
							+ isPushEnabled);
		}
		...
		isRegisterDistributionEnabled = parseProperty(conf, "com.openexchange.push.udp.registerDistributionEnabled", isRegisterDistributionEnabled);
		if (LOG.isDebugEnabled()) {
