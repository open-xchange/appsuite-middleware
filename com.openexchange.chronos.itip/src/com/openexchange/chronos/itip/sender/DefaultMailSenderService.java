...
public class DefaultMailSenderService implements MailSenderService {
    ...
    private void prepareAndSend(NotificationMail mail, Session session, State.Type type, Event event, boolean addAttachments) throws MessagingException, OXException {
        ...
        message.setMessageId(generateHeaderValue(session, event.getUid(), true));
        message.setInReplyTo(generateHeaderValue(session, event.getUid(), false));
        message.setReference(generateHeaderValue(session, event.getUid(), false));
    ...
    private String generateHeaderValue(Session session, String uid, boolean timestamp) throws OXException {
        HostnameService hostnameService = Services.getService(HostnameService.class, true);
        String hostname = hostnameService.getHostname(session.getUserId(), session.getContextId());
