...
public class HazelcastUpgradeActivator extends HousekeepingActivator {
    ...
    private static final Version LEGACY_HAZELCAST_VERSION = new Version(3, 12, 6);

    /** The applied port offset suitable for the underlying 'legacy' Hazelcast library if automatic offset is enabled */
    private static final int AUTO_PORT_OFFSET = 0; // no offset for Hazelcast 3.12.6
    ...
    private List<ClientConfig> getConfigs(ConfigurationService configService) throws OXException {
        ...
        int port;
        String portValue = configService.getProperty("com.openexchange.hazelcast.network.client.port", "auto");
        if ("auto".equalsIgnoreCase(portValue)) {
            port = configService.getIntProperty("com.openexchange.hazelcast.network.port", 5701) + AUTO_PORT_OFFSET;
        } else {
            try {
                port = Integer.parseInt(portValue);
            } catch (NumberFormatException e) {
                throw ConfigurationExceptionCodes.INVALID_CONFIGURATION.create(e, "com.openexchange.hazelcast.network.client.port");
            }
        }
            ...
            if (null != members && 0 < members.length) {
                for (String member : members) {
                    if (Strings.isNotEmpty(member)) {
                        ...
                        config.getNetworkConfig().addAddress(hostAddress + ":" + port);
                        alternativeConfig.getNetworkConfig().addAddress(hostAddress + ":" + port);
                    }
                }
            }
        }
        ...
        alternativeConfig.getSerializationConfig().addPortableFactory(DynamicPortableFactory.FACTORY_ID, dynamicPortableFactory);
