...
public class AttendeeMapping extends AbstractICalMapping<VEvent, Event> {
	...
	private Attendee importAttendee(net.fortuna.ical4j.model.property.Attendee property) {
		...
		attendee.setCommonName(null != cnParameter ? cnParameter.getValue() : null);
		Parameter sentByParameter = property.getParameter(Parameter.SENT_BY);
		attendee.setSentBy(null != sentByParameter ? sentByParameter.getValue() : null);
		Parameter partstatParameter = property.getParameter(Parameter.PARTSTAT);
		attendee.setPartStat(null != partstatParameter ? Enums.parse(ParticipationStatus.class, partstatParameter.getValue(), null) : null);
		Parameter roleParameter = property.getParameter(Parameter.ROLE);
		attendee.setRole(null != roleParameter ? Enums.parse(ParticipantRole.class, roleParameter.getValue(), null) : null);
		Parameter cutypeParameter = property.getParameter(Parameter.CUTYPE);
		attendee.setCuType(null != cutypeParameter ? Enums.parse(CalendarUserType.class, cutypeParameter.getValue(), null) : null);
		Parameter rsvpParameter = property.getParameter(Parameter.RSVP);
		attendee.setRsvp(null != rsvpParameter ? Boolean.valueOf(rsvpParameter.getValue()) : null);
		return attendee;
	}
