...
public class TimeZoneUtils {
    ...
    public static TimeZone selectTimeZone(DateProperty property, ICalParameters parameters) {
        return selectTimeZone(property, parameters.get(ICalParameters.DEFAULT_TIMEZONE, TimeZone.class));
    }

    public static TimeZone selectTimeZone(DateProperty property, TimeZone defaultTimeZone) {
        if (property.isUtc()) {
            return TimeZones.UTC;
        }
        net.fortuna.ical4j.model.TimeZone parsedTimeZone = property.getTimeZone();
        if (null != parsedTimeZone) {
            return selectTimeZone(parsedTimeZone.getID(), TimeZone.getAvailableIDs(parsedTimeZone.getRawOffset()), defaultTimeZone);
        }
        ...
        if (null != tzidParameter && Strings.isNotEmpty(tzidParameter.getValue())) {
            return selectTimeZone(tzidParameter.getValue(), TimeZone.getAvailableIDs(), defaultTimeZone);
        }
