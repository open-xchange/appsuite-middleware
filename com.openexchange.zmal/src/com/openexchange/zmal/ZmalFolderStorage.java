...
public final class ZmalFolderStorage extends MailFolderStorage implements IMailFolderStorageEnhanced2 {
        ...
        try {
            ...
            synchronized (f) {
                if (!doesExist(entry)) {
                    ...
                    if (null == f) {
                        throw IMAPException.create(IMAPException.Code.FOLDER_NOT_FOUND, zmalConfig, session, fullName);
                    }
                    return new int[] { 0, 0 };
                }
                if (!entry.canOpen()) {
            ...
            throw MimeMailException.handleMessagingException(e, zmalConfig, session);
        ...
        try {
            if (DEFAULT_FOLDER_ID.equals(parentFullName)) {
                ...
                synchronized (parent) {
                    ...
                    if (zmalConfig.getImapCapabilities().hasNamespace()) {
                        /*
            ...
            if (doesExist(parentEntry)) {
                 ...
                 * Holds LOOK-UP right?
                 */
                if (zmalConfig.isSupportsACLs() && parentEntry.canOpen()) {
                    try {
                        if (!zmalConfig.getACLExtension().canLookUp(RightsCache.getCachedRights(parent, true, session, accountId))) {
                            throw IMAPException.create(IMAPException.Code.NO_LOOKUP_ACCESS, zmalConfig, session, parentFullName);
                        }
                    } catch (final MessagingException e) {
                        if (!startsWithNamespaceFolder(parentFullName, parentEntry.getSeparator())) {
                            throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, parentFullName);
                        }
                    }
                }
    ...
    public String createFolder(final MailFolderDescription toCreate) throws OXException {
        ...
        try {
            ...
            if (getIMAPFolder(fullName).exists()) {
                throw IMAPException.create(IMAPException.Code.DUPLICATE_FOLDER, zmalConfig, session, fullName);
            }
        } catch (final MessagingException e) {
            // Ignore for now
        }
        ...
        try {
            ...
            synchronized (parent) {
                if (!parent.exists()) {
                    parent = checkForNamespaceFolder(parentFullname);
                    if (null == parent) {
                        throw IMAPException.create(IMAPException.Code.FOLDER_NOT_FOUND, zmalConfig, session, parentFullname);
                    }
                }
                /*
                 ...
                 */
                if (zmalConfig.isSupportsACLs()) {
                    try {
                        if (isParentDefault) {
                            if (!(RootSubfolderCache.canCreateSubfolders((DefaultFolder) parent, true, session, accountId).booleanValue())) {
                                throw IMAPException.create(IMAPException.Code.NO_CREATE_ACCESS, zmalConfig, session, DEFAULT_FOLDER_ID);
                            }
                        } else {
                            if (!zmalConfig.getACLExtension().canCreate(RightsCache.getCachedRights(parent, true, session, accountId))) {
                                throw IMAPException.create(IMAPException.Code.NO_CREATE_ACCESS, zmalConfig, session, parentFullname);
                            }
                        }
                    } catch (final MessagingException e) {
                        /*
                         ...
                         */
                        if (!zmalConfig.getImapCapabilities().hasNamespace() || !NamespaceFoldersCache.containedInPersonalNamespaces(
                            parentFullname,
                            imapStore,
                            true,
                            session,
                ...
                final boolean mboxEnabled =
                    MBoxEnabledCache.isMBoxEnabled(zmalConfig, parent, new StringBuilder(parent.getFullName()).append(separator).toString());
                if (!checkFolderNameValidity(name, separator, mboxEnabled)) {
                    throw IMAPException.create(IMAPException.Code.INVALID_FOLDER_NAME, zmalConfig, session, Character.valueOf(separator));
                }
                if (isParentDefault) {
                    /*
                ...
                synchronized (createMe) {
                    if (createMe.exists()) {
                        throw IMAPException.create(IMAPException.Code.DUPLICATE_FOLDER, zmalConfig, session, createMe.getFullName());
                    }
                    final int ftype;
                    if (mboxEnabled) {
                        /*
                     ...
                     */
                    if (zmalConfig.isSupportsACLs() && toCreate.containsPermissions()) {
                        ...
                        if (initialACLs != null) {
                            ...
                            final Entity2ACLArgs args = IMAPFolderConverter.getEntity2AclArgs(session, createMe, zmalConfig);
                            final Map<String, ACL> m = acl2map(newACLs);
                            if (!equals(initialACLs, m, entity2ACL, args)) {
                                final ACLExtension aclExtension = zmalConfig.getACLExtension();
                                if (aclExtension.canSetACL(createMe.myRights())) {
                                     ...
                                     * Add a warning
                                     */
                                    zmalAccess.addWarnings(Collections.<OXException> singletonList(IMAPException.create(
    ...
    private Entity2ACL getEntity2ACL() throws OXException {
        return Entity2ACL.getInstance(imapStore, zmalConfig);
    }
    ...
    public String renameFolder(final String fullName, final String newName) throws OXException {
        if (DEFAULT_FOLDER_ID.equals(fullName)) {
            ...
            throw IMAPException.create(IMAPException.Code.NO_RENAME_ACCESS, zmalConfig, session, name);
        }
        try {
            ...
            synchronized (renameMe) {
                if (zmalConfig.isSupportsACLs() && ((renameMe.getType() & Folder.HOLDS_MESSAGES) > 0)) {
                    try {
                        if (!zmalConfig.getACLExtension().canCreate(RightsCache.getCachedRights(renameMe, true, session, accountId))) {
                            throw IMAPException.create(IMAPException.Code.NO_RENAME_ACCESS, zmalConfig, session, fullName);
                        }
                    } catch (final MessagingException e) {
                        /*
                         * MYRIGHTS command failed for given mailbox
                         */
                        throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, fullName);
                    }
                }
                if (getChecker().isDefaultFolder(fullName)) {
                    throw IMAPException.create(IMAPException.Code.NO_DEFAULT_FOLDER_UPDATE, zmalConfig, session, renameMe.getFullName());
                }
                /*
                 * Notify message storage about outstanding rename
                 */
                zmalAccess.getMessageStorage().notifyIMAPFolderModification(fullName);
                ...
                final IMAPFolder renameFolder;
                {
                     ...
                     */
                    mboxEnabled =
                        MBoxEnabledCache.isMBoxEnabled(zmalConfig, par, new StringBuilder(par.getFullName()).append(separator).toString());
                }
                if (doesExist(renameFolder, false)) {
                    throw IMAPException.create(IMAPException.Code.DUPLICATE_FOLDER, zmalConfig, session, renameFolder.getFullName());
                }
                 ...
                 */
                if (!success) {
                    throw IMAPException.create(IMAPException.Code.RENAME_FAILED, zmalConfig, session, renameMe.getFullName(), newFullName, "<not-available>");
                }
    ...
    public String moveFolder(final String fullName, final String newFullname) throws OXException {
        if (DEFAULT_FOLDER_ID.equals(fullName) || DEFAULT_FOLDER_ID.equals(newFullname)) {
            throw IMAPException.create(IMAPException.Code.NO_ROOT_MOVE, zmalConfig, session, new Object[0]);
        }
        try {
            if (DEFAULT_FOLDER_ID.equals(fullName)) {
            ...
            synchronized (moveMe) {
                ...
                if (move) {
                    ...
                    if (getChecker().isDefaultFolder(oldFullname)) {
                        throw IMAPException.create(IMAPException.Code.NO_DEFAULT_FOLDER_UPDATE, zmalConfig, session, oldFullname);
                    }
                    ...
                    synchronized (destFolder) {
                        ...
                        if (zmalConfig.isSupportsACLs() && ((destFolder.getType() & Folder.HOLDS_MESSAGES) > 0)) {
                            try {
                                if (isDestRoot) {
                                    ...
                                    if (!zmalConfig.getACLExtension().canCreate(
                ...
                if (rename) {
                    ...
                    if (getChecker().isDefaultFolder(moveMe.getFullName())) {
                        throw IMAPException.create(IMAPException.Code.NO_DEFAULT_FOLDER_UPDATE, zmalConfig, session, moveMe.getFullName());
                    } else if (zmalConfig.isSupportsACLs() && ((moveMe.getType() & Folder.HOLDS_MESSAGES) > 0)) {
                        try {
                            if (!zmalConfig.getACLExtension().canCreate(RightsCache.getCachedRights(moveMe, true, session, accountId))) {
                                throw IMAPException.create(IMAPException.Code.NO_CREATE_ACCESS, zmalConfig, session, moveMe.getFullName());
                            }
                        } catch (final MessagingException e) {
                            throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, moveMe.getFullName());
                        }
                    }
                    /*
                     * Rename can only be invoked on a closed folder
                     ...
                     */
                    if (!success) {
                        throw IMAPException.create(IMAPException.Code.RENAME_FAILED, zmalConfig, session, moveMe.getFullName(), newFullName, "<not-available>");
                    }
        ...
        try {
            ...
            synchronized (updateMe) {
                ...
                if (performSubscription && defaultFolder && !toUpdate.isSubscribed()) {
                    throw IMAPException.create(IMAPException.Code.NO_DEFAULT_FOLDER_UNSUBSCRIBE, zmalConfig, session, fullName);
                }
                /*
                 ...
                 * Proceed update
                 */
                if (zmalConfig.isSupportsACLs() && toUpdate.containsPermissions()) {
                    final ACL[] oldACLs = getACLSafe(updateMe);
                    if (oldACLs != null) {
                        final ACL[] newACLs = permissions2ACL(toUpdate.getPermissions(), updateMe);
                        final Entity2ACL entity2ACL = getEntity2ACL();
                        final Entity2ACLArgs args = IMAPFolderConverter.getEntity2AclArgs(session, updateMe, zmalConfig);
                        final Map<String, ACL> m = acl2map(newACLs);
                        if (!equals(oldACLs, m, entity2ACL, args)) {
                             ...
                             */
                            if (newACLs.length == 0) {
                                throw IMAPException.create(IMAPException.Code.NO_ADMIN_ACL, zmalConfig, session, updateMe.getFullName());
                            }
                            {
                                boolean adminFound = false;
                                for (int i = 0; (i < newACLs.length) && !adminFound; i++) {
                ...
                if (/* !MailProperties.getInstance().isIgnoreSubscription() && */performSubscription) {
                    ...
                    if (zmalConfig.isSupportsACLs()) {
                        if ((updateMe.getType() & Folder.HOLDS_MESSAGES) > 0) {
                            try {
                                if (!zmalConfig.getACLExtension().canLookUp(RightsCache.getCachedRights(updateMe, true, session, accountId))) {
                                    throw IMAPException.create(IMAPException.Code.NO_LOOKUP_ACCESS, zmalConfig, session, fullName);
                                }
                            } catch (final MessagingException e) {
                                throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, fullName);
                            }
                        } else {
                            throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, fullName);
                        }
                    }
            ...
            synchronized (deleteMe) {
                ...
                if (hardDelete) {
                    ...
                    if (isSubfolderOf(deleteMe.getParent().getFullName(), trashFullName, getSeparator()) || !inferiors(trashFolder)) {
                         ...
                         */
                        zmalAccess.getMessageStorage().notifyIMAPFolderModification(trashFolder.getFullName());
    ...
    public void clearFolder(final String fullName, final boolean hardDelete) throws OXException {
        try {
            ...
            synchronized (f) {
                ...
                try {
                    if (!isSelectable(f)) {
                        throw IMAPException.create(IMAPException.Code.FOLDER_DOES_NOT_HOLD_MESSAGES, zmalConfig, session, f.getFullName());
                    }
                    if (zmalConfig.isSupportsACLs()) {
                        final Rights myrights = RightsCache.getCachedRights(f, true, session, accountId);
                        if (!zmalConfig.getACLExtension().canRead(myrights)) {
                            throw IMAPException.create(IMAPException.Code.NO_READ_ACCESS, zmalConfig, session, f.getFullName());
                        }
                        if (!zmalConfig.getACLExtension().canDeleteMessages(myrights)) {
                            throw IMAPException.create(IMAPException.Code.NO_DELETE_ACCESS, zmalConfig, session, f.getFullName());
                        }
                    }
                } catch (final MessagingException e) {
                    throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, f.getFullName());
                }
                /*
                ...
                try {
                    ...
                    if (backup) {
                        zmalAccess.getMessageStorage().notifyIMAPFolderModification(trashFullname);
                    }
                    final StringBuilder debug = DEBUG ? new StringBuilder(128) : null;
                    final int blockSize = zmalConfig.getZmalProperties().getBlockSize();
                    ...
                    if (backup) {
                        try {
                            ...
                            throw IMAPException.create(IMAPException.Code.MOVE_ON_DELETE_FAILED, zmalConfig, session, e, new Object[0]);
                        }
                    }
                    /*
                     * Delete through storing \Deleted flag...
    ...
    public MailFolder[] getPath2DefaultFolder(final String fullName) throws OXException {
        try {
             ...
             * Obtain folder lock once to avoid multiple acquire/releases when invoking folder's getXXX() methods
             */
            if (zmalConfig.isSupportsACLs() && isSelectable(f)) {
                try {
                    if (!zmalConfig.getACLExtension().canLookUp(RightsCache.getCachedRights(f, true, session, accountId))) {
                        throw IMAPException.create(IMAPException.Code.NO_LOOKUP_ACCESS, zmalConfig, session, fullName);
                    }
                } catch (final MessagingException e) {
                    throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, fullName);
    ...
    public String getDefaultFolderPrefix() throws OXException {
        try {
             ...
             */
            {
                final String server = zmalConfig.getServer().toLowerCase(Locale.US);
    ...
    public com.openexchange.mail.Quota[] getQuotas(final String folder, final com.openexchange.mail.Quota.Type[] types) throws OXException {
        try {
            ...
            synchronized (f) {
                ...
                try {
                    if (!isSelectable(f)) {
                        throw IMAPException.create(IMAPException.Code.FOLDER_DOES_NOT_HOLD_MESSAGES, zmalConfig, session, fullName);
                    }
                    if (zmalConfig.isSupportsACLs()) {
                        final Rights myrights = RightsCache.getCachedRights(f, true, session, accountId);
                        if (!zmalConfig.getACLExtension().canRead(myrights)) {
                            throw IMAPException.create(IMAPException.Code.NO_READ_ACCESS, zmalConfig, session, fullName);
                        }
                        /*-
                ...
                if (!zmalConfig.getImapCapabilities().hasQuota()) {
    ...
    private void deleteFolder(final IMAPFolder deleteMe) throws OXException, MessagingException {
        ...
        if (getChecker().isDefaultFolder(fullName)) {
            throw IMAPException.create(IMAPException.Code.NO_DEFAULT_FOLDER_DELETE, zmalConfig, session, fullName);
        } else if (!doesExist(deleteMe, false)) {
            throw IMAPException.create(IMAPException.Code.FOLDER_NOT_FOUND, zmalConfig, session, fullName);
        }
        try {
            if (zmalConfig.isSupportsACLs() && ((deleteMe.getType() & Folder.HOLDS_MESSAGES) > 0) && !zmalConfig.getACLExtension().canDeleteMailbox(
                RightsCache.getCachedRights(deleteMe, true, session, accountId))) {
                throw IMAPException.create(IMAPException.Code.NO_CREATE_ACCESS, zmalConfig, session, fullName);
            }
        } catch (final MessagingException e) {
            throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, fullName);
        }
        ...
        if (!deleteMe.delete(true)) {
            throw IMAPException.create(IMAPException.Code.DELETE_FAILED, zmalConfig, session, fullName);
        }
        mailInterfaceMonitor.addUseTime(System.currentTimeMillis() - start);
        /*
    ...
    private boolean stillHoldsFullRights(final IMAPFolder defaultFolder, final ACL[] newACLs, final ACLExtension aclExtension) throws OXException {
        ...
        final String ownerACLName =
            getEntity2ACL().getACLName(session.getUserId(), ctx, IMAPFolderConverter.getEntity2AclArgs(session, defaultFolder, zmalConfig));
    ...
    private IMAPFolder moveFolder(final IMAPFolder toMove, final IMAPFolder destFolder, final IMAPFolder newFolder, final boolean checkForDuplicate) throws MessagingException, OXException {
        final String destFullName = destFolder.getFullName();
        if ((destFolder.getType() & Folder.HOLDS_FOLDERS) == 0) {
            throw IMAPException.create(IMAPException.Code.FOLDER_DOES_NOT_HOLD_FOLDERS, zmalConfig, session, destFullName);
        }
        final String moveFullname = toMove.getFullName();
        final int toMoveType = toMove.getType();
        if (zmalConfig.isSupportsACLs() && ((toMoveType & Folder.HOLDS_MESSAGES) > 0)) {
            try {
                if (!zmalConfig.getACLExtension().canRead(RightsCache.getCachedRights(toMove, true, session, accountId))) {
                    throw IMAPException.create(IMAPException.Code.NO_READ_ACCESS, zmalConfig, session, moveFullname);
                }
            } catch (final MessagingException e) {
                throw IMAPException.create(IMAPException.Code.NO_ACCESS, zmalConfig, session, e, moveFullname);
            }
            try {
                if (!zmalConfig.getACLExtension().canCreate(RightsCache.getCachedRights(toMove, true, session, accountId))) {
                    throw IMAPException.create(IMAPException.Code.NO_CREATE_ACCESS, zmalConfig, session, moveFullname);
                }
            } catch (final MessagingException e) {
                /*
                 * MYRIGHTS command failed for given mailbox
        ...
        if (checkForDuplicate && newFolder.exists()) {
            throw IMAPException.create(IMAPException.Code.DUPLICATE_FOLDER, zmalConfig, session, getNameOf(newFolder));
        }
        /*
        ...
        if ((toMoveType & Folder.HOLDS_MESSAGES) > 0) {
            ...
            zmalAccess.getMessageStorage().copyMessagesLong(moveFullname, newFullname, uids, true);
        }
        /*
         * Iterate subfolders
         */
         ...
         * Notify message storage
         */
        zmalAccess.getMessageStorage().notifyIMAPFolderModification(moveFullname);
        /*
         * Remove cache entries
         */
        ...
        for (int i = 0; i < perms.length; i++) {
            ...
            try {
                acls.add(aclPermission.getPermissionACL(
                    IMAPFolderConverter.getEntity2AclArgs(session, imapFolder, zmalConfig),
                    zmalConfig,
                    imapStore,
                    ctx));
            } catch (final OXException e) {
                if (Entity2ACLExceptionCode.UNKNOWN_USER.equals(e)) {
                    ...
                    if (DEBUG) {
                        LOG.debug(new StringBuilder().append("User ").append(aclPermission.getEntity()).append(
                            " is not known on IMAP server \"").append(zmalConfig.getImapServerAddress()).append('"').toString());
                    }
    ...
    private IMAPFolder getIMAPFolderWithRecentListener(final String fullName) throws MessagingException {
        ...
        if (MailAccount.DEFAULT_ID == accountId && zmalConfig.getZmalProperties().notifyRecent()) {
